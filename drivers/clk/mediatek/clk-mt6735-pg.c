...
#define MD1_PWR_STA_MASK		BIT(0) /* correct */
#define MD2_PWR_STA_MASK		BIT(22) /* correct */
#define CONN_PWR_STA_MASK		BIT(1) /* correct */
...
static int spm_mtcmos_ctrl_mdsys1(int state)
{
    ...
    if (state == STA_POWER_DOWN) {
        ...
        while (spm_read(SPM_PCM_PASR_DPD_3)) {
            ...
            udelay(1);
            if(count>1000) {
		printk("MD power on: SPM no response\n");
		printk("PCM_IM_PTR : 0x%x (%u) \n", spm_read(SPM_PCM_IM_PTR),
...
struct mtk_power_gate scp_clks[] __initdata = {
	...
	PGATE(SCP_SYS_DIS, pg_dis, NULL, NULL, SYS_DIS), //mm_sel
	PGATE(SCP_SYS_MFG, pg_mfg, pg_mfg_2d, NULL, SYS_MFG), //mfg_sel
	PGATE(SCP_SYS_ISP, pg_isp, NULL, NULL, SYS_ISP), //pre_clk null?
	PGATE(SCP_SYS_VDE, pg_vde, NULL, NULL, SYS_VDE), //vdec_sel
