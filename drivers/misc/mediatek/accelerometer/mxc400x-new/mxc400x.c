...
#define GSE_LOG(fmt, args...)    printk(KERN_ERR fmt, ##args)
#else
#define GSE_TAG
...
#define GPIO_GSE_SDA_PIN (GPIO51 | 0x80000000)
#define GPIO_GSE_SCL_PIN (GPIO52 | 0x80000000)
...
char cust_i2c_write_bytes(unsigned char addr, unsigned char regaddr, unsigned char *udata, unsigned int count)
{
	...
	for( i = 0; i< count; i++)
	{
		...
		if(0 != res)
		{
			printk("MXC400X cust_i2c_write_bytes reg data error:%d \n",__LINE__);
			return -1;
		}
	}
	cust_i2c_stop();
...
static int MXC400X_ReadData(struct i2c_client *client, s16 data[MXC400X_AXES_NUM])
{
	...
	else
	{
		...
		//GSE_LOG("data[MXC400X_AXIS_X] =%d ,data[MXC400X_AXIS_Y] = %d,data[MXC400X_AXIS_Y] =%d\n", data[MXC400X_AXIS_X],data[MXC400X_AXIS_Y],data[MXC400X_AXIS_Z]);
...
static int MXC400X_CheckDeviceID(struct i2c_client *client)
{
	...
	databuf[0] = MXC400X_REG_ID;   
     printk("MXC400X   _______________________05");
#if defined(SW_SIMULATION)
...
#endif
     printk("MXC400X   _______________________06");
	if(res <= 0)
	{
		goto exit_MXC400X_CheckDeviceID;
	}
	...
	databuf[0]= (databuf[0]&0x3f);
     ...
     printk("MXC400X   _______________________07");
	if(databuf[0]!= MXC400X_ID)
	{
    	GSE_ERR("MXC400X_CheckDeviceID databuf=%d failed!\n ", databuf[0]);
		return MXC400X_ERR_IDENTIFICATION;
	}
	
	GSE_ERR("MXC400X_CheckDeviceID %d done!\n ", databuf[0]);
    ...
    while(res < 50)
    {
        cust_hwmsen_read_block(client, 0x0e, databuf, 1);
        msleep(1);
...
static int MXC400X_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);
		...
		printk("MXC400X_SetPowerMode is: %d \n",enable);
...
static int mxc400x_init_client(struct i2c_client *client, int reset_cali)
{
	 struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);
	 int res = 0;
     //printk("MXC400X   _______________________01");
     ...
     //printk("MXC400X   _______________________02");
	res = MXC400X_SetBWRate(client, MXC400X_BW_50HZ);
	if(res != MXC400X_SUCCESS ) 
	{
		return res;
	}
     //printk("MXC400X   _______________________03");
    ...
    // printk("MXC400X   _______________________04");
...
{
    printk("MXC400X ____________________________________10");
...
/* Vanzo:yangzhihong on: Mon, 17 Aug 2015 22:28:11 +0800
        acc[MXC400X_AXIS_Z] = cali_sensor_data[2];
 */
// End of Vanzo:yangzhihong
	    buf[0] = acc[MXC400X_AXIS_X];
        buf[1] = acc[MXC400X_AXIS_Y];
        buf[2] = acc[MXC400X_AXIS_Z];
		sprintf(buf, "%x %x %x", acc[MXC400X_AXIS_X], acc[MXC400X_AXIS_Y], acc[MXC400X_AXIS_Z]);
	}
	return res;
}
static int MXC400X_ReadSensorDataForDaemon(struct i2c_client *client, int *buf, int bufsize)
{
	struct mxc400x_i2c_data *obj = (struct mxc400x_i2c_data*)i2c_get_clientdata(client);
	u8 databuf[20];
	int acc[MXC400X_AXES_NUM];
	...
	res = MXC400X_ReadData(client, obj->data);
	if(res)
	{        
		GSE_ERR("I2C error: ret value=%d", res);
		return MXC400X_ERR_I2C;
...
static int MXC400X_ReadRawData(struct i2c_client *client, int *buf)
{
    ...
    printk("MXC400X ____________________________________09");
	...
	{
		sprintf(buf, "MXC400X_ReadRawData %04x %04x %04x", 
			obj->data[MXC400X_AXIS_X], 
			obj->data[MXC400X_AXIS_Y],
			obj->data[MXC400X_AXIS_Z]);
	}
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	MXC400X_ReadChipInfo(client, (char*)strbuf, MXC400X_BUFSIZE);
       ...
       unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case COMPAT_GSENSOR_IOCTL_GET_LAYOUT:
			...
			if(err < 0)
				{
					printk(KERN_ERR "COMPAT_GSENSOR_IOCTL_GET_LAYOUT is failed!\n");
				}
			break;
...
static long mxc400x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case GSENSOR_IOCTL_READ_CHIPINFO:
			...
			MXC400X_ReadChipInfo(client, (char*)strbuf, MXC400X_BUFSIZE);
			if(copy_to_user(data, strbuf, strlen((char*)strbuf)+1))
			{
				err = -EFAULT;
				break;
			}
		...
		case GSENSOR_IOCTL_READ_SENSORDATA:
            ...
            MXC400X_ReadSensorData(client, strbuf, MXC400X_AXES_NUM);
...
//  begin.................
		...
		case GSENSOR_IOCTL_GET_STATUS:
			 temp_flag=((sensor_power << 1)|(enable_status &0x01));
			 data = (void __user *) arg;
			 if(data == NULL)
			 {
				err = -EINVAL;
		...
		case GSENSOR_IOCTL_GET_DATA:
			 ...
			 err = MXC400X_ReadSensorDataForDaemon(client, vec,MXC400X_AXES_NUM);
		...
		case GSENSOR_IOCTL_READ_REG:
			...
			err = mxc400x_i2c_read_block(client, test, &buf, 1);
			if(err)
			{
				GSE_ERR("error: %d\n", err);
			}
...
static int mxc400x_enable_nodata(int en)
{
	...
	printk("MXC400X_enable_nodata OK!\n");
	enable_status = en;
	return 0;
}
...
{
	int buff[MXC400X_BUFSIZE];
    MXC400X_ReadSensorDataForDaemon(obj_i2c_data->client, buff, MXC400X_BUFSIZE);
	*x = buff[0];
	*y = buff[1];
	*z = cali_sensor_data;
...
//	 struct hwmsen_object sobj;
	...
	struct acc_data_path data={0};
	 ...
	 err = MXC400X_CheckDeviceID(client);
...
//static int mxc400x_probe(struct platform_device *pdev)
static int  mxc400x_local_init(void)
{
	//struct acc_hw *hw = mxc400x_get_cust_acc_hw();
...
static int __init mxc400x_driver_init(void)
{
	...
	const char *name = "mediatek,mxc400x";
