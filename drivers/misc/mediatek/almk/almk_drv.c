...
static int check_all_minfree(void *param, void *param2)
{
    ...
    for_each_process(p) {
        ...
        printk(KERN_ALERT"trying to alloc %d bytes (%d pages)\n"
                "(NR_FREE_PAGES) + "
                "(NR_FILE_PAGES) + "
                "(NR_FILE_DIRTY) - "
                "nr_pages = (%d + %d + %d - %d) = %d\n"
                ...
                free_pages - nr_pages,
                get_min_free_pages(p->pid));
        printk(KERN_ALERT"allocation is %s\n", 
...
static int almk_probe(struct platform_device *pdev)
{
    ...
    class_dev = (struct class_device *)device_create(almk_class, NULL, almk_devno, NULL, ALMK_DEVNAME);
