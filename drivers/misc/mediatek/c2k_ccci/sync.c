...
#define ASCDPRT(fmt, arg...)  do{ \
    if(asc_debug) \
        printk("[C2K MODEM] " fmt, ##arg); \
    }while(0)
#define ASCPRT(fmt, arg...)  printk("[C2K MODEM] " fmt, ##arg)
...
struct asc_tx_handle{
	...
	struct timer_list timer_wait_after_cp_sleep;	//after trigger cp sleep, we should wait for a while before wake cp, to make sure cp can handle ap_wake_cp interrupt
...
static irqreturn_t asc_irq_cp_indicate_state(int irq, void *data)
{
    ...
    c2k_gpio_set_irq_type(cfg->gpio_ready, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
...
static irqreturn_t asc_irq_cp_wake_ap(int irq, void *data)
{
    ...
    c2k_gpio_set_irq_type(cfg->gpio_wake, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
    ...
    if (mt_get_gpio_in(GPIO120)){	//GPIO120 high, md exception happend
        printk("[MODEM SDIO] GPIO120 high, modem exception!\n");
        c2k_exception = 1;
		    gpio_irq_cbp_excp_ind();
    }
    else
    {
        printk("[MODEM SDIO] no exception!\n");
    }
#endif
...
static void asc_rx_notifier(struct asc_rx_handle *rx, int ntf)
{
    ...
    if(ASC_NTF_RX_PREPARE == ntf){
		...
		ASCPRT("asc_rx_notifier start, ntf = %d\n", ntf);
		list_for_each_entry_safe(user, t, &rx->user_list, node){
			infor = &user->infor;
			if(infor->notifier){
...
static int asc_rx_handle_init(struct asc_rx_handle *rx)
{
    ...
    if(((cfg->gpio_wake) & 0xFFFF) >= 0){
        ...
        c2k_gpio_set_irq_type(cfg->gpio_wake, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
        ret = c2k_gpio_request_irq(cfg->gpio_wake, asc_irq_cp_wake_ap, IRQF_SHARED | IRQF_NO_SUSPEND, "cp_wake_ap", rx);
        c2k_gpio_irq_unmask(cfg->gpio_wake);
        if (ret < 0) {
...
static inline void asc_tx_sleep_cp(struct asc_tx_handle *tx)
{
	...
	if (tx->cfg.gpio_wake == AP_USING_REGISTER){
		...
		mod_timer(&tx->timer_wait_after_cp_sleep, jiffies + msecs_to_jiffies(ASC_TX_AFTER_CP_SLEEP_TIME));
		c2k_ap_wake_cp(!tx->cfg.polar);
	}
#endif
...
static inline int asc_tx_cp_be_ready(struct asc_tx_handle *tx)
{
    ...
    if(((tx->cfg.gpio_ready) & 0xFFFF) >= 0)	
        ret = ((!!c2k_gpio_get_value(tx->cfg.gpio_ready)) == (tx->cfg.polar));
...
static int asc_tx_handle_init(struct asc_tx_handle *tx)
{
    ...
    if(((cfg->gpio_ready) & 0xFFFF) >= 0){
        ...
        c2k_gpio_set_irq_type(cfg->gpio_ready, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
        ret = c2k_gpio_request_irq(cfg->gpio_ready, asc_irq_cp_indicate_state, 
	...
	atomic_set(&tx->trigger_cp_sleep, 0);
	setup_timer(&tx->timer_wait_after_cp_sleep, asc_tx_wait_after_cp_sleep, (unsigned long)tx);
    asc_tx_sleep_cp(tx);
...
EXPORT_SYMBOL(asc_tx_ready_count);
