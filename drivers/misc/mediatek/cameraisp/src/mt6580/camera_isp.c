...
static MUINT32 g_prv_tstamp_s[_rt_dma_max_] = {0};
static MUINT32 g_prv_tstamp_us[_rt_dma_max_] = {0};
		...
		case ISP_RT_BUF_CTRL_DEQUE:
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
				...
				for (i = 0; i < g_deque_buf.count; i++) {
					...
					/* { */
					if (0 != g_prv_tstamp_s[rt_dma]) {
					/**/	if ((g_prv_tstamp_s[rt_dma] >
						     g_deque_buf.data[i].timeStampS)
						    ||
						    ((g_prv_tstamp_s[rt_dma] ==
						      g_deque_buf.data[i].timeStampS)
						     && (g_prv_tstamp_us[rt_dma] >=
							 g_deque_buf.data[i].timeStampUs))) {
							LOG_ERR
							    ("[rtbc]TS rollback,prv\"%d.%06d\",cur\"%d.%06d\"",
							     g_prv_tstamp_s[rt_dma], g_prv_tstamp_us[rt_dma],
							     g_deque_buf.data[i].timeStampS,
							     g_deque_buf.data[i].timeStampUs);
						}
					}

					g_prv_tstamp_s[rt_dma] = g_deque_buf.data[i].timeStampS;
					g_prv_tstamp_us[rt_dma] = g_deque_buf.data[i].timeStampUs;
					/* } */
				}
...
static MINT32 ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	if (g_IspInfo.UserCount > 0) {
		...
		LOG_DBG("Curr UserCount(%d), (process, pid, tgid)=(%s, %d, %d), log_limit_line(%d), first user\n",
			g_IspInfo.UserCount, current->comm, current->pid, current->tgid, i);
	}
	#if (LOG_CONSTRAINT_ADJ == 1)
...
static MINT32 ISP_release(struct inode *pInode, struct file *pFile)
{
	...
	LOG_DBG("Curr UserCount(%d), (process, pid, tgid)=(%s, %d, %d), log_limit_line(%d),	last user",
		g_IspInfo.UserCount, current->comm, current->pid, current->tgid, pr_detect_count);
