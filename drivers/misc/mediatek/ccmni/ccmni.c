    ...
    if (packet_type == IPV6_VERSION) {
        ...
        if (total_len <= 128 - sizeof(struct ccci_header)) {
            ...
            if (unlikely(ccmni_debug_level&CCMNI_DBG_LEVEL_ACK_SKB)) {
                CCMNI_INF_MSG(md_id, "[SKB] ack=%d: proto=%d syn=%d fin=%d rst=%d ack=%d tot_len=%d l4_off=%d doff=%d\n",\
	                ret, nexthdr, tcph->syn, tcph->fin, tcph->rst, tcph->ack, total_len, l4_off, tcph->doff);
            }
        } else {
            if (unlikely(ccmni_debug_level&CCMNI_DBG_LEVEL_ACK_SKB)) {
        ...
        if (ntohs(iph->tot_len) <= 128 - sizeof(struct ccci_header)) {
            ...
            if (unlikely(ccmni_debug_level&CCMNI_DBG_LEVEL_ACK_SKB)) {
                CCMNI_INF_MSG(md_id, "[SKB] ack=%d: proto=%d syn=%d fin=%d rst=%d ack=%d tot_len=%d ihl=%d doff=%d\n",\
	                ret, iph->protocol, tcph->syn, tcph->fin, tcph->rst, tcph->ack, ntohs(iph->tot_len), iph->ihl, tcph->doff);
            }
        } else {
            if (unlikely(ccmni_debug_level&CCMNI_DBG_LEVEL_ACK_SKB)) {
...
static void ccmni_dbg_eth_header(int md_id, bool tx, struct ethhdr *ethh){
    if (ethh != NULL) {
    	 CCMNI_INF_MSG(md_id, "[SKB][%s] ethhdr: proto=0x%04x dest_mac=%02x:%02x:%02x:%02x:%02x:%02x src_mac=%02x:%02x:%02x:%02x:%02x:%02x\n",\
...
static void ccmni_dbg_ip_header(int md_id, bool tx, struct iphdr *iph){
    if (iph != NULL) {
    	 CCMNI_INF_MSG(md_id, "[SKB][%s] iphdr: ihl=0x%02x ver=0x%02x tos=0x%02x tot_len=0x%04x id=0x%04x frag_off=0x%04x ttl=0x%02x proto=0x%02x check=0x%04x saddr=0x%08x daddr=0x%08x\n",\
...
static void ccmni_dbg_tcp_header(int md_id, bool tx, struct tcphdr *tcph){
    if (tcph != NULL) {
    	 CCMNI_INF_MSG(md_id, "[SKB][%s] tcp_hdr: src=0x%04x dest=0x%04x seq=0x%08x ack_seq=0x%08x urg=%d ack=%d psh=%d rst=%d syn=%d fin=%d\n",\
...
static int ccmni_open(struct net_device *dev)
{
	...
	CCMNI_INF_MSG(ccmni->md_id, "CCMNI%d_Open: cnt=(%d,%d), md_ab=0x%X\n", \
		ccmni->index, atomic_read(&ccmni->usage), atomic_read(&ccmni_tmp->usage), ccmni_ctl->ccci_ops->md_ability);
	return 0;
}
...
static int ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if(unlikely(skb->len > dev->mtu)) { //dev->mtu is changed  if dev->mtu is changed by upper layer
	...
	if (unlikely(ccmni_debug_level&CCMNI_DBG_LEVEL_TX)) {
		CCMNI_INF_MSG(ccmni->md_id, "[TX]CCMNI%d head_len=%d len=%d ack=%d tx_ch=%d\n", ccmni->index, \
			skb_headroom(skb), skb->len, is_ack, ccci_tx_ch);
	}
...
static int ccmni_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	switch(cmd) {
		case SIOCSTXQSTATE:
			...
			if (likely(ccmni_tmp != NULL)) {
				CCMNI_INF_MSG(ccmni->md_id, "SIOCSTXQSTATE: CCMNI%d_state=0x%x, cnt=(%d, %d)\n", \
					ccmni->index, ifr->ifr_ifru.ifru_ivalue, atomic_read(&ccmni->usage), atomic_read(&ccmni_tmp->usage));
			} else {
		...
		case SIOCCCMNICFG:
			...
			CCMNI_INF_MSG(md_id, "SIOCCCMNICFG: CCMNI%d iRAT MD%d->MD%d, dev_cnt=%d, md_cnt=%d, md_irat_cnt=%d\n", \
				ccmni->index, (md_id+1), (ifr->ifr_ifru.ifru_ivalue+1), atomic_read(&ccmni->usage), atomic_read(&ccmni_tmp->usage), atomic_read(&ccmni_irat->usage));
			break;
