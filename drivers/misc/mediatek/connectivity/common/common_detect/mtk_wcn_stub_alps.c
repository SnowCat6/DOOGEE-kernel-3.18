...
static void mtk_wcn_cmb_sdio_request_eirq(msdc_sdio_irq_handler_t irq_handler, void *data)
{
	...
	if (node) {
		...
		ret = request_irq(wifi_irq, mtk_wcn_cmb_sdio_eirq_handler_stub, IRQF_TRIGGER_LOW,
				"WIFI-eint", NULL);
		/* request_irq() indicates we can handle interrupts now so the
		 * irq_enable_flag should be true. And irq_enable_flag also
		 * makes the following mtk_wcn_cmb_sdio_disable_eirq() really
		 * calls disable_irq_nosync() to avoid interrupts coming too early.
		 */
		atomic_set(&irq_enable_flag, 1);
		CMB_STUB_LOG_INFO("WIFI EINT irq %d !!\n", wifi_irq);
#endif
