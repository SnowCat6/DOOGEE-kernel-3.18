...
/*step1.PMIC ctrl*/
		...
		pmic_set_register_value(MT6350_VCN_1V8_CON1, 0);
#else
#if !defined(CONFIG_MTK_PMIC_LEGACY)
		...
		if (co_clock_type) {
...
#if CONSYS_CLOCK_BUF_CTRL
			clk_buf_ctrl(CLK_BUF_CONNSRC, 1);
#endif
			...
			CONSYS_REG_WRITE((conn_reg.da_xobuf_base + CONSYS_DA_XOBUF_OFFSET),
				CONSYS_REG_READ(conn_reg.da_xobuf_base + CONSYS_DA_XOBUF_OFFSET) & ~(0x1 << 6));
			CONSYS_REG_WRITE((conn_reg.da_xobuf_base + CONSYS_DA_XOBUF_OFFSET),
				CONSYS_REG_READ(conn_reg.da_xobuf_base + CONSYS_DA_XOBUF_OFFSET) | (0x1 << 7));
...
#endif
#if 0
		pmic_read_interface(0x512, &pmic_value, 0xffff, 0);
		WMT_PLAT_ERR_FUNC("PMIC:0x512:%d\n", pmic_value);
		pmic_read_interface(0x41c, &pmic_value, 0xffff, 0);
		WMT_PLAT_ERR_FUNC("PMIC:0x41C:%d\n", pmic_value);
#endif
/*step2.MTCMOS ctrl*/
...
#if MT6580_WORKAROUND
		pmic_set_register_value(MT6350_VCN_1V8_CON1, 1);
#else
#if !defined(CONFIG_MTK_PMIC_LEGACY)
		pmic_set_register_value(PMIC_VCN_1V8_ON_CTRL, 0);
		if (reg_VCN18) {
...
INT32 mtk_wcn_consys_hw_init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6580-consys");
	if (node) {
