...
wmt_stp_wifi_lte_coex (VOID)
{
	...
	(0x8163 == wmt_ic_ops_soc.icId) || 
       ...
       (0x0337 == wmt_ic_ops_soc.icId))
		{
			iRet = wmt_core_init_script(set_wifi_lte_coex_table_6752, osal_array_size(set_wifi_lte_coex_table_6752));
			WMT_INFO_FUNC("wmt_core:set_wifi_lte_coex_table_6752 %s(%d)\n",iRet ? "fail" : "ok",iRet);
		}
		else
		{
    		iRet = wmt_core_init_script(set_wifi_lte_coex_table_0, osal_array_size(set_wifi_lte_coex_table_0));
			WMT_INFO_FUNC("wmt_core:set_wifi_lte_coex_table_0 %s(%d)\n",iRet ? "fail" : "ok",iRet);
		}
	}
