...
static INT32 mtk_wmt_probe(struct platform_device *pdev)
{
	...
	WMT_PLAT_INFO_FUNC("[CCF]clk_infra_conn_main=%p\n",clk_infra_conn_main);
#endif /* !defined(CONFIG_MTK_LEGACY) */
...
/*step1.PMIC ctrl*/	
		...
		else
		{
			...
			hwPowerOn(MT6328_POWER_LDO_VCN28, VOL_2800, "wcn_drv"); /*fix vcn28 not balance warning*/
		}
#endif
...
/*step2.MTCMOS ctrl*/
...
#else
		WMT_PLAT_INFO_FUNC("[CCF] clk_prepare_enable(clk_scp_conn_main) calling\n");
		clk_prepare_enable(clk_scp_conn_main);
#endif /* defined(CONFIG_MTK_LEGACY) */
#else
		...
		WMT_PLAT_INFO_FUNC("enable MT_CG_INFRA_CONNMCU_BUS CLK\n");
#else
		clk_prepare_enable(clk_infra_conn_main);
		WMT_PLAT_INFO_FUNC("[CCF]enable clk_infra_conn_main\n");
#endif /* defined(CONFIG_MTK_LEGACY) */
#endif
	...
	/*3.assert CONNSYS CPU SW reset  0x10007018	"[12]=1'b1  [31:24]=8'h88 (key)"*/
		...
		WMT_PLAT_INFO_FUNC("[CCF] clk_disable_unprepare(clk_infra_conn_main) calling\n");
#endif /* defined(CONFIG_MTK_LEGACY) */
#endif
...
#else
		WMT_PLAT_INFO_FUNC("[CCF] clk_disable_unprepare(clk_scp_conn_main) calling\n");
...
INT32
mtk_wcn_consys_hw_rst (UINT32 co_clock_type)
{
    INT32 iRet = 0;
...
{
	WMT_PLAT_INFO_FUNC(" name: %s, uname: %s, base: 0x%llx, size: 0x%llx\n", rmem->name, uname,
...
INT32 mtk_wcn_consys_hw_init()
{
	...
	{
		WMT_PLAT_INFO_FUNC("WMT platform driver registered OK\n");
	}
	return iRet;
