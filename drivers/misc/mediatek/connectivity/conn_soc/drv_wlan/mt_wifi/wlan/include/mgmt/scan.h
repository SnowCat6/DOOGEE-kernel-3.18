...
typedef struct _GSCAN_CHANNEL_BUCKET_T{
    ...
    UINT_8                      ucBucketFreqMultiple;      // desired period, in millisecond; if this is too low, the firmware should choose to generate results as fast as it can instead of failing the command
...
scnSetGSCNParam (
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_GSCN_REQ_T prCmdGscnParam
    );
