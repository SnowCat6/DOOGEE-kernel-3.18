    ...
    )
{
	...
	if(prScanParam->ucChannelListNum > 32) {
	    ...
	    if(prScanParam->eNetTypeIndex == NETWORK_TYPE_P2P_INDEX) {
	        rCmdScanReq.u2ChannelDwellTime = prScanParam->u2PassiveListenInterval;
					if(prScanParam->ucChannelListNum == 1)
					{
						rCmdScanReq.u2ChannelMinDwellTime = 100;
						rCmdScanReq.u2ChannelDwellTime = 10;
					}
	    }
#endif
    ...
    )
{
    ...
    if (prScanParam->u2IELen) {
        kalMemCopy(prCmdNloReq->aucIE,
                prScanParam->aucIE,
                sizeof(UINT_8) * prCmdNloReq->u2IELen);
    }
    ...
    )
{
    ...
    if(ucPscanAct == ENABLE)
        ...
        wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
            sizeof(CMD_SET_PSCAN_ENABLE),
            (PUINT_8)&rCmdPscnAction,
            NULL,
            0);
...
scnFsmPSCNSetParam (
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_SET_PSCAN_PARAM prCmdPscnParam
    )
{
    ...
    #if 1          
    printk("rCmdPscnParam.rCmdGscnParam : u4BasePeriod[%u], ucNumScnToCache[%u],  u4BufferThreshold[%u], u4NumBuckets[%u],fgGSCN[%d] fgNLO[%d] fgBatch[%d]    \n",
    ...
    prCmdPscnParam->rCmdGscnReq.u4NumBuckets,
    prCmdPscnParam->fgGScnEnable,
    ...
    for(i = 0; i < prCmdPscnParam->rCmdGscnReq.u4NumBuckets; i++){
        ...
        printk("rCmdPscnParam.rCmdGscnReq.arChannelBucket[%d] : band[%u], arChannelBucket[%u] ucNumChannels[%u], ucBucketFreqMultiple[%u] ucReportFlag[%u] \n",
        i, 
        ...
        printk("rCmdPscnParam.rCmdGscnParam.arChannelBucket[%d] has channel: ", i );
    
        for(j = 0; j < prCmdPscnParam->rCmdGscnReq.arChannelBucket[i].ucNumChannels; j++){                   
            printk(" %d,  ",prCmdPscnParam->rCmdGscnReq.arChannelBucket[i].arChannelList[j].ucChannel);        
        }                 
        printk("  \n" );
   }      
   #endif 
    ...
    if(1 /*prScanInfo->fgPscnOnnning == FALSE*/ ) {
        ...
        wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
            sizeof(CMD_SET_PSCAN_PARAM),
            (PUINT_8)prCmdPscnParam,
            NULL,
            0);
...
/*----------------------------------------------------------------------------*/
BOOLEAN
scnFsmPSCNSetHotlist (
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_SET_PSCAN_ADD_HOTLIST_BSSID prCmdPscnAddHotlist
    )
{
    ASSERT(prAdapter);
    ...
    if( prScanInfo->fgPscnOnnning && prScanInfo->prPscnParam->fgGScnEnable) {
        ...
        wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
            sizeof(CMD_SET_PSCAN_ADD_HOTLIST_BSSID),
            (PUINT_8)&rCmdPscnAddHotlist,
            NULL,
            0);
...
/*----------------------------------------------------------------------------*/
BOOLEAN
scnFsmPSCNAddSWCBssId (
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_SET_PSCAN_ADD_SWC_BSSID prCmdPscnAddSWCBssId
    )
{
    ...
    if( prScanInfo->fgPscnOnnning && prScanInfo->prPscnParam->fgGScnEnable) {
        ...
        wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
            sizeof(CMD_SET_PSCAN_ADD_SWC_BSSID),
            (PUINT_8)&rCmdPscnAddSWCBssId,
            NULL,
            0);
...
/*----------------------------------------------------------------------------*/
BOOLEAN
scnFsmPSCNSetMacAddr (
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_SET_PSCAN_MAC_ADDR prCmdPscnSetMacAddr
    )
{
    ...
    if( 1 /* (prScanInfo->fgPscnOnnning == TRUE*/ ) {
        ...
        wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
            sizeof(CMD_SET_PSCAN_MAC_ADDR),
            (PUINT_8)&rCmdPscnSetMacAddr,
            NULL,
            0);
...
scnSetGSCNParam (
    IN P_ADAPTER_T prAdapter,
    IN P_PARAM_WIFI_GSCAN_CMD_PARAMS prCmdGscnParam
    )
{
       
    DBGLOG(SCN, INFO, ("scnSetGSCNParam s s  \n")); 
    ASSERT(prAdapter);
    ...
    printk("u4BasePeriod[%d], u4NumBuckets[%d]: ", rCmdGscnParam.u4BasePeriod, rCmdGscnParam.u4NumBuckets);
    printk("\n");
    for(ucChannelBuckIndex; ucChannelBuckIndex < prCmdGscnParam->num_buckets ; ucChannelBuckIndex++){
        
        printk("assign channels to bucket[%d]:\n",ucChannelBuckIndex );

        for(i = 0; i < prCmdGscnParam->buckets[ucChannelBuckIndex].num_channels; i++){

            rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].arChannelList[i].ucChannel= (UINT_8)nicFreq2ChannelNum(prCmdGscnParam->buckets[ucChannelBuckIndex].channels[i].channel*1000);
            rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].arChannelList[i].ucPassive= (UINT_8)prCmdGscnParam->buckets[ucChannelBuckIndex].channels[i].passive;
            rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].arChannelList[i].u4DwellTimeMs = (UINT_8)prCmdGscnParam->buckets[ucChannelBuckIndex].channels[i].dwellTimeMs;

            printk(" [ucChannel %d, ucPassive %d, u4DwellTimeMs %d\n", 
        ...
        rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].u2BucketIndex= (UINT_16)prCmdGscnParam->buckets[ucChannelBuckIndex].bucket;
        rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].eBand= prCmdGscnParam->buckets[ucChannelBuckIndex].band;
        rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].ucBucketFreqMultiple =  (prCmdGscnParam->buckets[ucChannelBuckIndex].period /prCmdGscnParam->base_period);
        rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].ucNumChannels = prCmdGscnParam->buckets[ucChannelBuckIndex].num_channels;
        rCmdGscnParam.arChannelBucket[ucChannelBuckIndex].ucReportFlag = prCmdGscnParam->buckets[ucChannelBuckIndex].report_events;
    ...
    )
{
    ...
    if(prNewCmdBatchReq){
        ...
        memcpy(&prCmdPscnParam->rCmdBatchReq, &prScanInfo->prPscnParam->rCurrentCmdBatchReq, sizeof(CMD_BATCH_REQ_T));
    }
    else
    ...
    )
{
    ...
    if(prNewCmdGscnReq){       
        ...
        printk("no new prNewCmdGscnReq but there is a old one  \n" );
        memcpy(&prCmdPscnParam->rCmdGscnReq, &prScanInfo->prPscnParam->rCurrentCmdGscnReq, sizeof(CMD_GSCN_REQ_T));   
    ...
    if(prNewCmdGscnConfig){
        printk("set up prNewCmdGscnConfig  \n" );
        ...
        printk("no new prNewCmdGscnConfig but there is a old one  \n" );        
        memcpy( prCmdPscnParam , prScanInfo->prPscnParam, sizeof(CMD_SET_PSCAN_PARAM));
        prCmdPscnParam->rCmdGscnReq.u4BufferThreshold = prScanInfo->prPscnParam->rCurrentCmdGscnReq.u4BufferThreshold;
        prCmdPscnParam->rCmdGscnReq.ucNumScnToCache = (UINT_8)prScanInfo->prPscnParam->rCurrentCmdGscnReq.ucNumScnToCache;       
    }
    else
    ...
    )
{
   ...
   
   {
        ...
        if(fgRemoveBatchSCNfromPSCN){
            ...
            kalMemZero(&prScanInfo->prPscnParam->rCurrentCmdBatchReq, sizeof(CMD_NLO_REQ));
        }
        if(fgRemoveGSCNfromPSCN) {
            ...
            kalMemZero(&prScanInfo->prPscnParam->rCurrentCmdGscnReq, sizeof(CMD_NLO_REQ));
        }
        ...
        if(!fgRemoveNLOfromPSCN && !fgRemoveBatchSCNfromPSCN && !fgRemoveGSCNfromPSCN){    
            ...
            DBGLOG(SCN, INFO, ("(!fgRemoveNLOfromPSCN && !fgRemoveBatchSCNfromPSCN && !fgRemoveGSCNfromPSCN)<-----  \n"));           
        }
        else{            
    ...
    )
{
    ...
    if(fgRemoveNLOfromPSCN || fgRemoveBatchSCNfromPSCN || fgRemoveGSCNfromPSCN)   {
        ...
        scnRemoveFromPSCN(prAdapter,
            fgRemoveNLOfromPSCN, 
            fgRemoveBatchSCNfromPSCN, 
            fgRemoveGSCNfromPSCN, 
            prCmdPscnParam );
...
scnFsmSetGSCNConfig (
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_GSCN_SCN_COFIG_T prCmdGscnScnConfig
    )
{
    ASSERT(prAdapter);
...
scnFsmGetGSCNResult (
    IN P_ADAPTER_T prAdapter,
    IN P_CMD_GET_GSCAN_RESULT_T prGetGscnScnResultCmd
    )
{
    ...
    if( prScanInfo->fgPscnOnnning && prScanInfo->prPscnParam->fgGScnEnable) {
        wlanSendSetQueryCmd(prAdapter,
            ...
            NULL,
            NULL,
            sizeof(CMD_GET_GSCAN_RESULT_T),
            (PUINT_8)&rGetGscnScnResultCmd,
            NULL,
            0);

        printk("SET to FW !! rGetGscnScnResultCmd: ucGetNum [%d]  fgFlush [%d] \n", rGetGscnScnResultCmd.u4Num , rGetGscnScnResultCmd.ucFlush  );      
        ...
        switch(prScanInfo->eCurrentPSCNState) {
            case PSCN_IDLE:  
                ...
                if(eNextPSCNState == PSCN_RESET) {
                    if(fgRemoveNLOfromPSCN ||fgRemoveBatchSCNfromPSCN || fgRemoveGSCNfromPSCN ){
                        printk ("Unexpected remove NLO/BATCH/GSCN request\n");
                        eNextPSCNState = PSCN_IDLE;
                        break;
                    }
                    
                    if(prCmdNloReq ||prCmdBatchReq   ){  // 
                        printk("PSCN_IDLE->PSCN_RESET,.... scnFsmPSCNActionDISABLE\n");
                    ...
                    if(fgEnableGSCN){
                        if(prScanInfo->fgGScnParamSet ){
                            printk ("PSCN_IDLE->PSCN_SCANNING,.... scnFsmPSCNActionENABLE\n"); 
            ...
            case PSCN_RESET:
                ...
                if(!prScanInfo->prPscnParam->fgNLOScnEnable && !prScanInfo->prPscnParam->fgBatchScnEnable && !prScanInfo->prPscnParam->fgGScnEnable){
                    printk("PSCN_RESET->PSCN_IDLE,.... fgNLOScnEnable/fgBatchScnEnable/fgGScnEnable false\n");    
                    eNextPSCNState = PSCN_IDLE;
                }
                else{
                    if(prScanInfo->prPscnParam->fgNLOScnEnable || prScanInfo->prPscnParam->fgBatchScnEnable){
                        ...
                        scnFsmPSCNAction(prAdapter, ENABLE);
                        eNextPSCNState = PSCN_SCANNING;
                        printk("PSCN_RESET->PSCN_SCANNING,.... fgNLOScnEnable/fgBatchScnEnable ENABLE\n");         
                    }
                }
            ...
            case PSCN_SCANNING:
                printk ("PSCN_SCANNING\n");
                if(eNextPSCNState == PSCN_RESET) {
                    if(fgRemoveNLOfromPSCN ||fgRemoveBatchSCNfromPSCN || fgRemoveGSCNfromPSCN ){
                        printk("PSCN_SCANNING->PSCN_RESET,.... fgRemoveNLOfromPSCN/fgRemoveBatchSCNfromPSCN/fgRemoveGSCNfromPSCN \n"); 
                        scnFsmPSCNAction(prAdapter, DISABLE);
                        break;
                    }
                    
                    if(prCmdNloReq ||prCmdBatchReq || prCmdGscnReq || prNewCmdGscnConfig ){
                        printk("PSCN_SCANNING->PSCN_RESET,.... prCmdNloReq/prCmdBatchReq/prCmdGscnReq/prNewCmdGscnConfig \n"); 
                        scnFsmPSCNAction(prAdapter, DISABLE);
                        break;                        
                    }
                    ...
                    if(fgEnableGSCN){
                        if(prScanInfo->prPscnParam->fgGScnEnable && (!prScanInfo->fgPscnOnnning) ){
                            printk ("PSCN_SCANNING->PSCN_SCANNING,.... fgGScnEnable/!fgPscnOnnning \n");
                            //scnFsmPSCNAction(prAdapter, ENABLE);
                            eNextPSCNState = PSCN_SCANNING;
                        }
                        else{
            ...
            default:
                DBGLOG(SCN, LOUD, ("Unexpected state\n"));
                ASSERT(0);
                break;
