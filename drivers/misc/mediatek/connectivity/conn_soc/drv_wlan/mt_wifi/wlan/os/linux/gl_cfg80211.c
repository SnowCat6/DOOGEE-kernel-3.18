...
int mtk_cfg80211_vendor_get_capabilities(
...
int mtk_cfg80211_vendor_set_config(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    const void *data, int data_len)
{
    WLAN_STATUS rStatus;
    ...
    for(i=0; i<13; i++)
        ...
        *((UINT_32 *)data+i*4+1), *((UINT_32 *)data+i*4+2), *((UINT_32 *)data+i*4+3));
    ...
    nla_parse_nested(attr, GSCAN_ATTRIBUTE_REPORT_EVENTS,
        (struct nlattr *)(data - NLA_HDRLEN), policy);
    len_basic = 0;
    ...
    for (i = 0; i < prWifiScanCmd->num_buckets; i++) {
        ...
        pbucket = (struct nlattr *)((UINT_8 *)pbucket + NLA_HDRLEN); //request.attr_start(i) as nested attribute
        //printk("+++pure bucket size=%d pbucket=%p \r\n", len_bucket, pbucket);
        pbucket = (struct nlattr *)((UINT_8 *)pbucket + len_bucket); //pure bucket payload, not include channels
...
int mtk_cfg80211_vendor_set_scan_config(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    const void *data, int data_len)
{
    WLAN_STATUS rStatus;
    ...
    printk("%s for vendor command: data_len=%d \r\n", __FUNCTION__, data_len);
    for(i=0; i<2; i++)
        printk("0x%x 0x%x 0x%x 0x%x \r\n", *((UINT_32 *)data+i*4), *((UINT_32 *)data+i*4+1), *((UINT_32 *)data+i*4+2), *((UINT_32 *)data+i*4+3));
    ...
    printk("attr=0x%x, attr2=0x%x ", *(UINT_32 *)attr[GSCAN_ATTRIBUTE_NUM_AP_PER_SCAN], *(UINT_32 *)attr[GSCAN_ATTRIBUTE_REPORT_THRESHOLD]);
    ...
    const void *data, int data_len)
{
    ...
    {
        nla_parse_nested(attr, GSCAN_ATTRIBUTE_RSSI_HIGH, 
            (struct nlattr *)paplist, policy);
        paplist = (struct nlattr *)((UINT_8 *)paplist + NLA_HDRLEN); //request.attr_start(i) as nested attribute
        len_aplist = 0;
    ...
    printk("flush=%d, rssi_sample_size=%d lost_ap_sample_size=%d min_breaching=%d,"
        ...
        flush, rWifiChangeCmd.rssi_sample_size, rWifiChangeCmd.lost_ap_sample_size, rWifiChangeCmd.min_breaching,
...
int mtk_cfg80211_vendor_set_hotlist(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    const void *data, int data_len)
{
    WLAN_STATUS rStatus;
    ...
    nla_parse_nested(attr, GSCAN_ATTRIBUTE_NUM_AP,
        (struct nlattr *)(data - NLA_HDRLEN), policy);
    len_basic = 0;
...
int mtk_cfg80211_vendor_enable_scan(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    const void *data, int data_len)
{
    WLAN_STATUS rStatus;
    ...
    printk("%s for vendor command: data_len=%d, data=0x%x 0x%x\r\n", 
        __FUNCTION__, data_len, *((UINT_32 *)data), *((UINT_32 *)data+1));
    ...
    rStatus = kalIoctl(prGlueInfo,
            ...
            &rWifiScanActionCmd,
            sizeof(PARAM_WIFI_GSCAN_ACTION_CMD_PARAMS),
            FALSE,
            FALSE,
            TRUE,
            FALSE,
...
int mtk_cfg80211_vendor_get_scan_results(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    const void *data, int data_len)
{
    WLAN_STATUS rStatus;
    ...
    g_GetResultsCmdCnt++;
    DBGLOG(INIT, INFO, ("(g_GetResultsCmdCnt [%d], g_GetResultsBufferedCnt [%d] \n",g_GetResultsCmdCnt, g_GetResultsBufferedCnt));
    ...
    if(g_GetResultsBufferedCnt > 0){
        ...
        DBGLOG(INIT, INFO, ("(g_GetResultsBufferedCnt > 0), report buffered results instead of ask from FW \n"));
...
#endif
    {
        ...
        DBGLOG(INIT, INFO, ("no buffered gscn results, ask form FW \n"));
            ...
            &rGSscnResultParm,
            sizeof(PARAM_WIFI_GSCAN_GET_RESULT_PARAMS),
            FALSE,
            FALSE,
            TRUE,
            FALSE,
...
int mtk_cfg80211_vendor_get_channel_list(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    const void *data, int data_len)
{
    INT_32 i4Status = -EINVAL;
...
int mtk_cfg80211_vendor_event_complete_scan(
    struct wiphy *wiphy, struct wireless_dev *wdev,
...
int mtk_cfg80211_vendor_event_scan_results_avaliable(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    UINT_32 num)
{
    struct sk_buff *skb;
...
int mtk_cfg80211_vendor_event_full_scan_results(
    struct wiphy *wiphy, struct wireless_dev *wdev,
...
int mtk_cfg80211_vendor_event_significant_change_results(
    struct wiphy *wiphy, struct wireless_dev *wdev,
...
int mtk_cfg80211_vendor_event_hotlist_ap_found(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    P_PARAM_WIFI_GSCAN_RESULT pdata, UINT_32 data_len)
{
    struct sk_buff *skb;
...
int mtk_cfg80211_vendor_event_hotlist_ap_lost(
    struct wiphy *wiphy, struct wireless_dev *wdev,
    P_PARAM_WIFI_GSCAN_RESULT pdata, UINT_32 data_len)
{
    struct sk_buff *skb;
