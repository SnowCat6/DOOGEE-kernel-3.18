...
typedef struct _PARAM_WIFI_GSCAN_BUCKET_SPEC{
    ...
    PARAM_WIFI_GSCAN_CHANNEL_SPEC channels[GSCAN_MAX_CHANNELS];  // channels to scan; these may include DFS channels
...
typedef struct {
   ...
   UINT_32 on_time_roam_scan;          // msecs the radio is awake due to roam?scan (32 bits number accruing over time)
   UINT_32 on_time_pno_scan;           // msecs the radio is awake due to PNO scan (32 bits number accruing over time)
   UINT_32 on_time_hs20;               // msecs the radio is awake due to HS2.0 scans and GAS exchange (32 bits number accruing over time)
...
typedef struct {
   ...
   UINT_32 tx_mcast;                   // number of succesfully transmitted multicast data packets
