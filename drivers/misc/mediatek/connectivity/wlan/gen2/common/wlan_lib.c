...
WLAN_STATUS wlanUpdateNetworkAddress(IN P_ADAPTER_T prAdapter)
{
	...
	if (kalRetrieveNetworkAddress(prAdapter->prGlueInfo, &rMacAddr) == FALSE || IS_BMCAST_MAC_ADDR(rMacAddr)
		...
		DBGLOG(INIT, TRACE, "Using dynamically generated MAC address");
			...
			DBGLOG(INIT, TRACE, "dynamically read %x %x %x %x %x %x",rMacAddr[0],rMacAddr[1],rMacAddr[2],rMacAddr[3],rMacAddr[4],rMacAddr[5]);
...
#else
             get_random_bytes(&ramdom_1, sizeof(ramdom_1));
             kalMemCopy(&rMacAddr[1], &ramdom_1, 4);
             get_random_bytes(&ramdom_2, sizeof(ramdom_2));
             rMacAddr[5] =ramdom_2 ;
#endif
			...
			DBGLOG(INIT, TRACE, "dynamically write%x %x %x %x %x %x",rMacAddr[0],rMacAddr[1],rMacAddr[2],rMacAddr[3],rMacAddr[4],rMacAddr[5]);
			}
        }
    }
    else {
