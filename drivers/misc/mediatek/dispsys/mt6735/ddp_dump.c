...
static void mutex_dump_analysis(void)
{
    ...
    for (i = 0; i < 5; i++)
    {
            ...
            DISP_REG_GET(DISP_REG_CONFIG_MUTEX_SOF(i)+0x20*i)==SOF_SINGLE))
        {
          ...
          DDPDUMP("%s)\n",mutex_module);
          DDPDUMP("irq cnt: start=%d, end=%d \n", mutex_start_irq_cnt, mutex_done_irq_cnt);
        }
    } 
    return;
}
...
static void  mmsys_config_dump_reg(void)
{
    ...
    for (i = 0; i < 4*16; i += 16)
    {
        DDPMSG("SPM_PCM_DATA+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i, INREG32(SPM_PCM_REG0_DATA + i), INREG32(SPM_PCM_REG0_DATA + i + 0x4), INREG32(SPM_PCM_REG0_DATA + i + 0x8), INREG32(SPM_PCM_REG0_DATA + i + 0xc));
    }
#endif
...
static void  mmsys_config_dump_analysis(void)
{
    ...
    printk("==DISP MMSYS_CONFIG ANALYSIS==\n");
	printk("[ddp] mmsys_clock=0x%x\n", DISP_REG_GET(DISP_REG_CLK_CFG_0_MM_CLK));
    ...
    // AEE status dump
    {
        ...
        {
            DDPDUMP("isAEEEnabled=%d, O0L3=0x%x, AEE=0x%lx \n",isAEEEnabled, DISP_REG_GET(DISP_REG_OVL_L3_ADDR), get_Assert_Layer_PA());
