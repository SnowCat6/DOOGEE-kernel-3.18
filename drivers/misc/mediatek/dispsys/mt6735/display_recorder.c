...
int dprec_init(void)
{
    ...
    {
        // extern void sodi_mmp_init(void);
        // sodi_mmp_init();
    }
...
static unsigned long long ts_dprec_reset = 0;
...
static int dprec_logger_get_result_value(DPREC_LOGGER_ENUM source, unsigned long long *pcount, unsigned long long *pavg, unsigned long long *pfps, unsigned long long *pmax, unsigned long long *pmin)
{
	unsigned long flags = 0;
	spin_lock_irqsave(&gdprec_logger_spinlock, flags);
	int len = 0;
...
{        
	if(gCaptureWdmaLayerEnable)
	{
        ddp_mmp_rdma_layer( (RDMA_CONFIG_STRUCT *)rdma_layer, rdma_num,  gCapturePriLayerDownX,gCapturePriLayerDownY);
    }
	return -1;
}
...
static unsigned int dprec_logger_error_id = 0;
...
static unsigned int dprec_logger_fence_id = 0;
...
static unsigned int dprec_logger_hwop_id = 0;
