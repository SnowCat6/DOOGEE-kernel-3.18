...
extern unsigned long get_Assert_Layer_PA(void);
...
static char* ovl_get_status_name(DISP_OVL1_STATUS status)
{
    switch(status)
    {
        ...
        case DDP_OVL1_STATUS_PRIMARY_DISABLE: return "primary_disabled";
...
unsigned int ddp_ovl_get_cur_addr(bool rdma_mode, int layerid )
{
    ...
    else
    {
        ...
        {	//OVL 0
            if(DISP_REG_GET(DISP_REG_OVL_RDMA0_CTRL+(layerid-OVL1_MAX_LAYER_NUM)*0x20 ) & 0x1)
                return (DISP_REG_GET(DISP_REG_OVL_L0_ADDR+(layerid-OVL1_MAX_LAYER_NUM)*0x20));
            else
                return 0;
        }
#else
...
{
    unsigned int offset = ovl_index(module) * DISP_OVL_INDEX_OFFSET;
			...
			void *handle)
{
    ...
    {
        layer_min = 0;
        layer_max = OVL1_MAX_LAYER_NUM;// 1
    }
    ...
    {
        layer_min = OVL1_MAX_LAYER_NUM;// 1
        layer_max = OVL1_MAX_LAYER_NUM+4;// 5
    }
    else
    {
        layer_min = 0;
	...
	if(has_sec_layer) {
		...
		if(ovl_is_sec[ovl_idx] == 1 && !primary_display_is_ovl1to2_handle(handle)) {
			...
			static cmdqRecHandle nonsec_switch_handle = NULL;
			int ret;
			...
			DISP_REG_SET(nonsec_switch_handle, DISP_REG_OVL_SRC_CON+(module-DISP_MODULE_OVL0)*DISP_OVL_INDEX_OFFSET, 0);
			for(i=0; i<4; i++)
	...
	for (i = layer_min; i < layer_max; i++) {
		if (pConfig->ovl_config[i].layer_en != 0) {
			...
			// if AEE=1, assert layer addr must equal to asert_pa or reg value is not equal to assert_pa(maybe 0 after suspend)
			if (module==DISP_MODULE_OVL0 && 
				  i==(layer_max-1) && 
				  isAEEEnabled && 
				  pConfig->ovl_config[i].addr == 0)
			{
				DDPMLOG("O - assert layer skip. O%d/L%d/LMax%d/mva0x%lx,reg_addr=0x%x \n", 
				    module, i, layer_max, pConfig->ovl_config[i].addr, DISP_REG_GET(DISP_REG_OVL_L3_ADDR));
				ovl_layer_switch(module, (i-layer_min)%4, pConfig->ovl_config[i].layer_en, handle);	
			...
			DDPMLOG("O%d/L%d/S%d/%s/0x%lx/(%d,%d)/P%d/(%d,%d,%d,%d).\n", 
					...
					pConfig->ovl_config[i].dst_w,
					pConfig->ovl_config[i].dst_h );	
			MMProfileLogEx(ddp_mmp_get_events()->ovl_enable, MMProfileFlagPulse, (module-DISP_MODULE_OVL0)<<4 + ((i-layer_min)%4), pConfig->ovl_config[i].addr);
		}
		else
		{
            // from enable to disable
            if(DISP_REG_GET(DISP_REG_OVL_SRC_CON+(module-DISP_MODULE_OVL0)*DISP_OVL_INDEX_OFFSET)&(0x1<<((i-layer_min)%4)))
            {
                //DDPMLOG("disable L%d.\n",i%4);
                MMProfileLogEx(ddp_mmp_get_events()->ovl_disable, MMProfileFlagPulse, (module-DISP_MODULE_OVL0)<<4 + ((i-layer_min)%4), 0);
            }
		}
...
static char* ovl_get_state_name(unsigned int status)
{
	switch (status&0x3ff) 
	{
    	...
    	case 0x8:  return "reg_update";		
    	case 0x10: return "eng_clr(internal reset)";		
    	case 0x20: return "eng_act(processing)";		
    	case 0x40: return "h_wait_w_rst";		
    	case 0x80: return "s_wait_w_rst";		
    	case 0x100:return "h_w_rst";		
    	case 0x200:return "s_w_rst";		
    	default:   return "ovl_fsm_unknown";		
	}
}
