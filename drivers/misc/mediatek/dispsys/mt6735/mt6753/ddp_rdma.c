...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle, enum RDMA_MODE mode, bool isidle)
{
  // constant
  ...
  if (idx == 0)
  {
      DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_PSEUDO_SIZE, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 512);
      DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0x10);//0xff
  ...
  {	  
	  DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_PSEUDO_SIZE, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 256);
      ...
      if (width > 800)// FHD
      {
          ...
          {
		     sodi_threshold = 168|(196<<10);//low:168	high:196 (decouple in FHD)
		  }
      }
      else // HD
      {
		  ...
		  if (mode == RDMA_MODE_DIRECT_LINK)
		  {
		     sodi_threshold = 75|(88<<10);//low:75	high:88 (direct link in HD)
          }
          else
          {
		     sodi_threshold = 75|(88<<10);//low:75	high:216 (decouple in HD)
		  }
      }
