...
static void ccci_dump_log_rec(struct ccci_modem *md, struct ccci_log *log)
{
    ...
    if(!log->droped) {
        printk("[CCCI%d-DUMP]%08X %08X %02d %08X  %5lu.%06lu\n",
            md->index+1,log->msg.data[0],log->msg.data[1],log->msg.channel,log->msg.reserved,(unsigned long)ts_nsec, rem_nsec/1000);
    } else {
        printk("[CCCI%d-DUMP]%08X %08X %02d %08X  %5lu.%06lu -\n",
            md->index+1,log->msg.data[0],log->msg.data[1],log->msg.channel,log->msg.reserved,(unsigned long)ts_nsec, rem_nsec/1000);
    }
...
void ccci_dump_log_add(struct ccci_modem *md, DIRECTION dir, int queue_index, struct ccci_header *msg,int is_droped)
{
    ...
    if(dir==OUT){
        memcpy(&md->tx_history[queue_index][md->tx_history_ptr[queue_index]].msg, msg, sizeof(struct ccci_header));
    ...
    if(dir==IN){
        memcpy(&md->rx_history[queue_index][md->rx_history_ptr[queue_index]].msg, msg, sizeof(struct ccci_header));
...
void ccci_dump_log_history(struct ccci_modem *md,int dump_multi_rec, int tx_queue_num, int rx_queue_num)
{
    ...
    if(dump_multi_rec)
    {
        for(i=0; i<((tx_queue_num <= MAX_TXQ_NUM) ? tx_queue_num : MAX_TXQ_NUM); i++) {
            printk("[CCCI%d-DUMP]dump txq%d packet history, ptr=%d\n",md->index+1, i,md->tx_history_ptr[i]);
        ...
        for(i=0; i<((rx_queue_num <= MAX_RXQ_NUM) ? rx_queue_num : MAX_RXQ_NUM); i++) {
            printk("[CCCI%d-DUMP]dump rxq%d packet history, ptr=%d\n",md->index+1, i,md->rx_history_ptr[i]);
            for(j=0;j<PACKET_HISTORY_DEPTH;j++)
                ccci_dump_log_rec(md, &md->rx_history[i][j]);
        }
    }else{
        printk("[CCCI%d-DUMP]dump txq%d packet history, ptr=%d\n",md->index+1, tx_queue_num,md->tx_history_ptr[tx_queue_num]);
        for(j=0;j<PACKET_HISTORY_DEPTH;j++)
            ccci_dump_log_rec(md, &md->tx_history[tx_queue_num][j]);
        printk("[CCCI%d-DUMP]dump rxq%d packet history, ptr=%d\n",md->index+1, rx_queue_num,md->rx_history_ptr[rx_queue_num]);
        for(j=0;j<PACKET_HISTORY_DEPTH;j++)
