 ...
 */
static inline int ccci_port_recv_skb(struct ccci_modem *md, struct sk_buff *skb)
{
	...
	list_for_each_entry(port, port_list, entry) {
		 ...
		 */
		matched =(ccci_h->channel == port->rx_ch); //(port->ops->req_match==NULL)?(ccci_h->channel == port->rx_ch):port->ops->req_match(port, req);
		if(matched) {
#ifdef FEATURE_SEQ_CHECK_EN
