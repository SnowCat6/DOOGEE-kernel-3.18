...
TRACE_EVENT(cldma_irq,
	...
	TP_printk("%s	0x%u", ((__entry->irq_type==0)?"TX":"RX"), __entry->queue_mask)
);

TRACE_EVENT(cldma_rx,

	TP_PROTO(int queue_no,int ccci_ch, unsigned long long req_alloc_time,unsigned long long port_recv_time, unsigned long long skb_alloc_time,unsigned long long rx_cost_time, unsigned long long rx_bytes),
	...
	TP_printk("q%d	ch%d	%llu	%llu	%llu	%llu	%llu", __entry->queue_no,__entry->ccci_ch, __entry->req_alloc_time, __entry->port_recv_time,__entry->skb_alloc_time, __entry->rx_cost_time, __entry->rx_bytes)
);

TRACE_EVENT(cldma_rx_done,

	TP_PROTO(int queue_no, unsigned long long rx_interal, unsigned long long total_time,unsigned int total_count, unsigned long long rx_bytes, unsigned long long sample_time,unsigned long long sample_rxbytes),

	TP_ARGS(queue_no, rx_interal, total_time, total_count,rx_bytes,sample_time, sample_rxbytes),
	...
	TP_printk("q%d	%llu	%llu	%u	%llu	%llu	%llu", __entry->queue_no,__entry->rx_interal, __entry->total_time,__entry->total_count,__entry->rx_bytes,__entry->sample_time,__entry->sample_rxbytes)
);

TRACE_EVENT(cldma_tx,

	TP_PROTO(int queue_no, int ccci_ch, unsigned int free_slot, unsigned long long tx_interal, unsigned long long tx_cost_time, unsigned long long tx_bytes, unsigned long long sample_time, unsigned long long sample_txbytes),

	TP_ARGS(queue_no, ccci_ch, free_slot, tx_interal, tx_cost_time,tx_bytes,sample_time,sample_txbytes),
	...
	TP_printk("q%u	ch%d	%u	%llu	%llu	%llu	%llu	%llu", __entry->queue_no,__entry->ccci_ch, __entry->free_slot, __entry->tx_interal, __entry->tx_cost_time, __entry->tx_bytes, __entry->sample_time, __entry->sample_txbytes)
);

TRACE_EVENT(cldma_tx_done,

	TP_PROTO(int queue_no,unsigned long long tx_interal, unsigned long long total_time,unsigned int total_count),
	...
	TP_printk("q%d	%llu	%llu	%u", __entry->queue_no, __entry->tx_interal,__entry->total_time,__entry->total_count)
);
...
TRACE_EVENT(cldma_error,
	...
	TP_printk("q%d	ch=%d	err=%u	line_no=%u",  __entry->queue_no, __entry->ccci_ch,__entry->error_no,__entry->line_no)
);
