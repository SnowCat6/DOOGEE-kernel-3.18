...
TRACE_EVENT(port_net_rx,
	...
	TP_PROTO(unsigned int md_id, unsigned int queue_no, unsigned int ccci_ch, unsigned int rx_cb_time, unsigned int rx_total_time),

	TP_ARGS(md_id, queue_no, ccci_ch, rx_cb_time, rx_total_time),

	TP_STRUCT__entry(
		__field(	unsigned int,	md_id			)
	...
	TP_printk("md%u	q%d	ch%u	%u	%u", __entry->md_id+1, __entry->queue_no,__entry->ccci_ch, __entry->rx_cb_time, __entry->rx_total_time)
);

TRACE_EVENT(port_net_tx,

	TP_PROTO(int md_id, unsigned int queue_no,unsigned int ccci_ch, unsigned int req_alloc_time, unsigned int send_req_time, unsigned int tx_total_time),

	TP_ARGS(md_id, queue_no, ccci_ch, req_alloc_time, send_req_time, tx_total_time),
	...
	TP_printk("md%d	q%u	ch%u	%u	%u	%u",__entry->md_id+1, __entry->queue_no,__entry->ccci_ch, __entry->req_alloc_time,__entry->send_req_time, __entry->tx_total_time)
);
...
TRACE_EVENT(port_net_error,
	...
	TP_printk("md%d	q%d	ch=%d	err=%u	line_no=%u",(__entry->md_id+1) , __entry->queue_no, __entry->ccci_ch,__entry->error_no,__entry->line_no)
);
#endif
