...
/*****************************************************************************
Driver interface   -------------------------hct_driver  erick
*****************************************************************************/
static ssize_t store_flashContrl_value(struct device_driver *ddri, char *buf, size_t count)
{
    int sensorDev = 1;
	int strobeId = 1;
	FLASHLIGHT_FUNCTION_STRUCT** ppF=0;
...
static DRIVER_ATTR(flashContrl,     /*S_IWUSR | S_IRUGO*/0644, NULL, store_flashContrl_value);


/*----------------------------------------------------------------------------*/
static struct driver_attribute *flashlight_attr_list[] = {
	&driver_attr_flashContrl,        /*trace log*/
};
/*----------------------------------------------------------------------------*/
...
//@@    register_battery_oc_notify(&bat_oc_protection_powerlimit, BATTERY_OC_PRIO_FLASHLIGHT);
	...
	flashlight_create_attr(&flashlight_platform_driver.driver);//erick modify for temp test
	logI("[flashlight_probe] done! ~");
    return ret;
}
