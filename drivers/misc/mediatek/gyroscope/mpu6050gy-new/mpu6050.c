...
#define GYRO_ERROR(fmt, args...)	pr_err(GYRO_FLAG fmt, ##args)
...
#endif
	if (res <= 0) {
		GYRO_ERROR(" enable xyz gyro in FIFO error,enable: 0x%x!\n", databuf[0]);
...
#endif
		GYRO_ERROR("read power ctl register err!\n");
		return MPU6050_ERR_I2C;
	}
...
#endif
		GYRO_ERROR("read data format register err!\n");
		return MPU6050_ERR_I2C;
	}
...
#endif
		GYRO_ERROR("read scale register err!\n");
		return MPU6050_ERR_I2C;
	}
...
static int MPU6050_SetSampleRate(struct i2c_client *client, int sample_rate)
{
	...
	if (hwmsen_read_byte(client, MPU6050_REG_CFG, databuf)) {
#endif
		GYRO_ERROR("read gyro sample rate register err!\n");
		return MPU6050_ERR_I2C;
	}
...
#endif
	if (res <= 0) {
		GYRO_ERROR("write sample rate register err!\n");
		return MPU6050_ERR_I2C;
	}
...
#endif
	if (res <= 0) {
		GYRO_ERROR("write Power CTRL register err!\n");
		return MPU6050_ERR_I2C;
	}
...
#endif
	if (res <= 0) {
		GYRO_ERROR("write AUX_VDD register err!\n");
		return MPU6050_ERR_I2C;
	}
...
#endif
	if (res <= 0) {
		GYRO_ERROR("write FIFO CTRL register err!\n");
		return MPU6050_ERR_I2C;
	}
...
static int MPU6050_ReadFifoData(struct i2c_client *client, s16 *data, int *datalen)
{
...
#endif
	if (err) {
		GYRO_ERROR("read data high number of bytes error: %d\n", err);
		return -1;
	}
	packet_cnt = tmp << 8;
...
#endif
	if (err) {
		GYRO_ERROR("read data low number of bytes error: %d\n", err);
		return -1;
	}
	...
	*datalen = packet_cnt;
	...
	if (packet_cnt && (abs(packet_thresh - packet_cnt) < 4)) {
...
#endif
				GYRO_ERROR("MPU6050 read data from FIFO error: %d\n", err);
				return -2;
	    } else
		...
		GYRO_ERROR("MPU6050 Incorrect packet count: %d\n", packet_cnt);
		return -3;
	}
...
#endif
		GYRO_ERROR("MPU6050 read temperature data  error\n");
		return -2;
	}
...
#endif
		GYRO_ERROR("MPU6050 read gyroscope data  error\n");
		return -2;
	}
...
static int MPU6050_PROCESS_SMT_DATA(struct i2c_client *client, short *data)
{
	...
	if (abs(xAvg) > bias_thresh) {
		GYRO_ERROR("X-Gyro bias exceeded threshold\n");
		retval |= 1 << 3;
	}
	if (abs(yAvg) >  bias_thresh) {
		GYRO_ERROR("Y-Gyro bias exceeded threshold\n");
		retval |= 1 << 4;
	}
	if (abs(zAvg) > bias_thresh) {
		GYRO_ERROR("Z-Gyro bias exceeded threshold\n");
		retval |= 1 << 5;
	}
	...
	if (xRMS > RMS_thresh * total_num) {
		GYRO_ERROR("X-Gyro RMS exceeded threshold, RMS_thresh: %ld\n", RMS_thresh * total_num);
		retval |= 1 << 6;
	}
	if (yRMS > RMS_thresh * total_num) {
		GYRO_ERROR("Y-Gyro RMS exceeded threshold, RMS_thresh: %ld\n", RMS_thresh * total_num);
		retval |= 1 << 7;
	}
	if (zRMS > RMS_thresh * total_num) {
		GYRO_ERROR("Z-Gyro RMS exceeded threshold, RMS_thresh: %ld\n", RMS_thresh * total_num);
		retval |= 1 << 8;
	}
...
/* S16 gyro[MPU6050_AXES_NUM*MPU6050_FIFOSIZE]; */
	...
	for (i = 0; i < MPU6050_AXES_NUM; i++) {
		...
		if (res) {
			GYRO_ERROR("MPU6050_FIFOConfig error:%d!\n", res);
			return -3;
		}
		...
		if (res) {
			if (res == (-3))
				...
				GYRO_ERROR("MPU6050_ReadData error:%d!\n", res);
				return -3;
	    }
		} else {
			buf[0] = datalen;
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL) {
		GYRO_ERROR("i2c_data obj is null!!\n");
		return 0;
	}
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "0x%x", &trace))
		...
		GYRO_ERROR("invalid content: '%s', length = %zu\n", buf, count);
		    ...
		    void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			GYRO_ERROR("Enable gyroscope parameter error!\n");
			err = -EINVAL;
		} else {
			value = *(int *)buff_in;
			if (((value == 0) && (sensor_power == false))
			    || ((value == 1) && (sensor_power == true))) {
				GYRO_DBG("gyroscope device have updated!\n");
			} else {
	...
	if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			GYRO_ERROR("get gyroscope data parameter error!\n");
			err = -EINVAL;
		} else {
...
static long mpu6050_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_SMT_DATA:
		...
		if (copy_cnt) {
			err = -EFAULT;
			GYRO_ERROR("copy gyro data to user failed!\n");
		}
...
static int mpu6050_resume(struct i2c_client *client)
{
	...
	if (err) {
		GYRO_ERROR("initialize client fail!!\n");
		return err;
	}
...
static void mpu6050_early_suspend(struct early_suspend *h)
{
	...
	if (err) {
		GYRO_ERROR("write power control fail!!\n");
		return;
	}
...
static void mpu6050_late_resume(struct early_suspend *h)
{
	...
	if (err) {
		GYRO_ERROR("initialize client fail! err code %d!\n", err);
		return;
	}
	atomic_set(&obj->suspend, 0);
...
static int mpu6050_enable_nodata(int en)
{
	...
	if (res != MPU6050_SUCCESS) {
		GYRO_ERROR("MPU6050_SetPowerMode fail!\n");
		return -1;
	}
...
static int mpu6050_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err) {
		GYRO_ERROR("mpu6050_device misc register failed!\n");
	...
	if (err) {
		GYRO_ERROR("mpu6050 create attribute err = %d\n", err);
	...
	mutex_init(&obj->raw_data_mutex);
	{
		...
		if (IS_ERR(inv_daemon_class)) {
			GYRO_ERROR("cannot create inv daemon class, %s\n", INV_DAEMON_CLASS_NAME);
		...
		if (IS_ERR(inv_daemon_device)) {
			GYRO_ERROR("cannot create inv daemon device, %s\n", INV_DAEMON_DEVICE_NAME);
		...
		if (!inv_daemon_device) {
			GYRO_ERROR("cannot allocate inv daemon device, %s\n", INV_DAEMON_DEVICE_NAME);
		...
		if (result) {
			GYRO_ERROR("cannot register inv daemon device, %s\n", INV_DAEMON_DEVICE_NAME);
			goto exit_device_register_failed;
		}
		...
		if (result) {
			...
			GYRO_ERROR("cannot create inv daemon dev attr.\n");
...
static int mpu6050_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		GYRO_ERROR("mpu6050_delete_attr fail: %d\n", err);


	err = misc_deregister(&mpu6050_device);
	if (err)
		GYRO_ERROR("misc_deregister fail: %d\n", err);


	err = hwmsen_detach(ID_GYROSCOPE);
	if (err)
		GYRO_ERROR("hwmsen_detach fail: %d\n", err);
