...
u8 fgParserEDID(u8 *prbData)
{
	...
	for (bIdx = 0; bIdx < 2; bIdx++) {
		ui2HActive =
		    (u16) (*(prbData + EDID_ADDR_TIMING_DSPR_1 + 18 * bIdx + OFST_H_ACT_BLA_HI) &
			   0xf0) << 4;
...
{
	u32 ui4CEA_NTSC = 0, ui4CEA_PAL = 0, ui4OrgCEA_NTSC = 0, ui4OrgCEA_PAL =
	    0, ui4NativeCEA_NTSC = 0, ui4NativeCEA_PAL = 0;
	u8 bTemp, bIdx;
	u8 bLengthSum;
	u8 bType, bNo, bAudCode, bPcmChNum;
	u8 bTemp13, bTemp8, bLatency_offset = 0;
	u8 b3D_Multi_present = 0, b3D_Structure_7_0 = 1, b3D_MASK_15_8 = 1, b3D_MASK_7_0 =
...
u8 fgParserExtEDID(u8 *prData)
{
	...
	for (bIdx = 0; bIdx < 6; bIdx++) {
		...
		ui2HBlanking =
		    (u16) (*(prData + bOfst + 18 * bIdx + OFST_H_ACT_BLA_HI) & 0x0f) << 8;
...
void mt8193_AppGetEdidInfo(HDMI_EDID_INFO_T *pv_get_info)
{
	...
	pv_get_info->ui4_ntsc_resolution =
	    (_HdmiSinkAvCap.ui4_sink_cea_ntsc_resolution | _HdmiSinkAvCap.ui4_sink_dtd_ntsc_resolution);
	pv_get_info->ui4_pal_resolution =
	    (_HdmiSinkAvCap.ui4_sink_cea_pal_resolution | _HdmiSinkAvCap.ui4_sink_dtd_pal_resolution);
