...
#define RG_TYPICAL   			0x009e	//modified when write LSC !!!
#define BG_TYPICAL				0x00a2	//modified when write LSC!!!

#define MODULE_ID_gc8024_gcore	0x0001    //gc8024: sensor P/N;  gcore: module vendor
...
void GC8024_MIPI_ENABLE_OTP(kal_bool otp_state)
{
...
void GC8024_MIPI_SELECT_PAGE_OTP(OTP_PAGE otp_select_page)
{
	kal_uint8 otp_page;
...
kal_uint8 GC8024_MIPI_READ_OTP(kal_uint8 addr)
{
	kal_uint8 value;
...
void GC8024_MIPI_READ_OTP_INFO(void)
{
	kal_uint8 check_info_flag0,check_info_flag1;
	kal_uint8 index,i,j,cnt=0;
	kal_uint8 info_start_add,wb_start_add;
	kal_uint8 total_number=0; 
	kal_uint8 check_flag,type;
	kal_uint8 dd0=0,dd1=0,dd2=0;
	kal_uint16 x,y;

	GC8024_MIPI_ENABLE_OTP(KAL_TRUE);

	GC8024_MIPI_SELECT_PAGE_OTP(OTP_SELECT_PAGE0);	
	check_info_flag0 = GC8024_MIPI_READ_OTP(0x00);
	
	LOG_INF("GC8024_MIPI_READ_OTP_INFO:check_info_flag0 = 0x%x\n",check_info_flag0);	

//lsc
	switch((check_info_flag0>>6)&0x03)
	{
	case 0x00:
		LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP Shading is Empty !!\n");
		otp_info.lsc_flag = otp_info.lsc_flag|0x00;
		break;
	case 0x01:
		LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP Shading is Valid !!\n");
		for(i=0; i<6; i++)
		{
			for(j=0; j<6;j++)
			{
				otp_info.lsc_r[i][j] = GC8024_MIPI_READ_OTP(LSC_BASE_ADDR + 6*i + j);
				LOG_INF("GC8024_MIPI_READ_OTP_INFO: lsc_r_date[%d][%d]= 0x%x \n",i,j,otp_info.lsc_r[i][j]);
			}
		}
		for(i=0; i<6; i++)
		{			
			for(j=0; j<6;j++)
			{
				otp_info.lsc_g[i][j] = GC8024_MIPI_READ_OTP(LSC_BASE_ADDR + 36 + 6*i + j);
				LOG_INF("GC8024_MIPI_READ_OTP_INFO: lsc_g_date[%d][%d]= 0x%x \n",i,j,otp_info.lsc_g[i][j]);
			}
		}
		for(i=0; i<6; i++)
		{
			for(j=0; j<6;j++)
			{
				otp_info.lsc_b[i][j] = GC8024_MIPI_READ_OTP(LSC_BASE_ADDR + 72 + 6*i + j);
				LOG_INF("GC8024_MIPI_READ_OTP_INFO: lsc_b_date[%d][%d]= 0x%x \n",i,j,otp_info.lsc_b[i][j]);
			}
		}
		otp_info.lsc_flag = otp_info.lsc_flag|0x01;
		break;
	case 0x02:
		LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP Shading is Invalid !!\n");
...
//wb
	for(index=0;index<2;index++)
	{
		...
		{
		case 0x00:
			LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP wb group%d is Empty !!\n", index + 1);
			otp_info.wb_flag = otp_info.wb_flag|0x00;
			break;
		case 0x01:
			LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP wb group%d is Valid !!\n", index + 1);
			wb_start_add = WB_START_ADD + index * WB_WIDTH;
			otp_info.rg_gain = GC8024_MIPI_READ_OTP(wb_start_add);
			otp_info.bg_gain = GC8024_MIPI_READ_OTP(wb_start_add + 1); 
			otp_info.wb_flag = otp_info.wb_flag|0x01;
			break;
		case 0x02:
			LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP wb group%d is Invalid !!\n", index + 1);			
	...
	GC8024_MIPI_SELECT_PAGE_OTP(OTP_SELECT_PAGE1);	
	check_info_flag1= GC8024_MIPI_READ_OTP(0x00);
	
	LOG_INF("GC8024_MIPI_READ_OTP_INFO:check_info_flag1 = 0x%x\n",check_info_flag1);

//dd
	switch((check_info_flag1>>2)&0x03)
	{
	case 0x00:
		LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP DD is Empty !!\n");
		otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP DD is Valid !!\n");
		
		total_number = GC8024_MIPI_READ_OTP(0x0f) + GC8024_MIPI_READ_OTP(0x10);
		
		LOG_INF("GC8024_MIPI_READ_OTP_INFO:total_number = %d\n",total_number);
		for(i=0; i<total_number; i++)
		{
			check_flag = GC8024_MIPI_READ_OTP(DD_BASE_ADDR + 4*i + 3);
				
			if(check_flag&0x10)
			{//Read OTP
				type = check_flag&0x0f;
		
				dd0 = GC8024_MIPI_READ_OTP(DD_BASE_ADDR + 4*i);
				dd1 = GC8024_MIPI_READ_OTP(DD_BASE_ADDR + 4*i + 1);	
				dd2 = GC8024_MIPI_READ_OTP(DD_BASE_ADDR + 4*i + 2);
				x = ((dd1&0x0f)<<8) + dd0;
				y = (dd2<<4) + ((dd1&0xf0)>>4);
				LOG_INF("GC8024_MIPI_READ_OTP_INFO : type = %d , x = %d , y = %d \n",type,x,y);
				LOG_INF("GC8024_MIPI_READ_OTP_INFO : dd0 = %d , dd1 = %d , dd2 = %d \n",dd0,dd1,dd2);
				if(type == 3)
				{
					for(j=0; j<4; j++)
					{
			...
			{
				LOG_INF("GC8024_MIPI_READ_OTP_INFO:check_id[%d] = %x,checkid error!!\n",i,check_flag);
			}
		}
		otp_info.dd_cnt = cnt;
		...
		break;
	case 0x02:
		LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP DD is Invalid !!\n");
...
//info
	for(index=0;index<2;index++)
	{
		...
		{
		case 0x00:
			LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP info group%d is Empty !!\n", index + 1);
			break;
		case 0x01:
			LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP info group%d is Valid !!\n", index + 1);
			info_start_add = INFO_START_ADD + index * INFO_WIDTH;
			otp_info.module_id = GC8024_MIPI_READ_OTP(info_start_add);
			otp_info.lens_id = GC8024_MIPI_READ_OTP(info_start_add + 1); 
			otp_info.vcm_driver_id = GC8024_MIPI_READ_OTP(info_start_add + 2)&0x0f;
			otp_info.vcm_start_position = GC8024_MIPI_READ_OTP(info_start_add + 2)&0xf0;
			otp_info.vcm_id = GC8024_MIPI_READ_OTP(info_start_add + 3);
			otp_info.year = GC8024_MIPI_READ_OTP(info_start_add + 4);
			otp_info.month = GC8024_MIPI_READ_OTP(info_start_add + 5);
			otp_info.day = GC8024_MIPI_READ_OTP(info_start_add + 6);
			break;
		case 0x02:
			LOG_INF("GC8024_MIPI_READ_OTP_INFO: OTP info group%d is Invalid !!\n", index + 1);
			break;
		default :
			break;
		}		
	...
	LOG_INF("vcm_start_position=0x%x",otp_info.vcm_start_position);
	LOG_INF("data=%d-%d-%d",otp_info.year,otp_info.month,otp_info.day);
	LOG_INF("wb_rg=0x%x",otp_info.rg_gain);
	LOG_INF("wb_bg=0x%x",otp_info.bg_gain);

	GC8024_MIPI_ENABLE_OTP(KAL_FALSE);

}


void GC8024_MIPI_UPDATE_DD(void)
{
	...
	if(0x01 ==otp_info.dd_flag)
	{
		...
		for(i=0; i<otp_info.dd_cnt; i++)
		{
			...
			LOG_INF("GC8024_MIPI_UPDATE_DD travis, val0 = 0x%x , val1 = 0x%x , val2 = 0x%x \n",temp_val0,temp_val1,temp_val2);
			LOG_INF("GC8024_MIPI_UPDATE_DD travis, x = %d , y = %d \n",((temp_val1&0x0f)<<8) + temp_val0,(temp_val2<<4) + ((temp_val1&0xf0)>>4));	
		}
...
void GC8024_MIPI_UPDATE_AWB(void)
{
	...
	if(0x01==(otp_info.wb_flag&0x01))
	{
		...
		LOG_INF("GC8024_MIPI_UPDATE_AWB travis, r_gain_current = 0x%x , b_gain_current = 0x%x , base_gain = 0x%x \n",r_gain_current,b_gain_current,base_gain);

		r_gain = 0x80 * r_gain_current / base_gain;
		g_gain = 0x80 * g_gain_current / base_gain;
		b_gain = 0x80 * b_gain_current / base_gain;
		LOG_INF("GC8024_MIPI_UPDATE_AWB travis, r_gain = 0x%x , g_gain = 0x%x , b_gain = 0x%x \n",r_gain,g_gain,b_gain);
...
void GC8024_MIPI_UPDATE_LSC(void)
{
	...
	if(0x01 == otp_info.lsc_flag)
	{
		...
		for(i=0; i<6; i++)
		{
			for(j=6; j>0; j--)
			{
				...
				write_cmos_sensor(0xcc,otp_info.lsc_r[i][j-1]);
			}
		}
		cnter = 0;
		...
		for(i=0; i<6; i++)
		{
			...
			for(i=0; i<6; i++)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_r[i][j]);
				}
			}
			cnter = 0;
			write_cmos_sensor(0xcf,0x04);//G
			for(i=0; i<6; i++)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_g[i][j]);
				}
			}
			cnter = 0;
			write_cmos_sensor(0xcf,0x02);//B
			for(i=0; i<6; i++)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_b[i][j]);
				}
			}
			...
			for(i=6; i>0; i--)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_r[i-1][j-1]);
					LOG_INF("GC8024_MIPI_UPDATE_LSC travis, otp_info.lsc_r[%d][%d] = 0x%x \n",i-1,j-1,otp_info.lsc_r[i-1][j-1]);
				}
			}
			cnter = 0;
			write_cmos_sensor(0xcf,0x04);//G
			for(i=6; i>0; i--)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_g[i-1][j-1]);
					LOG_INF("GC8024_MIPI_UPDATE_LSC travis, otp_info.lsc_g[%d][%d] = 0x%x \n",i-1,j-1,otp_info.lsc_g[i-1][j-1]);
				}
			}
			cnter = 0;
			write_cmos_sensor(0xcf,0x02);//B
			for(i=6; i>0; i--)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_b[i-1][j-1]);
					LOG_INF("GC8024_MIPI_UPDATE_LSC travis, otp_info.lsc_b[%d][%d] = 0x%x \n",i-1,j-1,otp_info.lsc_b[i-1][j-1]);
				}
			}		
#endif
			...
			for(i=6; i>0; i--)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_r[i-1][j]);
				}
			}
			cnter = 0;
			write_cmos_sensor(0xcf,0x04);//G
			for(i=6; i>0; i--)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_g[i-1][j]);
				}
			}
			cnter = 0;
			write_cmos_sensor(0xcf,0x02);//B
			for(i=6; i>0; i--)
			{
				...
				{
					write_cmos_sensor(0xca,cnter++);
					write_cmos_sensor(0xcc,otp_info.lsc_b[i-1][j]);
				}
			}		
#endif
...
void GC8024_MIPI_UPDATE_OTP(void)
{
	GC8024_MIPI_UPDATE_DD();
	GC8024_MIPI_UPDATE_AWB();
	GC8024_MIPI_UPDATE_LSC();
}
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x28,Val28[PreorCap][gainlevel]); //[7]binning
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("gc8024 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("gc8024 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
#ifdef GC8024_MIPI_USE_OTP
	GC8024_MIPI_READ_OTP_INFO();
#endif
    /* initail sequence write in  */
    sensor_init();
#ifdef GC8024_MIPI_USE_OTP
	GC8024_MIPI_UPDATE_OTP();
#endif
