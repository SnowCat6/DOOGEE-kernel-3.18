...
#include "ov5670submipi_Sensor.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "ov5670sub_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670SUBMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 65,//65  unit , ns
...
{{ 2592, 1944,    0,	0, 2592, 1944, 2592, 1944, 0000, 0000, 2592,  1944,	  1,	0, 2560, 1920}, // Preview 
...
{
	LOG_INF("OV5670SUB_Sensor_Init_2lane E\n");
	...
	write_cmos_sensor(0x5045, 0x05); 
	write_cmos_sensor(0x5048, 0x10); 
	write_cmos_sensor(0x3610, 0xa8); 
...
//  write_cmos_sensor(0x0100, 0x00); 
	...
	write_cmos_sensor(0x380c, 0x06);//6 
	write_cmos_sensor(0x380d, 0x90); 
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C) + pinid);
