...
#include "ov5671mipi_Sensor.h"

#define PFX "ov5671_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5671MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{
	LOG_INF(" OV5671PreviewSetting_2lane enter\n");
...
{
	LOG_INF("OV5671CaptureSetting_2lane enter! currefps:%d\n",currefps);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_INF("Read sensor id fail, sensor id: 0x%x, write_id:0x%x\n", sensor_id, imgsensor.i2c_write_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
