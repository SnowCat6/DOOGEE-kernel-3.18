...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 65,//85,
	...
	.isp_driving_current = ISP_DRIVING_2MA,//ISP_DRIVING_8MA, //mclk driving current
	...
	.i2c_addr_table = {0x42, 0x20, 0x6c, 0xff},// {0x42, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
static void set_dummy(void)
{
	...
	write_cmos_sensor(0x380e, frame_length >> 8);
	write_cmos_sensor(0x380f, frame_length & 0xFF);	  
...
int read_otp(struct otp_struct *otp_ptr)
{
   ...
   // OTP VCM Calibration
   otp_flag = read_cmos_sensor(0x7023);
   addr = 0;
   if((otp_flag & 0xc0) == 0x40) 
   {
 ...
 // OTP Lenc Calibration
   otp_flag = read_cmos_sensor(0x702c);
   addr = 0;
   int checksum2=0;
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x366e, 0x08);
	write_cmos_sensor(0x3714, 0x27); // neil add 20151030	
	write_cmos_sensor(0x37c2, 0x14);
	write_cmos_sensor(0x3808, 0x06);
	write_cmos_sensor(0x3809, 0x60);
	write_cmos_sensor(0x380a, 0x04);
  ...
  write_cmos_sensor(0x380f, 0xb2);//0xde);
	write_cmos_sensor(0x3810, 0x00);
	write_cmos_sensor(0x3811, 0x02);  //FOV, please modify:Preview: R3811  08-->02
	write_cmos_sensor(0x3813, 0x02); 	//neil add 20151030
	write_cmos_sensor(0x3814, 0x03);
	write_cmos_sensor(0x3820, 0x90);
	write_cmos_sensor(0x3821, 0x67);
	write_cmos_sensor(0x382a, 0x03);
	write_cmos_sensor(0x4009, 0x05); //neil modify 20151030
	write_cmos_sensor(0x5795, 0x00);
	write_cmos_sensor(0x5796, 0x10);
	write_cmos_sensor(0x5797, 0x10);
	write_cmos_sensor(0x5798, 0x73);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150) {
	  ...
	  write_cmos_sensor(0x3714, 0x23); // neil add 20151030			
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//#ifdef OV8856R1AOTP
					//printk("lili>ov8856>read otp start\n");
				//	read_otp(&otp_ptr);
					//printk("lili>ov8856> read otp end\n");
				//#endif
				return ERROR_NONE;
				}
			LOG_INF("Read sensor id sucess, i2c_write_id: 0x%x,0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}	
...
static kal_uint32 open(void)
{
	...
	mdelay(10);
/*	#ifdef OV8856R1AOTP
		//printk("lili>ov8856>apply otp start\n");		
               read_otp(&otp_ptr);
               apply_otp(&otp_ptr);
		//printk("lili>ov8856>apply otp start\n");
	#endif
