...
#include "gc13003mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC13003_camera_sensor"
#define LOG_1 LOG_INF("GC13003,MIPI 4LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC13003DuringTestPattern = KAL_FALSE;
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        ...
        .linelength = 2000,                //record different mode's linelength
        .framelength = 792,//    //3168/2=        //record different mode's framelength
...
{
	{ 4208, 3120,	 0,    0, 4208, 3120, 2096,  1560, 0000, 0000, 2096,  1560, 	 0,    0, 2096,  1560}, // Preview 
	{ 4208, 3120,	 0,    0, 4208, 3120, 4208,  3120, 0000, 0000, 4208,  3120, 	 0,    0, 4208,  3120}, // capture 
	{ 4208, 3120,	 0,    0, 4208, 3120, 2096,  1560, 0000, 0000, 2096,  1560, 	 0,    0, 2096,  1560}, // video 
	{ 4208, 3120,	 0,    0, 4208, 3120, 2096,  1560, 0000, 0000, 2096,  1560, 	 0,    0, 2096,  1560}, // hight speed video 
	{ 4208, 3120,	 0,    0, 4208, 3120, 2096,  1560, 0000, 0000, 2096,  1560, 	 0,    0, 2096,  1560}  // slim video 
};
	...
	if(shutter_temp <= 150)
  	{
		...
		write_cmos_sensor(0x32, 0x8f);
  	}
  	else 
	{
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0x32, 0x88);
  	}
...
#define ANALOG_GAIN_2 90     // 1.403x
#define ANALOG_GAIN_3 127    // 1.982x 
#define ANALOG_GAIN_4 179    // 2.804x
#define ANALOG_GAIN_5 246    // 3.847x
#define ANALOG_GAIN_6 358    // 4.592x
#define ANALOG_GAIN_7 503    // 7.858x
#define ANALOG_GAIN_8 721    // 11.258x
#define ANALOG_GAIN_9 1032    // 16.121x
//#define ANALOG_GAIN_10 1405  // 21.95x
//#define ANALOG_GAIN_11 2024  // 31.63x
...
{	
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF("GC13003MIPI analogic gain 1x, GC13003MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC13003MIPI analogic gain 1.375x , GC13003MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC13003MIPI analogic gain 1.891x , GC13003MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC13003MIPI analogic gain 2.625x , GC13003MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC13003MIPI analogic gain 3.734x , GC13003MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC13003MIPI analogic gain 5.250x , GC13003MIPI add pregain = %d\n",temp);
	}
...
/*	else if((ANALOG_GAIN_9<= iReg)&&(iReg < ANALOG_GAIN_10))
	...
	else if(ANALOG_GAIN_11<= iReg)
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC13003MIPI analogic gain 7.516x , GC13003MIPI add pregain = %d\n",temp);
	}
*/	
	else
	{
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0xf8, 0x0b);
	write_cmos_sensor(0xf9, 0x0a);                  
	write_cmos_sensor(0xfa, 0x89);
	write_cmos_sensor(0xfc, 0xaa);                  
	write_cmos_sensor(0xfe, 0x03);                  
	...
	write_cmos_sensor(0x0d, 0x0c);                  
	write_cmos_sensor(0x0e, 0x40);                  
	...
	write_cmos_sensor(0x1d, 0x11);
	write_cmos_sensor(0x20, 0x88);                  
	write_cmos_sensor(0x21, 0x12);                  
	write_cmos_sensor(0x24, 0x80);                  
	write_cmos_sensor(0x25, 0xc2);                  
	write_cmos_sensor(0x26, 0x08);                  
	write_cmos_sensor(0x28, 0x65);                  
	write_cmos_sensor(0x29, 0xdf);
	write_cmos_sensor(0x2a, 0xc2);                  
	write_cmos_sensor(0x32, 0x88);
	write_cmos_sensor(0xcd, 0xb8);
	write_cmos_sensor(0xce, 0xdc);
	write_cmos_sensor(0xd1, 0x93);                  
	...
	/*Dark Sun*/				                            
	write_cmos_sensor(0x38, 0x47);                                            
	/*Degrid*/                                      
	write_cmos_sensor(0xfe, 0x01);                  
	write_cmos_sensor(0xc0, 0x00);                  
	/*MIPI*/				                                
	write_cmos_sensor(0xfe, 0x03);                  
	write_cmos_sensor(0x02, 0x07); //01 eden 20160901                 
...
static kal_uint32 open(void)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC13003DuringTestPattern = KAL_FALSE; 
