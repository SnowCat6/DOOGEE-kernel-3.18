...
#include "gc5005submipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5005sub_camera_sensor"
#define LOG_1 LOG_INF("GC5005,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
#define GC5005SUB_MIPI_USE_OTP

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC5005SubDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5005SUBMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
void GC5005SUB_MIPI_enable_OTP(kal_bool otp_state)
{
	...
	{ 
		...
		LOG_INF("Enter GC5005SUB_MIPI_enable_OTP: Enable OTP!\n");		
	}
	else			
	{
		otp_en = otp_en & 0x7f;
		...
		LOG_INF("Enter GC5005SUB_MIPI_enable_OTP: Disable OTP!\n");
	}
}

void GC5005SUB_MIPI_select_page_OTP(OTP_PAGE otp_select_page)
{
	kal_uint8 otp_page;
...
kal_uint8 GC5005SUB_MIPI_READ_OTP(kal_uint8 addr)
{
	kal_uint8 value;
...
void GC5005SUB_MIPI_WRITE_SRAM_DD(void)
{	
	...
	total_number = GC5005SUB_MIPI_READ_OTP(0x01)+ GC5005SUB_MIPI_READ_OTP(0x02);
	
	LOG_INF("GC5005SUB_MIPI_READ_OTP_DD:total_number = %d\n",total_number);
	for(i=0; i<total_number; i++)
	{
		check_flag = GC5005SUB_MIPI_READ_OTP(DD_BASE_ADDR + i*4 + 3);
			
		if(check_flag&0x10)
		{//Read OTP
			type = check_flag&0x0f;

			dd0 = GC5005SUB_MIPI_READ_OTP(DD_BASE_ADDR + i*4);
			dd1 = GC5005SUB_MIPI_READ_OTP(DD_BASE_ADDR + i*4 + 1);		
			dd2 = GC5005SUB_MIPI_READ_OTP(DD_BASE_ADDR + i*4 + 2);
			x = ((dd1&0x0f)<<8) + dd0;
			y = (dd2<<4) + ((dd1&0xf0)>>4);
			LOG_INF("GC5005SUB_MIPI_READ_OTP_DD : type = %d , x = %d , y = %d \n",type,x,y);
		...
		{
			LOG_INF("GC5005SUB_MIPI_READ_OTP_DD : check_id[%d] = %x,checkid error!!\n",i,check_flag);
		}
	}
...
void GC5005SUB_MIPI_OTP(void)
{
	...
	GC5005SUB_MIPI_enable_OTP(KAL_TRUE);
	
	GC5005SUB_MIPI_select_page_OTP(OTP_SELECT_PAGE0);	

//dd
	check_dd_flag = GC5005SUB_MIPI_READ_OTP(0x00);
	switch(check_dd_flag&0x03)
	{
	case 0x00:
		LOG_INF("GC5005SUB_MIPI_OTP: OTP DD is Empty !!\n");
		break;
	case 0x01:
		GC5005SUB_MIPI_WRITE_SRAM_DD();
		break;
	case 0x02:
		LOG_INF("GC5005SUB_MIPI_OTP: OTP DD is Invalid !!\n");
		break;
	default :
		break;
	}
	
	GC5005SUB_MIPI_enable_OTP(KAL_FALSE);
...
{	
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF("GC5005SUBMIPI analogic gain 1x, GC5005SUBMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005SUBMIPI analogic gain 1.469x , GC5005SUBMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005SUBMIPI analogic gain 2.165x , GC5005SUBMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005SUBMIPI analogic gain 3.105x , GC5005SUBMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005SUBMIPI analogic gain 3.632x , GC5005SUBMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005SUBMIPI analogic gain 4.750x , GC5005SUBMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005SUBMIPI analogic gain 8.569x , GC5005SUBMIPI add pregain = %d\n",temp);
	}
	else
	{                                                                                         
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5005SUBMIPI analogic gain 11.69x , GC5005SUBMIPI add pregain = %d\n",temp);
	}                                                                                         
...
static kal_uint32 open(void)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC5005SubDuringTestPattern = KAL_FALSE; 
