...
static imgsensor_struct imgsensor = {
	.mirror = IMAGE_H_MIRROR,//IMAGE_HV_MIRROR,//IMAGE_NORMAL,//IMAGE_V_MIRROR,				//mirrorflip information
...
static int Hi843B_write_burst_mode(struct Hynix_Sensor_reg table[])
{
    	int err = 0;
...
#ifdef HI843B_USE_OTP
/*************************************************************************
********************* HI843B OTP part code begin *****************************
**************************************************************************/
#define HI843b_DEBUG_OTP 0
#define HI843b_AKERR_MODULE_ID 0x03
#define HI843b_OTP_BASE 0x0201
#define HI843b_OTP_BASE_LEN 17

#define HI843b_OTP_WB 0x0c5f
#define HI843b_OTP_WB_LEN 30

//#define HI843b_RG_Ratio_Typical   355 //o-film
//#define HI843b_BG_Ratio_Typical   347
...
struct otp_struct g_current_otp;

//int hi843b_otp_update(void);
//static imgsensor_struct imgsensor;
//static kal_uint16 read_cmos_sensor(kal_uint32 addr);
//static void write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
...
static void hi843b_otp_disable(void)
{
	HI843B_OTP_write_cmos_sensor(0x0118, 0x00); // sleep on
	mDELAY(100);
	HI843B_OTP_write_cmos_sensor(0x003e, 0x00); // display mode
	HI843B_OTP_write_cmos_sensor(0x0118, 0x01); // sleep off
}
...
static int hi843b_otp_read_baseinfo(void)
{
	kal_uint16  index =0;
	kal_uint16  base = 0;
	kal_uint16  val = 0;
	...
	kal_uint16  check = 0;
	kal_uint16  flag = HI843B_Sensor_OTP_read(HI843b_OTP_BASE);
	i = val = 0;
	LOG_INF("mojl hi843b otp baseinfo flag 0x%x \n", flag);
	...
	if (flag &0x01)
		...
		LOG_INF(" hi843b otp base info fail flag %x \n", flag);
		return 1;
	}

	base = (HI843b_OTP_BASE + 1) + (index * HI843b_OTP_BASE_LEN);

  #if HI843b_DEBUG_OTP
	for(i=0;i<HI843b_OTP_BASE_LEN;i++) {
		val = HI843B_Sensor_OTP_read(base+i);
		LOG_INF("Hynix otp i %d addr 0x%x  val 0x%x\n", i, base+i, val);
	...
	//add checksum	
	check = checksum%256;
	g_current_otp.module_checksum_info = HI843B_Sensor_OTP_read(base+17);	
	LOG_INF("Hynix hi843b otp module_checksum_info 0x%x \n",g_current_otp.module_checksum_info);
	if (check == g_current_otp.module_checksum_info){
	...
	g_current_otp.module_integrator_id = HI843B_Sensor_OTP_read(base);	
	LOG_INF("mojl hi843b otp baseinfo module id 0x%x \n",g_current_otp.module_integrator_id);

	return 0;
}

static int hi843b_otp_read_wbinfo(void)
{
	kal_uint16  index =0;
	kal_uint16  base = 0;
	kal_uint16  val = 0;
	...
	kal_uint16  wbcheck = 0;
	kal_uint16  flag = HI843B_Sensor_OTP_read(HI843b_OTP_WB);
	i = val = 0;

	LOG_INF("Hynix hi842b otp wbinfo flag 0x%x \n", flag);
	...
	if (flag &0x01)
		...
		LOG_INF(" hi843b otp wbinfo fail flag %x \n", flag);
		return 1;
	}

	base = (HI843b_OTP_WB + 1) + (index * HI843b_OTP_WB_LEN);

  #if HI843b_DEBUG_OTP
	for(i=0;i<HI843b_OTP_WB_LEN;i++) {
		val = HI843B_Sensor_OTP_read(base+i);
		mDELAY(5);	
		LOG_INF("mojl wb addr 0x%x 0x%x \n", base+i, val);
		checksumwb = checksumwb + val;
	}
  #endif
	//add checksum
	wbcheck = checksumwb%256;
	g_current_otp.module_checksum_wb = HI843B_Sensor_OTP_read(base+30);	
	LOG_INF("Hynix hi843b otp module_checksum_wb 0x%x \n",g_current_otp.module_checksum_wb);
	if (wbcheck == g_current_otp.module_checksum_wb){
		LOG_INF("HI843B_Sensor: Module wb checksum PASS\n ");
		printk("wangk_HI843B_Sensor: Module wb checksum PASS\n ");
	}else{
		LOG_INF("HI843B_Sensor: Module wb checksum Fail\n ");
		printk("wangk_HI843B_Sensor: Module wb checksum Fail\n ");
	}
  
	g_current_otp.rg_ratio = HI843B_Sensor_OTP_read(base) << 4 | HI843B_Sensor_OTP_read(base+1) >> 4;
	g_current_otp.bg_ratio = HI843B_Sensor_OTP_read(base+2) << 4 | HI843B_Sensor_OTP_read(base+3) >> 4;

	g_current_otp.golden_rg_ratio = HI843B_Sensor_OTP_read(base+6) << 4 | HI843B_Sensor_OTP_read(base+7) >> 4;
	g_current_otp.golden_bg_ratio = HI843B_Sensor_OTP_read(base+8) << 4 | HI843B_Sensor_OTP_read(base+9) >> 4;

	LOG_INF("Hynix wb rg 0x%x bg 0x%x \n",g_current_otp.rg_ratio, g_current_otp.bg_ratio );
	LOG_INF("Hynix wb golden rg 0x%x golden bg 0x%x \n",g_current_otp.golden_rg_ratio, g_current_otp.golden_bg_ratio);

	return 0;
}

static int hi843b_write_gain(void)
{ 
	#define OTP_MULTIPLE_FAC    (128L)   // 128 = 2^7
	kal_uint16  r_gain, b_gain;
	kal_uint16   g_gain = 0x200;

	if (g_current_otp.rg_ratio == 0x0 || g_current_otp.bg_ratio == 0x0) {
		LOG_INF("otp read fail current rg bg is 0\n");
		return 1;
	}
  #ifdef OTP_SUPPORT_FLOATING
	r_gain = (int)(((float)g_current_otp.golden_rg_ratio/g_current_otp.rg_ratio) * 0x200);
	b_gain = (int)(((float)g_current_otp.golden_bg_ratio/g_current_otp.bg_ratio) * 0x200);
  #else
	r_gain = (((OTP_MULTIPLE_FAC * g_current_otp.golden_rg_ratio) / g_current_otp.rg_ratio) * 0x200) / OTP_MULTIPLE_FAC;
	b_gain = (((OTP_MULTIPLE_FAC * g_current_otp.golden_bg_ratio) / g_current_otp.bg_ratio) * 0x200) / OTP_MULTIPLE_FAC;
  #endif
	if(r_gain < b_gain)
	{
		if(r_gain < 0x200)
    ...
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x0508));
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x0509));
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x050a));
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x050b));
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x050c));
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x050d));
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x050e));
    //LOG_INF("%s: %d \n",__func__, read_cmos_sensor(0x050f));	
	return 0;
}
...
{	
	if (g_current_otp.rg_ratio == 0x0 || g_current_otp.bg_ratio == 0x0) {
		HI843BOTPSetting();
		hi843b_otp_read_baseinfo();
		hi843b_otp_read_wbinfo();
		hi843b_otp_disable();
	}
...
/*************************************************************************
********************* HI843B OTP part code end *******************************
**************************************************************************/
#endif /*HI843B_USE_OTP*/
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	else //PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
    {
		...
		LOG_INF("current_fps %d fps is not support,use cap1's setting: %d fps!\n",imgsensor.current_fps,imgsensor_info.cap1.max_framerate/10);   
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
                    ...
                    //LOG_INF("current_fps fps is not support, use cap's setting: %d fps!\n",framerate,imgsensor_info.cap.max_framerate/10);
