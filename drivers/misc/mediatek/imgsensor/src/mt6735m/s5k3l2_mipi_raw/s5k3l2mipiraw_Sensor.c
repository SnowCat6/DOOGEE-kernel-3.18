...
kal_uint16 S5K3L2XX_read_EEPROM(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
kal_uint16 S5K3L2XX_MIPI_read_otp_wb(struct S5K3L2XX_MIPI_otp_struct *otp)
{
   	kal_uint16 current_R_ratio_H, current_R_ratio_L, current_B_ratio_H, current_B_ratio_L, FLG = 0x00;
   
   	FLG = S5K3L2XX_read_EEPROM(0x0000); 
  	...
  	if(FLG!=1)
   	...
   	{
//  		LOG_INF("[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb] golden_r_ratio=0x%x, golden_b_ratio=0x%x,", golden_r_ratio, golden_b_ratio);
  		current_R_ratio_H = S5K3L2XX_read_EEPROM(0x0008);
  		current_R_ratio_L = S5K3L2XX_read_EEPROM(0x0009);
  		current_B_ratio_H = S5K3L2XX_read_EEPROM(0x000a);
  		current_B_ratio_L = S5K3L2XX_read_EEPROM(0x000b); 

   		otp->r_ratio = (current_R_ratio_H<<8) + current_R_ratio_L;
   		otp->b_ratio = (current_B_ratio_H<<8) + current_B_ratio_L;
	
   		printk("liuzhen:[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", otp->r_ratio, otp->b_ratio); 
...
{
   kal_uint16 R_GAIN, B_GAIN, Gr_GAIN, Gb_GAIN, G_GAIN, r_ratio, b_ratio, golden_r_ratio,golden_b_ratio,Base_gain;
   
    golden_r_ratio = 0x342; //golde R/G =0x342
    golden_b_ratio = 0x2cc; //golde B/G =0x2cc

    r_ratio = otp->r_ratio;
    b_ratio = otp->b_ratio;

    R_GAIN = golden_r_ratio * GAIN_DEFAULT / r_ratio;
    B_GAIN = golden_b_ratio * GAIN_DEFAULT / b_ratio;
    G_GAIN = GAIN_DEFAULT;


    if (R_GAIN < GAIN_DEFAULT || B_GAIN < GAIN_DEFAULT)
    {
        if (R_GAIN < B_GAIN)
            Base_gain = R_GAIN;
        else
    ...
    R_GAIN = GAIN_DEFAULT * R_GAIN / (Base_gain);
    B_GAIN = GAIN_DEFAULT * B_GAIN / (Base_gain);
    G_GAIN = GAIN_DEFAULT * G_GAIN / (Base_gain);
   ...
   printk("liuzhen:[S5K3L2XX] [S5K3L2XX_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);    
}
...
void S5K3L2XX_MIPI_write_otp_wb(struct S5K3L2XX_MIPI_otp_struct *otp)
{
   ...
   write_cmos_sensor1(0x020e, G_GAIN);
   write_cmos_sensor1(0x0210, R_GAIN);
   write_cmos_sensor1(0x0212, B_GAIN);
   write_cmos_sensor1(0x0214, G_GAIN);
}
...
void S5K3L2XX_MIPI_update_wb_register_from_otp(void)
{
   ...
   if(S5K3L2XX_MIPI_read_otp_wb(&current_otp) == 1)
   	{
