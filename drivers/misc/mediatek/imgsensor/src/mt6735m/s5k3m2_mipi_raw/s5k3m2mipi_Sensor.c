...
#if defined(CONFIG_S5K3M2_USE_AWB_OTP)

#define GAIN_DEFAULT       	0x0100

#define GOLDEN_VALUE_R		0x62
#define GOLDEN_VALUE_G		((0x98+0x99)/2)
#define GOLDEN_VALUE_B		0x5b

#define S5K3M2_OTP_WRITE_ID         0xB0

kal_uint16 S5K3M2_read_eeprom(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	
	char puSendCmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
	iReadRegI2C(puSendCmd , 2, (u8*)&get_byte, 1, S5K3M2_OTP_WRITE_ID);
	return get_byte&0x00ff;
}

void S5K3M2_MIPI_update_wb_register_from_otp(void)
{
	unsigned int UnitAwb_R_value=0, UnitAwb_Gr_value=0, UnitAwb_Gb_value=0, UnitAwb_G_value=0, UnitAwb_B_value=0;
	unsigned int UnitGainR=0, UnitGainG=0, UnitGainB=0, UnitGainGr=0, UnitGainGb=0;
	unsigned int GoldenRatioR=0, GoldenRatioB=0;
	unsigned int UnitRatioR=0, UnitRatioB=0;
	
	//unit awb (R . Gr . Gb . B)
	UnitAwb_R_value = S5K3M2_read_eeprom(0x000C);
	UnitAwb_Gr_value = S5K3M2_read_eeprom(0x000D);
	UnitAwb_Gb_value = S5K3M2_read_eeprom(0x000E);
	UnitAwb_B_value = S5K3M2_read_eeprom(0x000F);
	LOG_INF("Unit Value R=%d, Gr=%d, Gb=%d, B=%d\n", UnitAwb_R_value, UnitAwb_Gr_value, UnitAwb_Gb_value, UnitAwb_B_value);

	//UnitAwb_G_value
	UnitAwb_G_value = (UnitAwb_Gr_value + UnitAwb_Gb_value)/2;
	LOG_INF("Unit Value=%d,%d,%d. Golden Value=%d,%d,%d\n", UnitAwb_R_value, UnitAwb_G_value,UnitAwb_B_value, GOLDEN_VALUE_R,GOLDEN_VALUE_G,GOLDEN_VALUE_B);
	//unit ratio
	UnitRatioR = 512 * UnitAwb_R_value / UnitAwb_G_value;
	UnitRatioB = 512 * UnitAwb_B_value / UnitAwb_G_value;
	//golden ratio
	GoldenRatioR = 512 * GOLDEN_VALUE_R / GOLDEN_VALUE_G;
	GoldenRatioB = 512 * GOLDEN_VALUE_B / GOLDEN_VALUE_G;
	LOG_INF("Ratio UR=%d, UB=%d, GR=%d, GB=%d\n", UnitRatioR, UnitRatioB, GoldenRatioR, GoldenRatioB);
	//gain
	if(UnitRatioB < GoldenRatioB)
    	{
		if(UnitRatioR < GoldenRatioR)
		{
		    UnitGainG = GAIN_DEFAULT;
		    UnitGainB = GAIN_DEFAULT * GoldenRatioB / UnitRatioB;
		    UnitGainR = GAIN_DEFAULT * GoldenRatioR / UnitRatioR;
		}
		else
		{
		    UnitGainR = GAIN_DEFAULT;
		    UnitGainG = GAIN_DEFAULT * UnitRatioR / GoldenRatioR;
		    UnitGainB = UnitGainG * GoldenRatioB / UnitRatioB;	        
		}
	}
    	else
    	{
		if(UnitRatioR < GoldenRatioR)
		{
		    UnitGainB = GAIN_DEFAULT;
		    UnitGainG = GAIN_DEFAULT * UnitRatioB / GoldenRatioB;
		    UnitGainR = UnitGainG * GoldenRatioR / UnitRatioR;
		}
		else
		{
		    UnitGainGb = GAIN_DEFAULT* UnitRatioB / GoldenRatioB;
		    UnitGainGr = GAIN_DEFAULT* UnitRatioR / GoldenRatioR;
		    
		    if(UnitGainGb > UnitGainGr)
		    {
		        UnitGainB = GAIN_DEFAULT;
		        UnitGainG = UnitGainGb;
		        UnitGainR = UnitGainG * GoldenRatioR / UnitRatioR;
		    }
		    else
		    {
		        UnitGainR = GAIN_DEFAULT;
		        UnitGainG = UnitGainGr;
		        UnitGainB = UnitGainG * GoldenRatioB / UnitRatioB;
		    }	        
		}		
    	}
	
	LOG_INF("writesensor:UnitGainR=%d, UnitGainG=%d(%d,%d), UnitGainB=%d\n", UnitGainR, UnitGainG,UnitGainGb,UnitGainGr, UnitGainB);

	write_cmos_sensor(0x020e, UnitGainG);
	write_cmos_sensor(0x0210, UnitGainR);
	write_cmos_sensor(0x0212, UnitGainB);
	write_cmos_sensor(0x0214, UnitGainG);


	UnitGainR = read_cmos_sensor(0x0210);
	UnitGainB = read_cmos_sensor(0x0212);
	UnitGainG = read_cmos_sensor(0x0214);
	LOG_INF("readback: UnitGainR=%d, UnitGainG=%d, UnitGainB=%d\n", UnitGainR, UnitGainG, UnitGainB);
}
#endif //CONFIG_S5K3M2_USE_AWB_OTP
...
//	otp_lsc_update();
    ...
    LOG_INF("Exit\n");
  //s 2014/12/01, bruce
  } else if (chip_id == 0xD201) {
    LOG_INF("-- sensor_init, chip id = 0xD201\n");
  write_cmos_sensor(0x6028, 0x4000);
  write_cmos_sensor(0x6214, 0x7971);
  write_cmos_sensor(0x6218, 0x0100);
  //SW page
...
// s 2014/12/01, bruce
	...
	LOG_INF("--preview_setting, chip_id = 0xD201\n");
...
// s bruce, 2014/12/01
	...
	LOG_INF( "-- normal_capture_setting, chip_id = 0xD201\n");
write_cmos_sensor_8(0x0100,0x00);
mdelay(200);
write_cmos_sensor(0x6028,0x2000);
write_cmos_sensor(0x602A,0x14F0);
...
// s bruce 2014/12/01
	...
	LOG_INF("--hs_video_setting. chip_id = 0xD201\n");
write_cmos_sensor_8(0x0100,0x00);
mdelay(200);
	write_cmos_sensor(0x6028,0x2000);
	write_cmos_sensor(0x602A,0x14F0);
...
//s bruce, 2014/12/01
	...
	LOG_INF("--slim_video_setting. chip_id = 0xD201");
	write_cmos_sensor_8(0x0100,0x00);
  mdelay(200);
	write_cmos_sensor(0x6028,0x2000);
	write_cmos_sensor(0x602A,0x14F0);
...
extern int read_s5k3m2_eeprom_mtk_fmt(void);
#endif
...
#if defined(CONFIG_S5K3M2_USE_AWB_OTP)
	S5K3M2_MIPI_update_wb_register_from_otp();
#endif
	return ERROR_NONE;
}	/*	open  */
