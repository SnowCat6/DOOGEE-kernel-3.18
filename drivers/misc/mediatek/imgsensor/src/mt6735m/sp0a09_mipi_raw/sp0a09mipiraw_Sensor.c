...
#include "sp0a09mipiraw_Sensor.h"
...
#ifdef CONFIG_BSP_TW_ENHANCE
static int cur_idx = -1;
static int cur_pos = -1;

/*
  * Parameters:
  * @midx: main sensor index found in HWINFO sensor list
  * @midx: sub sensor index found in HWINFO sensor list
  * @pos: current sensor's position
  */
static int set_hwinfo(int idx, int p)
{
    cur_pos = p;
    cur_idx = idx;
    CERR("idx %d, pos %d\n", idx, p);
    /* read external configurable parameters from HWINFO */
    get_sensor_info_from_hwinfo(&imgsensor_info, idx, p);
    return 0;
}
#endif
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x31,0x06); //bit1 bit2 mirror & flip
  //write_cmos_sensor(0x01,0x01);     
  /*
  write_cmos_sensor(0xfd,0x00);
  ...
  write_cmos_sensor(0x13,0x10);
  write_cmos_sensor(0x6c,0x11);
  write_cmos_sensor(0x6d,0x11);
  write_cmos_sensor(0x6f,0x11);
  write_cmos_sensor(0x6e,0x12);
  write_cmos_sensor(0x69,0x23);
  write_cmos_sensor(0x71,0x21);
  write_cmos_sensor(0x14,0x01);
  write_cmos_sensor(0x15,0x20);
  write_cmos_sensor(0x16,0x2c);
  write_cmos_sensor(0x17,0x2d);
  write_cmos_sensor(0x70,0x2e);
  write_cmos_sensor(0x6a,0x18);
  write_cmos_sensor(0x72,0x1a);  
  write_cmos_sensor(0x1a,0x19);
  write_cmos_sensor(0x1b,0x03);
  write_cmos_sensor(0x1c,0x07);
  write_cmos_sensor(0x1e,0x13);
  write_cmos_sensor(0x1f,0x01);
  write_cmos_sensor(0x21,0x4d);
  write_cmos_sensor(0x22,0x48);
  write_cmos_sensor(0x27,0xbb);
  write_cmos_sensor(0x28,0x7f);
  write_cmos_sensor(0x24,0x20);
  write_cmos_sensor(0x03,0x00);
  write_cmos_sensor(0x04,0xa8);
...
static kal_uint32 open(void)
{
    ...
    if ((cur_idx >0 || cur_idx == 0) && cur_pos != 0) {
        CDBG("override original [%s] settings\n", "sensor_init");
        ret = write_settings_from_hwinfo(cur_idx, cur_pos, "sensor_init");
        if (ret < 0) {
            CERR("write settings failed, using the original set\n");
            sensor_init();
        }
    } else 
        sensor_init();
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if ((cur_idx >0 || cur_idx == 0) && cur_pos != 0) {
        CDBG("override original [%s] settings\n", "preview_setting");
        ret = write_settings_from_hwinfo(cur_idx, cur_pos, "preview_setting");
        if (ret < 0) {
            CERR("write settings failed, using the original set\n");
            preview_setting();
        }
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if ((cur_idx >0 || cur_idx == 0) && cur_pos != 0) {
        CDBG("override original [%s] settings\n", "capture_setting");
        ret = write_settings_from_hwinfo(cur_idx, cur_pos, "capture_setting");
        if (ret < 0) {
            CERR("write settings failed, using the original set\n");
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if ((cur_idx >0 || cur_idx == 0) && cur_pos != 0) {
        ...
        ret = write_settings_from_hwinfo(cur_idx, cur_pos, "normal_video_setting");
        if (ret < 0) {
            CERR("write settings failed, using the original set\n");
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if ((cur_idx >0 || cur_idx == 0) && cur_pos != 0) {
        CDBG("override original [%s] settings\n", "hs_video_setting");
        ret = write_settings_from_hwinfo(cur_idx, cur_pos, "hs_video_setting");
        if (ret < 0) {
            CERR("write settings failed, using the original set\n");
            hs_video_setting();
        }
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if ((cur_idx >0 || cur_idx == 0) && cur_pos != 0) {
        CDBG("override original [%s] settings\n", "slim_video_setting");
        ret = write_settings_from_hwinfo(cur_idx, cur_pos, "slim_video_setting");
        if (ret < 0) {
            CERR("write settings failed, using the original set\n");
            slim_video_setting();
        }
