...
#include "s5k3p3sqmipiraw_Sensor.h"

#define PFX "S5K3P3SQ_camera_sensor"
...
extern bool read_3P3SQ_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_3P3SQ_eeprom_vendorinfo( kal_uint16 addr, BYTE* data, kal_uint32 size);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			read_3P3SQ_eeprom_vendorinfo(0x01,vendorid,9);
            (*sensor_id) += (UINT32)vendorid[8];
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_3P3SQ_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
...
//kin0603
UINT32 S5K3P3SQ_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
