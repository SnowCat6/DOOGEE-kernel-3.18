					...
					struct clock_event_device *evt)
{
	...
	if (ctrl & ARCH_TIMER_CTRL_IT_STAT) {
		...
		/*add debug to check if error trigger */
		if(!save_data[cpu].int_status && ((save_data[cpu].timestamp + save_data[cpu].evt) > sched_clock()))
		{
			cntp_tval = localtimer_get_counter();
			pr_alert("%s error: cpu=%d, Now_CTRL=0x%lx, Now_CNTP_TVAL=%ld, Set_CTRL=0x%lx, time_expire=%ld, ISR_trigger=%lld, set_time=%lld\n",
	...
	{
			/*add debug to check if error trigger */
			pr_alert("%s: INT error trigger, cpu=%d, CNTP_CTL=0x%lx\n", __func__, cpu, ctrl);		
			BUG();
	}	
#endif	
	return IRQ_NONE;
...
static void save_localtimer_info(unsigned long evt, int access)
{
    ...
    ctrl = arch_timer_reg_read(access, ARCH_TIMER_REG_CTRL);
		...
		if(ctrl & ARCH_TIMER_CTRL_IT_STAT)
		{
			...
			{
					pr_alert("%s: cpu=%d, Now_CTRL=0x%x, Now_CNTP_TVAL=%d, save_data[cpu].ctrl=0x%lx, save_data[cpu].int_status=0x%x, save_data[cpu].evt=%ld, set_time=%lld, now_time=%lld \n", 
...
static void __init arch_timer_init(struct device_node *np)
{
...
#if 1		//force use physical if kernel not boot at EL2/PL2.
	...
	//sched_clock_register((void *)arch_timer_read_counter, 32, (unsigned long)arch_timer_rate);
}
