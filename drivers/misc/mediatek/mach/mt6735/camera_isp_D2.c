...
#include <mach/camera_isp_FrmB_D2.h>
...
static bool CAM_HAL_VER_IS3 = false;
...
static MINT32 ISP_DumpReg(MVOID)
{
    ...
    //Mipi source
    LOG_DBG("[0x%08X %08X],[0x%08X %08X]", (unsigned int)(0x10215830), (unsigned int)ISP_RD32(ISP_MIPI_ANA_ADDR+0x830),(unsigned int)(0x10215830), (unsigned int)ISP_RD32(ISP_MIPI_ANA_ADDR+0x830));
    LOG_DBG("[0x%08X %08X],[0x%08X %08X]", (unsigned int)(0x10215c30), (unsigned int)ISP_RD32(ISP_MIPI_ANA_ADDR+0xc30),(unsigned int)(0x10215c30), (unsigned int)ISP_RD32(ISP_MIPI_ANA_ADDR+0xc30));
    ...
    //CDP
    LOG_DBG("0x%08X %08X ",  (unsigned int)(ISP_TPIPE_ADDR + 0xCC0), ISP_RD32((void *)(ISP_ADDR + 0xCC0)));
    LOG_DBG("0x%08X %08X ",  (unsigned int)(ISP_TPIPE_ADDR + 0xD40), ISP_RD32((void *)(ISP_ADDR + 0xD40)));

    LOG_DBG("0x%08X %08X ",  (unsigned int)(ISP_ADDR_CAMINF), ISP_RD32((void *)ISP_ADDR_CAMINF));
    LOG_DBG("0x%08X %08X ",  (unsigned int)(ISP_TPIPE_ADDR + 0x150), ISP_RD32((void *)(ISP_ADDR + 0x150)));
...
static MBOOL ISP_BufWrite_Add(MUINT32 Size, ISP_REG_STRUCT *pData)
{
    ...
    for(i = 0; i < ISP_BUF_WRITE_AMOUNT; i++)
    {
        if(g_IspInfo.BufInfo.Write[i].Status == ISP_BUF_STATUS_HOLD)
        {
            ...
            if(copy_from_user((void *)(g_IspInfo.BufInfo.Write[i].pData+g_IspInfo.BufInfo.Write[i].Size), (void *)pData, Size) != 0)
            {
    ...
    for(i = 0; i < ISP_BUF_WRITE_AMOUNT; i++)
    {
        if(g_IspInfo.BufInfo.Write[i].Status == ISP_BUF_STATUS_EMPTY)
        {
            ...
            if(copy_from_user((void *)(g_IspInfo.BufInfo.Write[i].pData), (void __user *)pData, Size) != 0)
            {
                LOG_ERR("copy_from_user failed");
                return MFALSE;
            }
        ...
        if(ISP_REF_CNT_ID_MAX > ref_cnt_ctrl.id )
         {
            ...
            if(copy_to_user((void __user *)ref_cnt_ctrl.data_ptr, &imem_ref_cnt, sizeof(MINT32)) != 0)
            {
...
static MINT32 ISP_RTBC_DEQUE(MINT32 dma)
{
    ...
    if(0 == g_deque_buf.count)
    {
        ...
        {
            LOG_DBG("[rtbc][DEQUE]Buf List:i(%d),memID(%d),size(0x%x),VA(0x%llx),PA(0x%x),bFilled(%d)",i,
    ...
    if(g_deque_buf.count)
    {
        ...
        for(i = 0; i < g_deque_buf.count; i++)
        {
            ...
            if(g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL)
            {
                LOG_DBG("[rtbc][DEQUE]:index(%d),PA(0x%x),memID(%d),size(0x%x),VA(0x%llx)",index + i,
...
static long ISP_Buf_CTRL_FUNC(unsigned long Param)
{
    ...
    if(copy_from_user(&rt_buf_ctrl, (void __user*)Param, sizeof(ISP_BUFFER_CTRL_STRUCT)) == 0)
    {
        ...
        if(g_IspInfo.DebugMask & ISP_DBG_INT)
        {
            LOG_DBG("[rtbc]ctrl(0x%x),buf_id(0x%x),data_ptr(0x%p),ex_data_ptr(0x%p)",rt_buf_ctrl.ctrl,
                                                                                      rt_buf_ctrl.buf_id,
        ...
        switch(rt_buf_ctrl.ctrl)
        {
            ...
            case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE :
                if(copy_from_user(&g_rt_buf_info, (void __user *)rt_buf_ctrl.data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT)) == 0)
                {
                    ...
                    if((reg_val & 0x01) || (reg_val2 & 0x01))
                    {
                        ...
                        if(g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL)
                        {
                            LOG_DBG("[rtbc][ENQUE]:ex_data_ptr(0x%p)",rt_buf_ctrl.ex_data_ptr);
                        }

                        if(0 != rt_buf_ctrl.ex_data_ptr)
                        {
                            if(copy_from_user(&g_ex_rt_buf_info, (void __user *)rt_buf_ctrl.ex_data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT)) == 0)
                            {
                                for(i = 0; i < ISP_RT_BUF_SIZE; i++)
                                {
                                    if(g_pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr == g_rt_buf_info.base_pAddr)
                                    {
                                        ...
                                        LOG_DBG("[rtbc]dma(%d),old(%d),PA(0x%x),VA(0x%llx)",rt_dma,
                                                                                           i,
                                                                                           g_pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr,
                                                                                           g_pstRTBuf->ring_buf[rt_dma].data[i].base_vAddr);
                                        ...
                                        LOG_DBG("[rtbc]dma(%d),new(%d),PA(0x%x),VA(0x%llx)",rt_dma,
                                                                                           i,
                                                                                           g_pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr,
                                                                                           g_pstRTBuf->ring_buf[rt_dma].data[i].base_vAddr);
...
LOG_DBG("+LARB in DEQUE,BWL(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X),220(0x%08X)/(0x%08X),0x14(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)", \
    ...
    ISP_RD32(SMI_LARB4+0x10));
                ...
                if(g_deque_buf.count )
                {
                    if(copy_to_user((void __user *)rt_buf_ctrl.pExtend, &g_deque_buf, sizeof(ISP_DEQUE_BUF_INFO_STRUCT)) != 0)
                    {
   ...
   time_frmb_usec = do_div( time_frmb_sec, 1000000);    //sec and usec
    ...
    //push hw filled buffer to sw list
        ...
        if(IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_SOF1_INT_ST)
        {
            ...
            LOG_DBG("P1_SOF_%d(0x%08lx,0x%08lx,0x%08x,0x%08x)",((ISP_RD32(ISP_REG_ADDR_TG_INTER_ST)&0x00FF0000)>>16),
    ...
    else
    {
        ...
        for(j=0;j<ISP_IRQ_TYPE_AMOUNT_FRMB;j++)
        {
            for(i=0;i<IRQ_USER_NUM_MAX;i++)
            {
                ...
                IspInfo_FrmB.IrqInfo.Status[i][j] |= (IrqStatus[j] & IspInfo_FrmB.IrqInfo.Mask[j]);

                //2. update signal occuring time and passed by signal count
                if(IspInfo_FrmB.IrqInfo.MarkedFlag[i][j] & IspInfo_FrmB.IrqInfo.Mask[j])
                {
                    for(k=0;k<32;k++)
                    {
...
static long ISP_ioctl(struct file *pFile,MUINT32 Cmd,unsigned long Param)
{
    ...
    switch(Cmd)
    {
        ...
        {
            if (copy_from_user(&wakelock_ctrl, (void __user *)Param, sizeof(MUINT32)) != 0)
            {
        ...
        {
            if(copy_from_user(&RegIo, (void __user *)Param, sizeof(ISP_REG_IO_STRUCT)) == 0)
            {
        ...
        {
            if(copy_from_user(&HoldTime, (void __user *)Param, sizeof(ISP_HOLD_TIME_ENUM)) == 0)
            {
                spin_lock(&(g_IspInfo.SpinLockIsp));
        ...
        case ISP_WAIT_IRQ:
        {
            if(copy_from_user(&WaitIrq, (void __user *)Param, sizeof(ISP_WAIT_IRQ_STRUCT)) == 0)
            {
        ...
        case ISP_READ_IRQ:
        {
            if(copy_from_user(&ReadIrq, (void __user *)Param, sizeof(ISP_READ_IRQ_STRUCT)) == 0)
            {
                ...
                if(copy_to_user((void __user *)Param, &ReadIrq, sizeof(ISP_READ_IRQ_STRUCT)) != 0)
                {
        ...
        case ISP_CLEAR_IRQ:
        {
            if(copy_from_user(&ClearIrq, (void __user *)Param, sizeof(ISP_CLEAR_IRQ_STRUCT)) == 0)
            {
        ...
        {
            if(copy_from_user(DebugFlag, (void __user *)Param, sizeof(MUINT32)*2) == 0)
            {
        ...
        case ISP_WAIT_IRQ_FRMB:
            if(copy_from_user(&Irq_FrmB, (void __user *)Param, sizeof(ISP_WAIT_IRQ_STRUCT_FRMB)) == 0)
            {
                if((Irq_FrmB.UserInfo.UserKey>=IRQ_USER_NUM_MAX) || (Irq_FrmB.UserInfo.UserKey<=0))
                {
                ...
                if((Irq_FrmB.UserInfo.Type>=ISP_IRQ_TYPE_AMOUNT_FRMB) || (Irq_FrmB.UserInfo.Type<0))
                {
                    LOG_ERR("invalid type(%d), max(%d)",Irq_FrmB.UserInfo.Type,ISP_IRQ_TYPE_AMOUNT_FRMB);
                    Ret = -EFAULT;
                    break;
                }
                Ret = ISP_WaitIrq_FrmB(&Irq_FrmB);
                //
                if(copy_to_user((void __user *)Param, &Irq_FrmB, sizeof(ISP_WAIT_IRQ_STRUCT_FRMB)) != 0)
                {
                    LOG_ERR("copy_to_user failed");
                    Ret = -EFAULT;
                }
        ...
        case ISP_ED_QUEBUF_CTRL_FRMB:
             if(copy_from_user(&edQueBuf_FrmB, (void __user *)Param, sizeof(ISP_ED_BUFQUE_STRUCT_FRMB)) == 0)
            {
        ...
        case ISP_GET_DMA_ERR_FRMB:
            if(copy_to_user((void __user *)Param, &g_DmaErr_p1[0], sizeof(MUINT32)*nDMA_ERR) != 0){
                LOG_ERR("get dma_err fail\n");
            }
            break;
        case ISP_GET_INT_ERR_FRMB:
            if(copy_to_user((void __user *)Param, g_ISPIntErr, sizeof(MUINT32)*_IRQ_MAX) != 0){
        ...
        case ISP_REGISTER_IRQ_USER_KEY:
            if(copy_from_user(&RegUserKey_FrmB, (void __user *)Param, sizeof(ISP_REGISTER_USERKEY_STRUCT_FRMB)) == 0)
            {
                userKey=ISP_REGISTER_IRQ_USERKEY(RegUserKey_FrmB.userName);
                RegUserKey_FrmB.userKey=userKey;
                 if(copy_to_user((void __user *)Param, &RegUserKey_FrmB, sizeof(ISP_REGISTER_USERKEY_STRUCT_FRMB)) != 0){
...
static long ISP_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    {
        ret = filp->f_op->unlocked_ioctl(filp, ISP_SET_USER_PID,(unsigned long)compat_ptr(arg));
        return ret;
    }
#endif
    case COMPAT_ISP_WAKELOCK_CTRL:
    {
        ret = filp->f_op->unlocked_ioctl(filp, ISP_WAKELOCK_CTRL,(unsigned long)compat_ptr(arg));
        return ret;
    }
    ...
    {
        ret = filp->f_op->unlocked_ioctl(filp, ISP_DEBUG_FLAG_FRMB,(unsigned long)compat_ptr(arg));
        return ret;
    }
    case COMPAT_ISP_SENSOR_FREQ_CTRL:
    {
        ret = filp->f_op->unlocked_ioctl(filp, ISP_SENSOR_FREQ_CTRL,(unsigned long)compat_ptr(arg));
        return ret;
    }
    case COMPAT_ISP_GET_CUR_SOF_FRMB:
    {
        ret = filp->f_op->unlocked_ioctl(filp, ISP_GET_CUR_SOF_FRMB,(unsigned long)compat_ptr(arg));
        return ret;
    }
    case COMPAT_ISP_GET_INT_ERR_FRMB:
    {
        ret = filp->f_op->unlocked_ioctl(filp, ISP_GET_INT_ERR_FRMB,(unsigned long)compat_ptr(arg));
        return ret;
    }
...
static MINT32 mmap_kmem(struct file *filp, struct vm_area_struct *vma)
{
    ...
    LOG_INF("Vma->vm_pgoff(0x%x),Vma->vm_start(0x%x),Vma->vm_end(0x%x),length(0x%x)",\
            (unsigned int)(vma->vm_pgoff),(unsigned int)(vma->vm_start),(unsigned int)(vma->vm_end),(unsigned int)(length));
    if(length>ISP_RTBUF_REG_RANGE)
    {
        LOG_ERR("mmap range error! : length(0x%x),ISP_RTBUF_REG_RANGE(0x%x)!",(unsigned int)(length),ISP_RTBUF_REG_RANGE);
        return -EAGAIN;
    }
...
static MINT32 ISP_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
    ...
    else
    {
        ...
        LOG_INF("pVma->vm_pgoff(0x%x),phy(0x%x),pVmapVma->vm_start(0x%x),pVma->vm_end(0x%x),length(0x%x)",\
            (unsigned int)(pVma->vm_pgoff),(unsigned int)(pVma->vm_pgoff<<PAGE_SHIFT),(unsigned int)(pVma->vm_start),\
            (unsigned int)(pVma->vm_end),(unsigned int)(length));
        ...
        switch(pfn)
        {
            case IMGSYS_BASE_ADDR:    //imgsys
                ...
                {
                    LOG_ERR("mmap range error : length(0x%x),ISP_REG_RANGE(0x%x)!",(unsigned int)(length),ISP_REG_RANGE);
                    return -EAGAIN;
                }
                break;
            case SENINF_BASE_ADDR:
                ...
                {
                    LOG_ERR("mmap range error : length(0x%x),SENINF_REG_RANGE(0x%x)!",(unsigned int)(length),SENINF_REG_RANGE);
                    return -EAGAIN;
                }
                break;
            case PLL_BASE_ADDR:
                if(length>PLL_RANGE)
                {
                    LOG_ERR("mmap range error : length(0x%x),PLL_RANGE(0x%x)!",(unsigned int)(length),PLL_RANGE);
                    return -EAGAIN;
                }
                break;
            case MIPIRX_CONFIG_ADDR:
                ...
                {
                    LOG_ERR("mmap range error : length(0x%x),MIPIRX_CONFIG_RANGE(0x%x)!",(unsigned int)(length),MIPIRX_CONFIG_RANGE);
                    return -EAGAIN;
                }
                break;
            case MIPIRX_ANALOG_ADDR:
                ...
                {
                    LOG_ERR("mmap range error : length(0x%x),MIPIRX_ANALOG_RANGE(0x%x)!",(unsigned int)(length),MIPIRX_ANALOG_RANGE);
                    return -EAGAIN;
                }
                break;
            case GPIO_BASE_ADDR:
                ...
                {
                    LOG_ERR("mmap range error : length(0x%x),GPIO_RANGE(0x%x)!",(unsigned int)(length),GPIO_RANGE);
                    return -EAGAIN;
                }
                break;
            case EFUSE_BASE_ADDR:
                ...
                {
                    LOG_ERR("mmap range error : length(0x%x),EFUSE_RANGE(0x%x)!",(unsigned int)(length),EFUSE_RANGE);
                    return -EAGAIN;
                }
                break;
            default:
...
static MINT32 ISP_probe(struct platform_device *pDev)
{
    ...
    if(pDev == NULL)
    {
        ...
        for(i=0;i<ISP_CAM_BASEADDR_NUM;i++)
        {
            ...
            gISPSYS_Reg_FrmB[i] = cam_isp_dev->regs[i];
         ...
         for(i=0;i<ISP_CAM_IRQ_IDX_NUM;i++)
         {
              ...
              {
                 Ret = request_irq(cam_isp_dev->irq[i], (irq_handler_t)ISP_Irq,
              ...
              LOG_INF("DT, i=%d, name=%s, map_irq=%d\n", i, pDev->dev.of_node->name,cam_isp_dev->irq[i]);
...
m4u_callback_ret_t ISP_M4U_TranslationFault_callback(int port, unsigned int mva, void *data)
{
    ...
    LOG_DBG("[TF_%d]0x%08X %08X", port, (unsigned int)(ISP_TPIPE_ADDR + 0x000C), (unsigned int)ISP_RD32(ISP_ADDR + 0x000C));
    LOG_DBG("[TF_%d]0x%08X %08X", port, (unsigned int)(ISP_TPIPE_ADDR + 0x0010), (unsigned int)ISP_RD32(ISP_ADDR + 0x0010));
    LOG_DBG("[TF_%d]0x%08X %08X", port, (unsigned int)(ISP_TPIPE_ADDR + 0x0074), (unsigned int)ISP_RD32(ISP_ADDR + 0x0074));
    LOG_DBG("[TF_%d]0x%08X %08X", port, (unsigned int)(ISP_TPIPE_ADDR + 0x0078), (unsigned int)ISP_RD32(ISP_ADDR + 0x0078));
    LOG_DBG("[TF_%d]0x%08X %08X", port, (unsigned int)(ISP_TPIPE_ADDR + 0x007c), (unsigned int)ISP_RD32(ISP_ADDR + 0x007c));
    
    switch (port) {
	case M4U_PORT_IMGO:
                LOG_DBG("[TF_IMGO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0300), (unsigned int)ISP_RD32(ISP_ADDR + 0x0300));
                LOG_DBG("[TF_IMGO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0304), (unsigned int)ISP_RD32(ISP_ADDR + 0x0304));
                LOG_DBG("[TF_IMGO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0308), (unsigned int)ISP_RD32(ISP_ADDR + 0x0308));
                LOG_DBG("[TF_IMGO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x030C), (unsigned int)ISP_RD32(ISP_ADDR + 0x030C));
                LOG_DBG("[TF_IMGO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0310), (unsigned int)ISP_RD32(ISP_ADDR + 0x0310));
                LOG_DBG("[TF_IMGO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x031C), (unsigned int)ISP_RD32(ISP_ADDR + 0x031C));
           case M4U_PORT_IMGO2O:
                LOG_DBG("[TF_IMG2O]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0320), (unsigned int)ISP_RD32(ISP_ADDR + 0x0320));
                LOG_DBG("[TF_IMG2O]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0324), (unsigned int)ISP_RD32(ISP_ADDR + 0x0324));
                LOG_DBG("[TF_IMG2O]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0328), (unsigned int)ISP_RD32(ISP_ADDR + 0x0328));
                LOG_DBG("[TF_IMG2O]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x032C), (unsigned int)ISP_RD32(ISP_ADDR + 0x032C));
                LOG_DBG("[TF_IMG2O]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0330), (unsigned int)ISP_RD32(ISP_ADDR + 0x0330));
                LOG_DBG("[TF_IMG2O]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x033C), (unsigned int)ISP_RD32(ISP_ADDR + 0x033C));
            case M4U_PORT_LSCI:
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00A8), (unsigned int)ISP_RD32(ISP_ADDR + 0x00A8));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00B8), (unsigned int)ISP_RD32(ISP_ADDR + 0x00B8));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00BC), (unsigned int)ISP_RD32(ISP_ADDR + 0x00BC));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00E8), (unsigned int)ISP_RD32(ISP_ADDR + 0x00E8));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00EC), (unsigned int)ISP_RD32(ISP_ADDR + 0x00EC));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00F0), (unsigned int)ISP_RD32(ISP_ADDR + 0x00F0));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x026C), (unsigned int)ISP_RD32(ISP_ADDR + 0x026C));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0270), (unsigned int)ISP_RD32(ISP_ADDR + 0x0270));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0274), (unsigned int)ISP_RD32(ISP_ADDR + 0x0274));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0278), (unsigned int)ISP_RD32(ISP_ADDR + 0x0278));
                LOG_DBG("[TF_LSCI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x027c), (unsigned int)ISP_RD32(ISP_ADDR + 0x027c));
            case M4U_PORT_CAM_IMGI:
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0050), (unsigned int)ISP_RD32(ISP_ADDR + 0x0050));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0054), (unsigned int)ISP_RD32(ISP_ADDR + 0x0054));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00AC), (unsigned int)ISP_RD32(ISP_ADDR + 0x00AC));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x00B0), (unsigned int)ISP_RD32(ISP_ADDR + 0x00B0));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0230), (unsigned int)ISP_RD32(ISP_ADDR + 0x0230));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0234), (unsigned int)ISP_RD32(ISP_ADDR + 0x0234));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0238), (unsigned int)ISP_RD32(ISP_ADDR + 0x0238));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x023C), (unsigned int)ISP_RD32(ISP_ADDR + 0x023C));
                LOG_DBG("[TF_IMGI]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0240), (unsigned int)ISP_RD32(ISP_ADDR + 0x0240));
            case M4U_PORT_CAM_ESFKO:
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x035C), (unsigned int)ISP_RD32(ISP_ADDR + 0x035C));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0360), (unsigned int)ISP_RD32(ISP_ADDR + 0x0360));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0364), (unsigned int)ISP_RD32(ISP_ADDR + 0x0364));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0368), (unsigned int)ISP_RD32(ISP_ADDR + 0x0368));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x036C), (unsigned int)ISP_RD32(ISP_ADDR + 0x036C));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0370), (unsigned int)ISP_RD32(ISP_ADDR + 0x0370));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0374), (unsigned int)ISP_RD32(ISP_ADDR + 0x0374));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0378), (unsigned int)ISP_RD32(ISP_ADDR + 0x0378));
                LOG_DBG("[TF_ESFKO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x037C), (unsigned int)ISP_RD32(ISP_ADDR + 0x037C));
            case M4U_PORT_CAM_AAO:
                LOG_DBG("[TF_AAO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0388), (unsigned int)ISP_RD32(ISP_ADDR + 0x0388));
                LOG_DBG("[TF_AAO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x038C), (unsigned int)ISP_RD32(ISP_ADDR + 0x038C));
                LOG_DBG("[TF_AAO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0390), (unsigned int)ISP_RD32(ISP_ADDR + 0x0390));
                LOG_DBG("[TF_AAO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0394), (unsigned int)ISP_RD32(ISP_ADDR + 0x0394));
                LOG_DBG("[TF_AAO]0x%08X %08X",  (unsigned int)(ISP_TPIPE_ADDR + 0x0398), (unsigned int)ISP_RD32(ISP_ADDR + 0x0398));
	default:
	    break;
    }
