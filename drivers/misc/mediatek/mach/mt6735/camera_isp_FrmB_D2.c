...
*
********************************************************************************/
static __inline MUINT32 ISP_MsToJiffies_FrmB(MUINT32 Ms)
{
    return ((Ms * HZ + 512) >> 10);
}
...
*
********************************************************************************/
static __inline MUINT32 ISP_UsToJiffies_FrmB(MUINT32 Us)
{
...
*
********************************************************************************/
static __inline MUINT32 ISP_JiffiesToMs_FrmB(MUINT32 Jiffies)
{
...
*
********************************************************************************/
static __inline MUINT32 ISP_GetIRQState_FrmB(eISPIrq eIrq, MUINT32 type,MUINT32 userKey, MUINT32 stus)
{
    MUINT32 ret;
    MUINT32 flags;
    //
...
static MINT32 ISP_RTBC_ENQUE_FRMB(MINT32 dma,ISP_RT_BUF_INFO_STRUCT_FRMB* prt_buf_info)
{
    ...
    if (buffer_exist) {
        ...
        if ( pstRTBuf_FrmB->ring_buf[rt_dma].total_count == ISP_RT_BUF_SIZE) {
            LOG_ERR("[ENQUE]:[rtbc]:buffer full(%d)",pstRTBuf_FrmB->ring_buf[rt_dma].total_count);
        }
        else {
            //first time add
            ...
            pstRTBuf_FrmB->ring_buf[rt_dma].data[index].base_vAddr = prt_buf_info->base_vAddr;
            pstRTBuf_FrmB->ring_buf[rt_dma].data[index].base_pAddr = prt_buf_info->base_pAddr;
    ...
    if ( 1 == pstRTBuf_FrmB->ring_buf[rt_dma].empty_count) {
        if ( _imgo_ == rt_dma ) {
            ...
            ISP_WR32(ISP_REG_ADDR_IMGO_BASE_ADDR,pstRTBuf_FrmB->ring_buf[rt_dma].data[index].base_pAddr);
        }
        else if ( _img2o_ == rt_dma ) {
              //set base_addr at beginning before VF_EN
            ISP_WR32(ISP_REG_ADDR_IMG2O_BASE_ADDR,pstRTBuf_FrmB->ring_buf[rt_dma].data[index].base_pAddr);
        }
...
#endif
        pstRTBuf_FrmB->ring_buf[rt_dma].pre_empty_count = pstRTBuf_FrmB->ring_buf[rt_dma].empty_count;
...

{
    ...
    if(IspInfo_FrmB.DebugMask & ISP_DBG_BUF_CTRL) {
        LOG_DBG("[rtbc][DEQUE]+");
        LOG_DBG("[rtbc][DEQUE] dma(%d),total(%d)", dma, pstRTBuf_FrmB->ring_buf[rt_dma].total_count);
    }
    //
    pdeque_buf->count = 0;
    ...
    for ( i=0 ; i < pstRTBuf_FrmB->ring_buf[rt_dma].total_count ; i++ ) {
        //
        index = ( pstRTBuf_FrmB->ring_buf[rt_dma].start + i ) % pstRTBuf_FrmB->ring_buf[rt_dma].total_count;
        //
    ...
    if ( pdeque_buf->count ) {
        ...
        for (i=0;i<pdeque_buf->count;i++) {
            pdeque_buf->data[i].memID         = pstRTBuf_FrmB->ring_buf[rt_dma].data[index+i].memID;
            pdeque_buf->data[i].size          = pstRTBuf_FrmB->ring_buf[rt_dma].data[index+i].size;
            pdeque_buf->data[i].base_vAddr    = pstRTBuf_FrmB->ring_buf[rt_dma].data[index+i].base_vAddr;
            pdeque_buf->data[i].base_pAddr    = pstRTBuf_FrmB->ring_buf[rt_dma].data[index+i].base_pAddr;
            pdeque_buf->data[i].timeStampS    = pstRTBuf_FrmB->ring_buf[rt_dma].data[index+i].timeStampS;
            pdeque_buf->data[i].timeStampUs   = pstRTBuf_FrmB->ring_buf[rt_dma].data[index+i].timeStampUs;
            //
            if(IspInfo_FrmB.DebugMask & ISP_DBG_BUF_CTRL) {
...
#ifdef _89SERIAL_
static MUINT32 m_P1_RCNT_INC_CNT = 0;
#endif
...
static long ISP_Buf_CTRL_FUNC_FRMB(unsigned long Param)
{
    ...
    //
    if(copy_from_user(&rt_buf_ctrl, (void __user*)Param, sizeof(ISP_BUFFER_CTRL_STRUCT_FRMB)) == 0)
    {
...
//            case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE:
                //
                if(copy_from_user(&rt_buf_info, (void __user*)rt_buf_ctrl.data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT_FRMB)) == 0) {
                            ...
                            if ( 0 != rt_buf_ctrl.ex_data_ptr ) {
                                ...
                                if(copy_from_user(&deque_buf.data[0], (void __user*)rt_buf_ctrl.ex_data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT_FRMB)) == 0) {
                                    ...
                                    if (deque_buf.data[0].bufIdx != 0xFFFF){
                                        ...
                                        for (i=0;i<ISP_RT_BUF_SIZE;i++) {
                                           if ( pstRTBuf_FrmB->ring_buf[rt_dma].data[i].bufIdx == deque_buf.data[0].bufIdx ) {
                                               break;
                                           }
                                        }
                                    } else {
                                    ...
                                    if (i == ISP_RT_BUF_SIZE) {
                                        ...
                                        for (i=0;i<ISP_RT_BUF_SIZE;i+=4) {
                                            LOG_ERR("[rtbc][replace2]error idx-(0x%x/0x%x/0x%x/0x%x)\n",\
                                              pstRTBuf_FrmB->ring_buf[rt_dma].data[i+0].bufIdx,\
                                              pstRTBuf_FrmB->ring_buf[rt_dma].data[i+1].bufIdx,\
                                              pstRTBuf_FrmB->ring_buf[rt_dma].data[i+2].bufIdx,\
                                              pstRTBuf_FrmB->ring_buf[rt_dma].data[i+3].bufIdx);
                                        }
                                        return -EFAULT;
                                    }
                                    {
                                        //
                                        {
                                            ...
                                            IRQ_LOG_KEEPER(irqT,0,_LOG_DBG,"[rtbc][replace2]dma(%d),idx(%d) PA(0x%x_0x%x)\n",rt_dma,i,pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_pAddr,deque_buf.data[0].base_pAddr);
                                            //spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[irqT]), flags);
                                            pstRTBuf_FrmB->ring_buf[rt_dma].data[i].memID= deque_buf.data[0].memID;
                                            pstRTBuf_FrmB->ring_buf[rt_dma].data[i].size= deque_buf.data[0].size;
                                            pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_pAddr = deque_buf.data[0].base_pAddr;
                                            pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_vAddr = deque_buf.data[0].base_vAddr;
                                            pstRTBuf_FrmB->ring_buf[rt_dma].data[i].bFilled = ISP_RTBC_BUF_EMPTY;
                                            pstRTBuf_FrmB->ring_buf[rt_dma].data[i].image.frm_cnt = _INVALID_FRM_CNT_;

#ifdef _rtbc_buf_que_2_0_
                                            if(pstRTBuf_FrmB->ring_buf[rt_dma].empty_count<pstRTBuf_FrmB->ring_buf[rt_dma].total_count)
                                                pstRTBuf_FrmB->ring_buf[rt_dma].empty_count++;
                                            else{
                                                spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[irqT_Lock]), flags);
                                                IRQ_LOG_PRINTER(irqT,0,_LOG_DBG);
                                                LOG_ERR("[rtbc]dma(%d),PA(0x%x),over enque",rt_dma,pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_pAddr);
                                                return -EFAULT;
                                            }
                                ...
                                for (i=0;i<ISP_RT_BUF_SIZE;i++) {
                                    ...
                                    if ( pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_pAddr == rt_buf_info.base_pAddr ) {
                                        ...
                                        if(pstRTBuf_FrmB->ring_buf[rt_dma].empty_count<pstRTBuf_FrmB->ring_buf[rt_dma].total_count)
                                            ...
                                            spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[irqT_Lock]), flags);
                                            IRQ_LOG_PRINTER(irqT,0,_LOG_DBG);
                                            LOG_ERR("[rtbc]error:dma(%d),PA(0x%x),over enque",rt_dma,pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_pAddr);
                                            return -EFAULT;
                                        }

                                        //double check
                                        if (1) {
                                            if ( rt_buf_info.bufIdx != pstRTBuf_FrmB->ring_buf[rt_dma].data[i].bufIdx) {
                                                LOG_ERR("[rtbc][replace2]error: BufIdx MisMatch. 0x%x/0x%x", rt_buf_info.bufIdx, pstRTBuf_FrmB->ring_buf[rt_dma].data[i].bufIdx);
                                            }
                                        }
                            ...
                            //RCNT_INC++
                            {
                                ...
                                if(rt_buf_ctrl.ctrl == ISP_RT_BUF_CTRL_ENQUE_FRMB){
                                    if((_IRQ == irqT)|| (_IRQ_D == irqT) ) {
                                        if( (MTRUE == pstRTBuf_FrmB->ring_buf[ch_imgo].active) && (MTRUE == pstRTBuf_FrmB->ring_buf[ch_img2o].active) ){
                                            if ( 0 != rt_buf_ctrl.ex_data_ptr ){
                                            ...
                                            for(z=0;z<ISP_RT_BUF_SIZE;z++) {
                                                if(dma_en_recorder[ch_imgo][mFwRcnt.rdIdx[irqT]] && dma_en_recorder[ch_img2o][mFwRcnt.rdIdx[irqT]]) {
                                                    ...
                                                    dma_en_recorder[ch_imgo][mFwRcnt.rdIdx[irqT]] = dma_en_recorder[ch_img2o][mFwRcnt.rdIdx[irqT]] = MFALSE;
                                            ...
                                            if(MTRUE == pstRTBuf_FrmB->ring_buf[rt_dma].active) {
                                                if ( 0 != rt_buf_ctrl.ex_data_ptr ){
                                                    if ( (p1_fbc[rt_dma].Bits.FB_NUM == p1_fbc[rt_dma].Bits.FBC_CNT) ||
                                                         ((p1_fbc[rt_dma].Bits.FB_NUM-1) == p1_fbc[rt_dma].Bits.FBC_CNT)) {
                                                        mFwRcnt.bLoadBaseAddr[irqT] = MTRUE;
                                    ...
                                    if( (MTRUE == pstRTBuf_FrmB->ring_buf[ch_imgo].active) && (MTRUE == pstRTBuf_FrmB->ring_buf[ch_img2o].active) ){
                                        ...
                                        _openedDma = 2;
                                        if( (dma_en_recorder[ch_imgo][0] == MTRUE) && (dma_en_recorder[ch_img2o][0] == MTRUE))
                                        {
                        ...
                        for (i=0;i<deque_buf.count;i++) {
                            ...
                            if ( 0 != prv_tstamp_s[rt_dma] ) {
                                if ( ( prv_tstamp_s[rt_dma] > deque_buf.data[i].timeStampS) ||
                                    ...
                                    LOG_ERR("[rtbc]TS rollback,D(%d),prv\"%d.%06d\",cur\"%d.%06d\"",rt_dma,(int)(prv_tstamp_s[rt_dma]),(int)(prv_tstamp_us[rt_dma]),(int)(deque_buf.data[i].timeStampS),(int)(deque_buf.data[i].timeStampUs));
                                    ISP_DumpReg_FrmB();
                                }
                            }
...
LOG_DBG("+LARB in DEQUE,BWL(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X),220(0x%08X)/(0x%08X),0x14(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)", \
    ...
    ISP_RD32(SMI_LARB4+0x10));
                ...
                if ( deque_buf.count ) {
                    ...
                    if(copy_to_user((void __user*)rt_buf_ctrl.pExtend, &deque_buf, sizeof(ISP_DEQUE_BUF_INFO_STRUCT_FRMB)) != 0)
                    {
            ...
            case ISP_RT_BUF_CTRL_IS_RDY_FRMB:
                ...
                if ( ISP_RTBC_BUF_FILLED == pstRTBuf_FrmB->ring_buf[rt_dma].data[pstRTBuf_FrmB->ring_buf[rt_dma].read_idx].bFilled ) {
                    bWaitBufRdy = 0;
                }
                if(IspInfo_FrmB.DebugMask & ISP_DBG_BUF_CTRL){
                    MUINT32 z;
                    IRQ_LOG_KEEPER(irqT,0,_LOG_DBG,"cur dma:%d,read idx = %d,total cnt = %d,bWaitBufRdy= %d  ,",rt_dma,pstRTBuf_FrmB->ring_buf[rt_dma].read_idx,pstRTBuf_FrmB->ring_buf[rt_dma].total_count,bWaitBufRdy);
                    for(z=0;z<pstRTBuf_FrmB->ring_buf[rt_dma].total_count;z++){
                        IRQ_LOG_KEEPER(irqT,0,_LOG_DBG,"%d_",pstRTBuf_FrmB->ring_buf[rt_dma].data[z].bFilled);
                    }
            ...
            case ISP_RT_BUF_CTRL_CLEAR_FRMB:
                ...
                memset((char*)&pstRTBuf_FrmB->ring_buf[rt_dma],0x00,sizeof(ISP_RT_RING_BUF_INFO_STRUCT_FRMB));
...

                {
                    ...
                    if((pstRTBuf_FrmB->ring_buf[_imgo_].active== MFALSE) && (pstRTBuf_FrmB->ring_buf[_img2o_].active== MFALSE) )
                            out[0] = _IRQ;
...
static MINT32 ISP_LostP1Done_ErrHandle(MUINT32 dma)
{
    switch(dma){
        ...
        case _img2o_:
            ...
            pstRTBuf_FrmB->ring_buf[dma].start = pstRTBuf_FrmB->ring_buf[dma].start%pstRTBuf_FrmB->ring_buf[dma].total_count;
            break;
        default: break;
    }
}
#endif
static MINT32 ISP_SOF_Buf_Get_FrmB(eISPIrq irqT,unsigned long long sec,unsigned long usec,MBOOL bDrop,CQ_RTBC_FBC* pFbc,MUINT32* pCurr_pa)
{
#if defined(_rtbc_use_cq0c_)
...
#ifdef _89SERIAL_
        pstRTBuf_FrmB->ring_buf[ch_imgo].start = ((imgo_fbc.Bits.FBC_CNT + m_P1_RCNT_INC_CNT)%3 + 1) - 1;
        pstRTBuf_FrmB->ring_buf[ch_img2o].start = ((img2o_fbc.Bits.FBC_CNT + m_P1_RCNT_INC_CNT)%3 + 1) - 1;
#else
    ...
    if ( 0 == pstRTBuf_FrmB->dropCnt ) {
        ...
        //
        if ( curr_pa != pstRTBuf_FrmB->ring_buf[_working_dma].data[_dma_cur_fw_idx].base_pAddr ) {
            //
            ...
            for( i = 0; i< pstRTBuf_FrmB->ring_buf[_working_dma].total_count; i++ ) {
                //
                if ( curr_pa == pstRTBuf_FrmB->ring_buf[_working_dma].data[i].base_pAddr ) {
                    //
                    if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
    ...
    //LOG_INF("RTBC_DBG7 img2o:%d %d %d\n",pstRTBuf_FrmB->ring_buf[_img2o_].data[0].bFilled,pstRTBuf_FrmB->ring_buf[_img2o_].data[1].bFilled,pstRTBuf_FrmB->ring_buf[_img2o_].data[2].bFilled);
        ...
        if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_3){
            ...
            MUINT32 _tmp = pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].timeStampS*1000000 + pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].timeStampUs;
            if(g1stSof[irqT]){
                m_sec = 0;
                m_usec = 0;
            }
        ...
        if(_IRQ == irqT) {
            ...
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.xsize = ISP_RD32(ISP_REG_ADDR_IMGO_XSIZE)&0x3FFF;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.w = ((_tmp>>16)&0x7FFF) - _tmp&0x7FFF;
            _tmp = ISP_RD32(TG_REG_ADDR_GRAB_H);
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.h = ((_tmp>>16)&0x1FFF) - _tmp&0x1FFF;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.stride = ISP_RD32(ISP_REG_ADDR_IMGO_STRIDE)&0x3FFF;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.fmt = (ISP_RD32(ISP_REG_ADDR_FMT_SEL_P1)&0xF000)>>12;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.pxl_id = (ISP_RD32(ISP_REG_ADDR_PIX_ID)&0x03);
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.m_num_0 = ISP_RD32(ISP_REG_ADDR_TG_MAGIC_0);
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.frm_cnt = (ISP_RD32(ISP_REG_ADDR_TG_INTER_ST)&0x00FF0000)>>16;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].dmaoCrop.x = ISP_RD32(ISP_REG_ADDR_IMGO_CROP) & 0x3fff;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].dmaoCrop.y = (ISP_RD32(ISP_REG_ADDR_IMGO_CROP)>>16) & 0x1fff;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].dmaoCrop.w = (ISP_RD32(ISP_REG_ADDR_IMGO_XSIZE)&0x3FFF) + 1;
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].dmaoCrop.h = (ISP_RD32(ISP_REG_ADDR_IMGO_YSIZE)&0x1FFF) + 1;
            ...
            //
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.xsize = ISP_RD32(ISP_REG_ADDR_IMG2O_XSIZE)&0x3FFF;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.w = pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.w;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.h = (ISP_RD32(ISP_REG_ADDR_IMG2O_YSIZE)&0x1FFF) + 1;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.stride = ISP_RD32(ISP_REG_ADDR_IMG2O_STRIDE)&0x3FFF;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.fmt = (ISP_RD32(ISP_REG_ADDR_FMT_SEL_P1)&0xF000)>>12;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.pxl_id = pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.pxl_id;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.m_num_0 = pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.m_num_0;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].image.frm_cnt = pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].image.frm_cnt;
            ...
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].HrzInfo.srcW = ((_tmp>>16)&0x7FFF) - _tmp&0x7FFF;
            _tmp = ISP_RD32(TG_REG_ADDR_GRAB_H);
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].HrzInfo.srcH = ((_tmp>>16)&0x1FFF) - _tmp&0x1FFF;

            _tmp = ISP_RD32(ISP_REG_ADDR_HRZ_OUT_IMG);
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].HrzInfo.dstW =  _tmp&0x1FFF;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].HrzInfo.dstH = pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].HrzInfo.srcH;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].dmaoCrop.x = ISP_RD32(ISP_REG_ADDR_IMG2O_CROP) & 0x3fff;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].dmaoCrop.y = (ISP_RD32(ISP_REG_ADDR_IMG2O_CROP)>>16) & 0x1fff;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].dmaoCrop.w = (ISP_RD32(ISP_REG_ADDR_IMG2O_XSIZE)&0x3FFF) + 1;
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].dmaoCrop.h = (ISP_RD32(ISP_REG_ADDR_IMG2O_YSIZE)&0x1FFF) + 1;
            ...
            if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
                IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc]TStamp(%d.%06d),curr(%d),pa(0x%x/0x%x),cq0c(0x%x)\n", \
                    ...
                    imgo_idx,
                    pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].base_pAddr,  \
                    pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].base_pAddr,  \
...
static MINT32 ISP_DONE_Buf_Time_FrmB(eISPIrq irqT,unsigned long long sec,unsigned long usec,CQ_RTBC_FBC* pFbc)
{
    int i,k;
    int i_dma;
    unsigned int curr;
    ...
    {
        LOG_ERR("non-supported dma port (%d/%d)\n",pstRTBuf_FrmB->ring_buf[ch_imgo].active,pstRTBuf_FrmB->ring_buf[ch_img2o].active);
        return 0;
    }
#ifdef _89SERIAL_
    if(_dma_cur_fbc.Bits.WCNT > 0){
        ...
        if( _sw_wcnt  > (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2) )  {
            ...
            if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
                IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc] WCNT(%d)>start+2(%d),shiftT(%d)\n",_sw_wcnt, pstRTBuf_FrmB->ring_buf[_working_dma].start + 2, shiftT);
            }
            if(shiftT > 0)
                IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc%d]:alert(%d,%d)\n",irqT,pstRTBuf_FrmB->ring_buf[_working_dma].start,_sw_wcnt);
        }
        else if( _sw_wcnt  < (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2) )  {
            shiftT = _sw_wcnt + _dma_cur_fbc.Bits.FB_NUM - (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2);
            if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
                IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc] WCNT(%d)<start+2(%d),shiftT(%d)\n",_sw_wcnt, pstRTBuf_FrmB->ring_buf[_working_dma].start + 2, shiftT);
            }
            if(shiftT >= _dma_cur_fbc.Bits.FB_NUM)
            {
                LOG_ERR("err shiftT = (%d,%d ,%d)\n",_sw_wcnt,_dma_cur_fbc.Bits.FB_NUM,pstRTBuf_FrmB->ring_buf[_working_dma].start);
                shiftT = (_dma_cur_fbc.Bits.FB_NUM?(_dma_cur_fbc.Bits.FB_NUM - 1):(_dma_cur_fbc.Bits.FB_NUM));
            }
        }
        else{}//_dma_cur_fbc.Bits.WCNT == (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2)
    }
#else
    //isr cb timing shift err handle
    if(_dma_cur_fbc.Bits.WCNT > 0){
        if( _dma_cur_fbc.Bits.WCNT  > (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2) )  {
            shiftT = _dma_cur_fbc.Bits.WCNT - pstRTBuf_FrmB->ring_buf[_working_dma].start - 2;
            if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
                IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc] WCNT(%d)>start+2(%d),shiftT(%d)\n",_dma_cur_fbc.Bits.WCNT, pstRTBuf_FrmB->ring_buf[_working_dma].start + 2, shiftT);
            }
            if(shiftT > 0)
                IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc%d]:alert(%d,%d)\n",irqT,pstRTBuf_FrmB->ring_buf[_working_dma].start,_dma_cur_fbc.Bits.WCNT);
        }
        else if( _dma_cur_fbc.Bits.WCNT  < (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2) )  {
            shiftT = _dma_cur_fbc.Bits.WCNT + _dma_cur_fbc.Bits.FB_NUM - (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2);
            if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
                IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc] WCNT(%d)<start+2(%d),shiftT(%d)\n",_dma_cur_fbc.Bits.WCNT, pstRTBuf_FrmB->ring_buf[_working_dma].start + 2, shiftT);
            }
            if(shiftT >= _dma_cur_fbc.Bits.FB_NUM)
            {
                LOG_ERR("err shiftT = (%d,%d ,%d)\n",_dma_cur_fbc.Bits.WCNT,_dma_cur_fbc.Bits.FB_NUM,pstRTBuf_FrmB->ring_buf[_working_dma].start);
                shiftT = (_dma_cur_fbc.Bits.FB_NUM?(_dma_cur_fbc.Bits.FB_NUM - 1):(_dma_cur_fbc.Bits.FB_NUM));
            }
        }
        else{}//_dma_cur_fbc.Bits.WCNT == (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2)
...
#endif
    {
        for ( i=0;i<=1;i++ ) {
            ...
            while(1)
            {
                ...
                //
                if(pstRTBuf_FrmB->ring_buf[i_dma].data[curr].bFilled == ISP_RTBC_BUF_EMPTY)
                {
                    if(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2)
                        IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"[rtbc][DONE]:dma_%d,fill buffer,cur_%d\n",i_dma,curr);
                    pstRTBuf_FrmB->ring_buf[i_dma].data[curr].bFilled = ISP_RTBC_BUF_FILLED;
                    //start + 1
                ...
                else
                {
                    if(1){//(IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
                        LOG_ERR("dma_%d,cur_%d,bFilled_%d != EMPTY(%d %d %d)\n",\
                                ...
                                curr,\
                                pstRTBuf_FrmB->ring_buf[i_dma].data[curr].bFilled,\
                                pstRTBuf_FrmB->ring_buf[i_dma].data[0].bFilled,pstRTBuf_FrmB->ring_buf[i_dma].data[1].bFilled,pstRTBuf_FrmB->ring_buf[i_dma].data[2].bFilled);
    ...
    if(pstRTBuf_FrmB->ring_buf[ch_imgo].active && pstRTBuf_FrmB->ring_buf[ch_img2o].active){
        if(pstRTBuf_FrmB->ring_buf[ch_imgo].start!= pstRTBuf_FrmB->ring_buf[ch_img2o].start){
            LOG_ERR("start idx mismatch %d_%d(%d %d %d,%d %d %d)",pstRTBuf_FrmB->ring_buf[ch_imgo].start,pstRTBuf_FrmB->ring_buf[ch_img2o].start,\
            pstRTBuf_FrmB->ring_buf[ch_imgo].data[0].bFilled,pstRTBuf_FrmB->ring_buf[ch_imgo].data[1].bFilled,pstRTBuf_FrmB->ring_buf[ch_imgo].data[2].bFilled,\
            pstRTBuf_FrmB->ring_buf[ch_img2o].data[0].bFilled,pstRTBuf_FrmB->ring_buf[ch_img2o].data[1].bFilled,pstRTBuf_FrmB->ring_buf[ch_img2o].data[2].bFilled);
        }
    }
...
static MINT32 ISP_MARK_IRQ(ISP_WAIT_IRQ_STRUCT_FRMB irqinfo)
{
    ...
    spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
    IspInfo_FrmB.IrqInfo.MarkedFlag[irqinfo.UserInfo.UserKey][irqinfo.UserInfo.Type] |= irqinfo.UserInfo.Status;
    spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
...
static MINT32 ISP_GET_MARKtoQEURY_TIME(ISP_WAIT_IRQ_STRUCT_FRMB* irqinfo)
{
    ...
    spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
    if(irqinfo->UserInfo.Status & IspInfo_FrmB.IrqInfo.MarkedFlag[irqinfo->UserInfo.UserKey][irqinfo->UserInfo.Type])
    {
        //
...
static MINT32 ISP_FLUSH_IRQ(ISP_WAIT_IRQ_STRUCT_FRMB irqinfo)
{
    ...
    //1. enable signal
    spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
    IspInfo_FrmB.IrqInfo.Status[irqinfo.UserInfo.UserKey][irqinfo.UserInfo.Type] |= irqinfo.UserInfo.Status;
    spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
...
static MINT32 ISP_WaitIrq_FrmB(ISP_WAIT_IRQ_STRUCT_FRMB* WaitIrq)
{
    ...
    if(IspInfo_FrmB.DebugMask & ISP_DBG_INT)
    {
        if(WaitIrq->UserInfo.Status& (ISP_IRQ_INT_STATUS_SOF1_INT_ST|ISP_IRQ_INT_STATUS_PASS1_TG1_DON_ST/*|ISP_IRQ_P1_STATUS_D_SOF1_INT_ST|ISP_IRQ_P1_STATUS_D_PASS1_DON_ST*/)){
    ...
    if(WaitIrq->Clear == ISP_IRQ_CLEAR_STATUS_FRMB)
    {
        ...
        IspInfo_FrmB.IrqInfo.Status[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type] &= (~WaitIrq->UserInfo.Status);
        spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
        return Ret;
    }
    else
    {
        if(WaitIrq->UserInfo.Status & IspInfo_FrmB.IrqInfo.MarkedFlag[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type])
        {
            ...
            freeze_passbysigcnt = !(ISP_GetIRQState_FrmB(eIrq,WaitIrq->UserInfo.Type,WaitIrq->UserInfo.UserKey, WaitIrq->UserInfo.Status));
        }
        else
        {
            ...
            {
                spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
                if(IspInfo_FrmB.IrqInfo.Status[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type] & WaitIrq->UserInfo.Status)
                {
            ...
            else if(WaitIrq->Clear == ISP_IRQ_CLEAR_ALL_FRMB)
            {
                spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
                //LOG_DBG("WARNING: Clear(%d), Type(%d): IrqStatus(0x%08X) has been cleared",WaitIrq->Clear,WaitIrq->Type,IspInfo_FrmB.IrqInfo.Status[WaitIrq->Type]);
                IspInfo_FrmB.IrqInfo.Status[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type] = 0;
                spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
            }
        }
    }
    ...
    Timeout = wait_event_interruptible_timeout(
                        ...
                        ISP_GetIRQState_FrmB(eIrq,WaitIrq->UserInfo.Type,WaitIrq->UserInfo.UserKey, WaitIrq->UserInfo.Status),
                        ISP_MsToJiffies_FrmB(WaitIrq->Timeout));

    if((0 == WaitIrq->UserInfo.UserKey) && (IspInfo_FrmB.DebugMask & ISP_DBG_INT) && (WaitIrq->UserInfo.Status & ISP_IRQ_INT_STATUS_SOF1_INT_ST))
    {
#if CONFIG_K_FOR_SYSTRACE
        char strName[128];
    ...
    if(WaitIrq->UserInfo.Status & IspInfo_FrmB.IrqInfo.MarkedFlag[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type])
    {
        ...
        {
             WaitIrq->TimeInfo.passedbySigcnt = IspInfo_FrmB.IrqInfo.PassedBySigCnt[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type][idx]- 1;
        }
        else
        {
            WaitIrq->TimeInfo.passedbySigcnt = IspInfo_FrmB.IrqInfo.PassedBySigCnt[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type][idx];
        }
    }
    LOG_DBG(" [WAITIRQv3]user(%d)  sigNum(%d/%d), d0 sec/usec (%d/%d), d1 sec/usec (%d/%d)\n",WaitIrq->UserInfo.UserKey,\
             IspInfo_FrmB.IrqInfo.PassedBySigCnt[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type][idx],WaitIrq->TimeInfo.passedbySigcnt,WaitIrq->TimeInfo.tMark2WaitSig_sec,WaitIrq->TimeInfo.tMark2WaitSig_usec,\
            WaitIrq->TimeInfo.tLastSig2GetSig_sec,WaitIrq->TimeInfo.tLastSig2GetSig_usec);
    //
    #if 0
    if(IspInfo_FrmB.DebugMask & ISP_DBG_INT)
    {
    ...
    if(WaitIrq->UserInfo.Status & IspInfo_FrmB.IrqInfo.MarkedFlag[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type])
    {
        IspInfo_FrmB.IrqInfo.MarkedFlag[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type] &= (~WaitIrq->UserInfo.Status);
        IspInfo_FrmB.IrqInfo.MarkedTime_usec[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type][idx]= 0;
        IspInfo_FrmB.IrqInfo.MarkedTime_sec[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type][idx]= 0;
        IspInfo_FrmB.IrqInfo.PassedBySigCnt[WaitIrq->UserInfo.UserKey][WaitIrq->UserInfo.Type][idx]=0;
    }
    spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
...
//    MUINT32 IrqStatus[ISP_IRQ_TYPE_AMOUNT]={0};
    ...
    MUINT32 d_cur_v_cnt = 0;
        ...
        if( (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_PASS1_TG1_DON_ST) || (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_SOF1_INT_ST) )
    ...
    if(IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_SOF1_INT_ST)
    {
...
#ifdef _rtbc_buf_que_2_0_
        {
            ...
            if (1 == mFwRcnt.bLoadBaseAddr[_IRQ]) {
                if(pstRTBuf_FrmB->ring_buf[_imgo_].active) {
                    IRQ_LOG_KEEPER(_IRQ,m_CurrentPPB,_LOG_INF," p1_%d:wr2Phy_0x%x: ",_imgo_,pstRTBuf_FrmB->ring_buf[_imgo_].data[pstRTBuf_FrmB->ring_buf[_imgo_].start].base_pAddr);
                    ISP_WR32(ISP_REG_ADDR_IMGO_BASE_ADDR,pstRTBuf_FrmB->ring_buf[_imgo_].data[pstRTBuf_FrmB->ring_buf[_imgo_].start].base_pAddr);
                }
                if(pstRTBuf_FrmB->ring_buf[_img2o_].active) {
                    IRQ_LOG_KEEPER(_IRQ,m_CurrentPPB,_LOG_INF," p1_%d:wr2Phy_0x%x: ",_img2o_,pstRTBuf_FrmB->ring_buf[_img2o_].data[pstRTBuf_FrmB->ring_buf[_img2o_].start].base_pAddr);
                    ISP_WR32(ISP_REG_ADDR_IMG2O_BASE_ADDR,pstRTBuf_FrmB->ring_buf[_img2o_].data[pstRTBuf_FrmB->ring_buf[_img2o_].start].base_pAddr);
                }
        ...
        if(IspInfo_FrmB.DebugMask & ISP_DBG_INT) {
            ...
            IRQ_LOG_KEEPER(_IRQ,m_CurrentPPB,_LOG_INF,"P1_SOF_%d_%d(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x)\n",sof_count[_PASS1],cur_v_cnt, \
                                                 (unsigned int)_fbc_chk[0].Reg_val, (unsigned int)_fbc_chk[1].Reg_val, \
                                                 ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR), \
                                                 ISP_RD32(ISP_REG_ADDR_IMG2O_BASE_ADDR), \
...
static MBOOL ISP_BufWrite_Alloc_FrmB(void)
{
    ...
    for(i=0; i<ISP_BUF_WRITE_AMOUNT; i++)
    {
        IspInfo_FrmB.BufInfo.Write[i].Status = ISP_BUF_STATUS_EMPTY_FRMB;
        IspInfo_FrmB.BufInfo.Write[i].Size = 0;
        IspInfo_FrmB.BufInfo.Write[i].pData = (MUINT8*)kmalloc(ISP_BUF_SIZE_WRITE, GFP_ATOMIC);
        if(IspInfo_FrmB.BufInfo.Write[i].pData == NULL)
        {
...
*
********************************************************************************/
static MINT32 ISP_release_FrmB()
{
