...
#include <mach/camera_sysram_D1.h>
#include <mach/mt_reg_base.h>
#include <mach/camera_sysram_imp_D1.h>
	...
	struct vm_area_struct *pVma)
{
	...
	if ((length > ISP_VALID_REG_RANGE) || (pfn < IMGSYS_BASE_ADDR) || (pfn > (IMGSYS_BASE_ADDR+ISP_VALID_REG_RANGE))) {
		LOG_ERR("mmap range error : vm_start(0x%lx), vm_end(0x%lx), length(%lx), pfn(%u)!", pVma->vm_start, pVma->vm_end, length, (unsigned int)pfn);
		return -EAGAIN;
	}
	if (remap_pfn_range(
		pVma,
