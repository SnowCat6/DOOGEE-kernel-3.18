...
#include <mach/camera_sysram_D2.h>
#include <mach/mt_reg_base.h>
#include <mach/camera_sysram_imp_D2.h>
    ...
    struct vm_area_struct*  pVma)
{
	...
	{
		LOG_ERR("mmap range error : vm_start(0x%lx),vm_end(0x%lx),length(0x%lx),pfn(0x%lx)!",pVma->vm_start,pVma->vm_end,length,pfn);
		return -EAGAIN;
	}
    if(remap_pfn_range(
            pVma,
...
static int compat_get_sysram_userenum_data(
            compat_SYSRAM_USER_ENUM __user *data32,
...
{
    long ret;
    LOG_MSG("SYSRAM_ioctl_compat E, cmd(0x%x), SYSRAM_ALLOC/SYSRAM_FREE/SYSRAM_DUMP/compat_SYSRAM_ALLOC/compat_SYSRAM_FREE(0x%x/0x%x/0x%x/0x%x/0x%x)",cmd,(unsigned int)SYSRAM_ALLOC,(unsigned int)SYSRAM_FREE,(unsigned int)SYSRAM_DUMP,(unsigned int)COMPAT_SYSRAM_ALLOC,(unsigned int)COMPAT_SYSRAM_FREE);
