				  ...
				  const char *buf, size_t count)
{
    ...
    node = of_find_compatible_node(NULL,NULL,"mediatek,rf_clock_buffer");
    if(node) {
        of_property_read_u32(node,"buffer1",&CLK_BUF1_STATUS);
        of_property_read_u32(node,"buffer2",&CLK_BUF2_STATUS);
        of_property_read_u32(node,"buffer3",&CLK_BUF3_STATUS);
        of_property_read_u32(node,"buffer4",&CLK_BUF4_STATUS);	
    }
    else {
        clk_buf_err("%s can't find compatible node\n", __func__);
    }    
