...
#define EMI_REG_BASE            0x10203000
...
#ifdef USING_PR_LOG
#define REG_DUMP(addr) do { dcm_info("%-30s(0x%08lx): 0x%08x \n", #addr, addr, reg_read(addr)); } while(0)
#else
#define REG_DUMP(addr) do { dcm_info("%s(0x%X): 0x%X \n", #addr, addr, reg_read(addr)); } while(0)
#endif
...
int dcm_armcore_pll_clkdiv(int pll, int div)
{
        ...
        reg_write(TOP_CKMUXSEL, aor(reg_read(TOP_CKMUXSEL),
                                    ~(3<<2), 0<<0)); //26Mhz
...

        {
                /* the cg of pll_src_2 */
                if (pll==2) 
                        reg_write(APMIXED_PLL_CON2, aor(reg_read(APMIXED_PLL_CON2), ~(0x1<<5), 1 << 5));
        }
                                    ...
                                    ~(3<<2), pll<<2));
        {
                /* cg clear after mux */
                if (pll!=2) 
                        reg_write(APMIXED_PLL_CON2, aor(reg_read(APMIXED_PLL_CON2), ~(0x1<<5), 0 << 5));
        }

        reg_write(TOP_CKDIV1, 
                  aor(reg_read(TOP_CKDIV1), ~(0x1f<<0), div << 0)); 
	...
	if (on) {
		...
		reg_write(TOPCKG_DCM_CFG,
			aor(reg_read(TOPCKG_DCM_CFG),
				...
				and(~reg_read(TOPCKG_DCM_CFG),
					TOPCKG_DCM_CFG_FMEM_TOGGLE_MASK)));
		reg_write(TOPCKG_DCM_CFG,
			aor(reg_read(TOPCKG_DCM_CFG),
				~TOPCKG_DCM_CFG_FMEM_MASK,
				TOPCKG_DCM_CFG_FMEM_ON));
...
int dcm_infra(ENUM_INFRA_DCM on)
{
        ...
        if (on) {
                ...
                reg_write(INFRA_GLOBALCON_DCMDBC, 
                          aor(reg_read(INFRA_GLOBALCON_DCMDBC), ~INFRA_GLOBALCON_DCMDBC_MASK, INFRA_GLOBALCON_DCMDBC_ON));
                reg_write(INFRA_GLOBALCON_DCMFSEL, 
                          aor(reg_read(INFRA_GLOBALCON_DCMFSEL), ~INFRA_GLOBALCON_DCMFSEL_MASK, INFRA_GLOBALCON_DCMFSEL_ON));
#endif 
                reg_write(INFRA_GLOBALCON_DCMCTL, 
                          aor(reg_read(INFRA_GLOBALCON_DCMCTL), ~INFRA_GLOBALCON_DCMCTL_MASK, INFRA_GLOBALCON_DCMCTL_ON));
        }
        else {
                reg_write(INFRA_GLOBALCON_DCMCTL, 
                          aor(reg_read(INFRA_GLOBALCON_DCMCTL), ~INFRA_GLOBALCON_DCMCTL_MASK, INFRA_GLOBALCON_DCMCTL_OFF));
        }                
...
#if 1  //override the dbc and fsel setting !!
                reg_write(PERI_GLOBALCON_DCMDBC, 
                          aor(reg_read(PERI_GLOBALCON_DCMDBC), ~PERI_GLOBALCON_DCMDBC_MASK, PERI_GLOBALCON_DCMDBC_ON));
                reg_write(PERI_GLOBALCON_DCMFSEL, 
                          aor(reg_read(PERI_GLOBALCON_DCMFSEL), ~PERI_GLOBALCON_DCMFSEL_MASK, PERI_GLOBALCON_DCMFSEL_ON));
#endif 
...
#if 1  //override the dbc and fsel setting !!
                reg_write(PERI_GLOBALCON_DCMDBC, 
                          aor(reg_read(PERI_GLOBALCON_DCMDBC), ~PERI_GLOBALCON_DCMDBC_MASK, PERI_GLOBALCON_DCMDBC_OFF));
                reg_write(PERI_GLOBALCON_DCMFSEL, 
                          aor(reg_read(PERI_GLOBALCON_DCMFSEL), ~PERI_GLOBALCON_DCMFSEL_MASK, PERI_GLOBALCON_DCMFSEL_OFF));
#endif 
                reg_write(PERI_GLOBALCON_DCMCTL, aor(reg_read(PERI_GLOBALCON_DCMCTL), 
                                                     ~PERI_GLOBALCON_DCMCTL_MASK,
...
int dcm_ddrphy(ENUM_DDRPHY_DCM on)
{
        if (on) {
			  ...
			  aor(reg_read(DDRPHY_MEMPLL_DIVIDER),
                                        ~DDRPHY_MEMPLL_DIVIDER_MASK,
                                        DDRPHY_MEMPLL_DIVIDER_ON));
        } else {
                reg_write(DDRPHY_MEMPLL_DIVIDER,
			  aor(reg_read(DDRPHY_MEMPLL_DIVIDER),
                                        ~DDRPHY_MEMPLL_DIVIDER_MASK,
                                        DDRPHY_MEMPLL_DIVIDER_OFF));
        }
...
{
        if (on) {
                reg_write(EFUSE_REG_DCM_ON, aor(reg_read(EFUSE_REG_DCM_ON), 
                                            ~(1<<0),
                                            (1<<0)));
        }
        else {
