...
static void display_early_memory_info(void)
{
	...
	u64 extm_base = 0x22345678, extm_size = 0;
	u64 tee_base = 0x32345678, tee_size = 0;
	/* system memory */
	...
	if (node) {
		...
		kernel_mem_sz += fb_size;

		/* external memory */
    		tags = (struct tag *)of_get_flat_dt_prop(node,
				"atag,ext_mem", NULL);
		if (tags) {
			extm_base = tags->u.mem64.start;
			extm_size = tags->u.mem64.size;
		}
		...
		if (tags) {
			tee_base = tags->u.tee_reserved_mem.start;
			tee_size = tags->u.tee_reserved_mem.size;
			kernel_mem_sz += fb_size;
		}
	}
	...
	if (extm_size) {
		MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
		"[PHY layout]ext_mem:   0x%llx - 0x%llx  (0x%llx)\n",
		(unsigned long long)extm_base,
		(unsigned long long)(extm_base + extm_size - 1),
		(unsigned long long)extm_size);
	}

	if (tee_size) {
		MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
		"[PHY layout]tee_mem:   0x%llx - 0x%llx  (0x%llx)\n",
		(unsigned long long)tee_base,
		(unsigned long long)(tee_base + tee_size - 1),
		(unsigned long long)tee_size);
	}
