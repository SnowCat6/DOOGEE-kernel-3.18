...
#else
	{ARMPLL_DEF_FREQ, 0, 9, 0, 8, 0xF6000}, //0 ~ -8%
#endif
	{0,0,0,0,0,0} //EOF
};
...
static void wait_mempll_dds_stable(
    unsigned int target_dds,
    unsigned long reg_mon,
    unsigned int wait_count)
{
...
static int mt_fh_hal_dvfs(enum FH_PLL_ID pll_id, unsigned int dds_value)
{
	...
	//FH_MSG("5. write back to ncpo");
    {
        ...
        reg_pll_con1 = g_reg_pll_con1[pll_id];       
				...
				if(pll_id == FH_MEM_PLLID) 
    			{
    					FH_MSG_DEBUG("Org MEMPLL_CON1:0x%08x  MEMPLL_CON1>>11_DDS: 0x%08x", fh_read32(reg_pll_con1), ((fh_read32(reg_pll_con1)&0xFFFFFFFE)>>11)&MASK21b);
    					if (fh_read32(reg_pll_con1)&0x1)
    					{
    	    				fh_write32(reg_pll_con1,(((fh_read32(g_reg_dds[pll_id])&MASK21b)<<11)&0xFFFFF800));
							}
							else
							{
									fh_write32(reg_pll_con1,(((fh_read32(g_reg_dds[pll_id])&MASK21b)<<11)&0xFFFFF800)|0x1);
							}     	    			
        	    FH_MSG_DEBUG("New MEMPLL_CON1:0x%08x  MEMPLL_CON1>>11_DDS: 0x%08x", fh_read32(reg_pll_con1), ((fh_read32(reg_pll_con1)&0xFFFFFFFE)>>11)&MASK21b);
	  			}
				else
					{
...
static int mt_fh_hal_dfs_armpll(unsigned int pll, unsigned int dds)
{
	...
	if(g_fh_pll[pll].fh_status == FH_FH_ENABLE_SSC){
		...
		pll_dds =  (DRV_Reg32(g_reg_dds[pll])) & MASK21b;
		fh_dds	=  (DRV_Reg32(g_reg_mon[pll])) & MASK21b;
...
static int mt_fh_hal_dfs_mempll(unsigned int target_dds)
{
	...
	if(g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC)
    {
        ...
        wait_dds_stable(pll_dds, g_reg_mon[pll_id], 100);
			...
			fh_write32(REG_FHCTL2_CFG, (DRV_Reg32(REG_FHCTL2_CFG)|0x20));
...
static int fh_dvfs_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    case FH_ARM_PLLID:
    	  ...
    	  mt_fh_hal_dfs_mempll((DRV_Reg32(REG_FHCTL2_DDS)&MASK21b));
...
if (DRV_Reg32(REG_FHCTL2_CFG)&0x20)	
	...
	seq_printf(m, "ARM:0x%08x MAIN:0x%08x  ",
			DRV_Reg32(REG_ARMPLL_CON0),	
			DRV_Reg32(REG_MAINPLL_CON0));
	seq_printf(m,  "MM:0x%08x VENC:0x%08x MSDC:0x%08x TVD:0x%08x\r\n",
			DRV_Reg32(REG_MMPLL_CON0),	DRV_Reg32(REG_VENCPLL_CON0),
			DRV_Reg32(REG_MSDCPLL_CON0),	DRV_Reg32(REG_TVDPLL_CON0));


	seq_printf(m, "\r\nPLL_CON1 :\r\n");
	seq_printf(m, "ARM:0x%08x MAIN:0x%08x MEM:0x%08x ",
			DRV_Reg32(REG_ARMPLL_CON1),	
			DRV_Reg32(REG_MAINPLL_CON1),	DRV_Reg32(REG_MEMPLL_CON1));
	seq_printf(m, "MM:0x%08x VENC:0x%08x MSDC:0x%08x TVD:0x%08x\r\n",
			DRV_Reg32(REG_MMPLL_CON1),	DRV_Reg32(REG_VENCPLL_CON1),
			DRV_Reg32(REG_MSDCPLL_CON1),	DRV_Reg32(REG_TVDPLL_CON1));
...
static int __fh_debug_proc_read(struct seq_file* m, void* v, fh_pll_t * pll)
{
	...
	seq_printf(m, "id=ARMPLL=MAINPLL=MEMPLL=MMPLL=VENCPLL=MSDCPLL=TVDPLL\r\n" );
