...
static int _mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    if (mt_gpufreq_dvfs_table_type == 0)    // 600M for 6735/6753, 550M for 6735M
        _mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_e1_0));
    else if (mt_gpufreq_dvfs_table_type == 1)    // 450M
        _mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_e1_1));
    else
        _mt_setup_gpufreqs_table(OPPS_ARRAY_AND_SIZE(mt_gpufreq_opp_tbl_e1_1));
...
static ssize_t mt_gpufreq_opp_freq_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d", &fixed_freq) == 1)
    {
        if (fixed_freq == 0) {
            ...
            g_is_vcorefs_on = is_vcorefs_can_work();
            if((!can_do_gpu_dvs()) && mt_gpufreq_dvfs_table_type == 0) {
                unsigned int limited_freq = mt_gpufreqs[g_gpufreq_max_id_vcorefs_off].gpufreq_khz;
