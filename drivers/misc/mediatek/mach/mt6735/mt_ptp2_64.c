...
static unsigned long ptp2_phy_base;
#endif
...
#define PTP2_BASEADDR_PHYS    ((ptp2_phy_base) + 0x600)
#define PTP2_CTRL_REG_BASEADDR_PHYS   (PTP2_BASEADDR_PHYS + 0x70)
#define PTP2_CTRL_REG_0_PHYS          (PTP2_CTRL_REG_BASEADDR_PHYS + 0x8)     // 0x78
#define PTP2_CTRL_REG_1_PHYS          (PTP2_CTRL_REG_BASEADDR_PHYS + 0xc)     // 0x7C
#define PTP2_CTRL_REG_2_PHYS          (PTP2_CTRL_REG_BASEADDR_PHYS + 0x10)    // 0x80
#define PTP2_CTRL_REG_3_PHYS          (PTP2_CTRL_REG_BASEADDR_PHYS + 0x14)    // 0x84
#define PTP2_CTRL_REG_4_PHYS          (PTP2_CTRL_REG_BASEADDR_PHYS + 0x18)    // 0x88
...
#define ptp2_read(addr)	DRV_Reg32(addr)
	...
	#define ptp2_write(addr, val)    mcusys_smc_write_phy(addr##_PHYS, val)
	#define ptp2_write_field(addr, range, val)    mcusys_smc_write_phy(addr##_PHYS, ((ptp2_read(addr) & ~(BITMASK(range))) | BITS(range, val)))
#else
...
void enable_LO(void)
{
	...
	if( (ptp2_ctrl_lo[0] & 0x03) == 0 )
		config_LO_CTRL(PTP2_RAMPSTART_3, 1, 1, 0, 0, 1, 1, 0xE, 0xE, 0xE, 8, 0x0, 0x0, 0x0, 0); //For >= 4 core on and best case; Fix me
	else
	{
        config_LO_CTRL(
...
static int __init ptp2_init(void)
{
    ...
    if (!node) 
    {
        ptp2_err("%s(), error: cannot find node ", __func__); 
    }

    if (of_address_to_resource(node, 0, &r)) 
    {
        ptp2_err("%s(), error: cannot get phys addr", __func__);
    }
    ptp2_phy_base = r.start;
    ptp2_warn("[PTP2 PHYSICAL] ptp2_phy_base=0x%p\n",(void *)ptp2_phy_base);
    
    if(node){
	/* Setup IO addresses */
	ptp2_base = of_iomap(node, 0);
	ptp2_warn("[PTP2 VIRTUAL] ptp2_base=0x%p\n",(void *)ptp2_base);
    }
