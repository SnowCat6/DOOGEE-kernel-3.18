...
int emmc_rpmb_req_write_data(struct mmc_card *card, struct rpmb_ioc_param *param)
{
    ...
    while (left_blkcnt) {
        ...
        rpmb_frame = (struct s_rpmb *)kzalloc(tran_blkcnt * sizeof(struct s_rpmb) + tran_blkcnt * 512, 0);
        if (rpmb_frame == NULL)
...
static int emmc_rpmb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    int err = 0;
    struct mmc_card *card = mtk_msdc_host[0]->mmc->card;//emmc_rpmb_host->card;
    ...
    switch (cmd) {
        ...
        case RPMB_IOCTL_READ_DATA:
            ...
            ret = emmc_rpmb_req_read_data(mtk_msdc_host[0]->mmc->card, &param);

            copy_to_user((void*)arg, &param, sizeof(param));
        ...
        case RPMB_IOCTL_WRITE_DATA:
            ...
            ret = emmc_rpmb_req_write_data(mtk_msdc_host[0]->mmc->card, &param);
