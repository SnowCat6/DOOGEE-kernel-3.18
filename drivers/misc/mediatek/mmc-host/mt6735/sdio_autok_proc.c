...
int wait_sdio_autok_ready(void *data){
    ...
    if (1/*(btmod!=META_BOOT) && (btmod!=FACTORY_BOOT) && (btmod!=ATE_FACTORY_BOOT)*/){
        ...
        if( cancel_delayed_work_sync(&(host->set_vcore_workq)) == 0){ //true if dwork was pending, false otherwise
        	pr_warn("** no pending vcore_workq\n");
        }else{
        	pr_warn("** cancel vcore_workq\n");
        }
	//ALPS02017456        	
    ...
    for (i=0; i<HOST_MAX_NUM; i++) {
        if (p_autok_thread_data->p_autok_progress[i].host_id == host->id) {
            if (p_autok_thread_data->p_autok_progress[i].done == 1 &&
                ...
                u32 vcore_uv = 0;
                vcore_uv = autok_get_current_vcore_offset();
		pr_warn("init_tune_sdio@msdc_autok_apply_param, id:%d vcore_uv:%d\n",host->id, vcore_uv);
                        ...
                        const char *buf, size_t count)
{
    ...
    switch(select){
        ...
        case PARAMS:
            ...
            pr_err("[AUTOKD] p_single_autok[%d].vol_count=%d", id, p_single_autok[id].vol_count);
            pr_err("[AUTOKD] p_single_autok[%d].param_count=%d", id, p_single_autok[id].param_count);
            for(i=0; i<p_single_autok[id].vol_count; i++){
                pr_err("[AUTOKD] p_single_autok[%d].vol_list[%d]=%d", id, i, p_single_autok[id].vol_list[i]);
            }
            for(i=0; i<p_single_autok[id].vol_count; i++){
                for(j=0; j<p_single_autok[id].param_count; j++)
                    pr_err("[AUTOKD] p_single_autok[%d].ai_data[%d][%d]=%d", id, i, j, p_single_autok[id].ai_data[i][j].data.sel);
            }
