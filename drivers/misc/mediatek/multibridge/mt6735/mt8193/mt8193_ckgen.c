...
{
	pr_info("[CKGEN] mt8193_ckgen_early_suspend() enter\n");
	...
	pr_info("[CKGEN] bus clock switch\n");
	...
	pr_info("[CKGEN] mt8193_ckgen_early_suspend() exit\n");
}

static void mt8193_ckgen_late_resume(struct early_suspend *h)
{
	pr_info("[CKGEN] mt8193_ckgen_late_resume() enter\n");
	...
	pr_info("[CKGEN] mt8193_ckgen_late_resume() exit\n");
}
...
static long mt8193_ckgen_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_info("[CKGEN] cmd=%s, arg=0x%08lx\n", _mt8193_ckgen_ioctl_spy(cmd), arg);
	...
	case MTK_MT8193_GPIO_CTRL:
	{
		...
		if (t_gpio.u4Mode == MT8193_GPIO_OUTPUT) {
			...
			pr_info("[CKGEN] GPIO INPUT VALUE IS %d\n", u4Val);
		}
...
static int __init mt8193_ckgen_init(void)
{
	...
	pr_info("[CKGEN] mt8193_ckgen_init() enter\n");
	...
	pr_info("[CKGEN] mt8193_ckgen_init() exit\n");
...
int mt8193_CKGEN_AgtOnClk(enum e_CLK_T eAgt)
{
	...
	pr_info("mt8193_CKGEN_AgtOnClk() %d\n", eAgt);
...
int mt8193_CKGEN_AgtOffClk(enum e_CLK_T eAgt)
{
	...
	pr_info("mt8193_CKGEN_AgtOffClk() %d\n", eAgt);
...
int mt8193_CKGEN_AgtSelClk(enum e_CLK_T eAgt, u32 u4Sel)
{
	...
	pr_info("mt8193_CKGEN_AgtSelClk() %d\n", eAgt);
...
{
	pr_info("mt8193_CKGEN_AgtGetClk() %d\n", eAgt);
	return 0;
}
...
int mt8193_ckgen_i2c_write(u16 addr, u32 data)
{
	...
	pr_info("mt8193_ckgen_i2c_write() 0x%x; 0x%x\n", addr, data);
...
u32 mt8193_ckgen_i2c_read(u16 addr)
{
	...
	pr_info("mt8193_ckgen_i2c_read() 0x%x; 0x%x\n", addr, u4_val);
	if (u4_ret != 0)
		pr_err("mt8193_i2c_read() fails!!!!!!\n");

	pr_info("mt8193_ckgen_i2c_read() 0x%x; value is 0x%x\n", addr, u4_val);
	return u4_val;
}
...
u32 mt8193_ckgen_measure_clk(u32 u4Func)
{
	...
	pr_info("[CKGEN] mt8193_ckgen_measure_clk() %d\n", u4Func);
	...
	pr_info("[CKGEN] Measure Done CLK [0X%X] for func [%d] delay count [0x%X]\n",
...
void mt8193_nfi_ana_pwr_control(bool power_on)
{
	...
	pr_info("[CKGEN] mt8193_nfi_ana_pwr_control() %d\n", power_on);
	if (power_on) {
...
void mt8193_bus_clk_switch(bool bus_26m_to_32k)
{
	...
	if (bus_26m_to_32k) {
		...
		pr_notice("[early_suspend] u4Tmp=0x%x\n", u4Tmp);
		...
		u4Tmp = CKGEN_READ32(REG_RW_DCXO_ANACFG9);
		u4Tmp &= (~(DCXO_ANACFG9_BUS_CK_SOURCE_SEL_MASK << DCXO_ANACFG9_BUS_CK_SOURCE_SEL_SHIFT));
		pr_notice("[late_resume] u4Tmp=0x%x\n", u4Tmp);
		CKGEN_WRITE32(REG_RW_DCXO_ANACFG9, u4Tmp);
...
void mt8193_bus_clk_switch_to_26m(void)
{
	...
	pr_info(" mt8193_bus_clk_switch_to_26m()\n");
...
void mt8193_disable_dcxo_core(void)
{
	...
	pr_info("mt8193_disable_dcxo_core()\n");
...
void mt8193_enable_dcxo_core(void)
{
	...
	pr_info("mt8193_enable_dcxo_core()\n");
...
static int mt8193_ckgen_suspend(struct platform_device *pdev, pm_message_t state)
{
	pr_info("[CKGEN] mt8193_ckgen_suspend() enter\n");
	...
	msleep(50);
#endif
	pr_info("[CKGEN] mt8193_ckgen_suspend() exit\n");
...
static int mt8193_ckgen_resume(struct platform_device *pdev)
{
	pr_info("[CKGEN] mt8193_ckgen_resume() enter\n");
	...
	msleep(20);
#endif
	pr_info("[CKGEN] mt8193_ckgen_resume() exit\n");
