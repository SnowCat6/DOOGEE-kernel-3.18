...
int mt8193_ckgen_config_pad_level_shift(int i4GroupNum, int i4TurnLow)
{
	...
	pr_info("[CKGEN] mt8193_ckgen_config_pad_level_shift() %d, %d\n", i4GroupNum, i4TurnLow);
...
u32 mt8193_ckgen_reg_rw_test(u16 addr)
{
	...
	pr_info("[CKGEN] mt8193_ckgen_reg_rw_test() 0x%x\n", addr);
	...
	pr_info("[CKGEN] mt8193_ckgen_reg_rw_test() success at 0x%x\n", addr);
...
u32 mt8193_io_agent_test(void)
{
	...
	if (u4Tmp == 0x55555555)
		pr_info("[CKGEN] TEST PASS at 0x18\n");
	else
	...
	if (u4Tmp == 0x55555555)
		pr_info("[CKGEN] TEST PASS at 0x408\n");
	else
	...
	if (u4Tmp == 0x55555555)
		pr_info("[CKGEN] TEST PASS at 0x608\n");
	else
	...
	if (u4Tmp == 0x00055555)
		pr_info("[CKGEN] TEST PASS at 0xa10\n");
	else
	...
	if (u4Tmp == 0xaaaaffff)
		pr_info("[CKGEN] TEST PASS at 0x1200\n");
	else
	...
	if (u4Tmp == 0x05550555)
		pr_info("[CKGEN] TEST PASS at 0x1608\n");
	else
	...
	if (u4Tmp == 0x55555555)
		pr_info("[CKGEN] TEST PASS at 0x1708\n");
	else
	...
	if (u4Tmp == 0x55555555)
		pr_info("[CKGEN] TEST PASS at 0x1d00\n");
	else
	...
	pr_info("[CKGEN] IO AGENT TEST FINISH ------------------------------------------\n");
...
{
	pr_info("[CKGEN] mt8193_spm_control_test()enters. %d\n", u4Func);
	...
	pr_info("[CKGEN] mt8193_spm_control() exit\n");
}
