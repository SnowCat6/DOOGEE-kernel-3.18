...
static ssize_t mt6605_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
	...
	//pr_debug("%s : %zu, &mt6605_dev=%p", __func__, count, mt6605_dev);
				...
				loff_t *offset)
{
	...
	do {
		...
		if (ret == count_ori) {
			...
			//pr_debug("%s :remain_bytes, %d,%d,%zu,%d,%d\n", __func__, ret_tmp, ret,
			//	 count, count_ori, idx);
		}
			...
			gpio_dir = mt_get_gpio_dir(gpio_num);
				...
				result = mt_get_gpio_in(gpio_num);	/*  */
                        ...
                        if (tmp_gpio == MTK_NFC_GPIO_IRQ && result == 0x01) {
                            printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl, irq is high call mt6605_dev_irq_handler.\n");
                            mt6605_dev_irq_handler();
                        }
