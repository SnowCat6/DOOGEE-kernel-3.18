...
static int param_valid(struct part_t *parts, int nr_parts)
{
    ...
    pgpt_start = 0;
    pgpt_size = (u64)(parts[0].start);

    sgpt_start = (u64)(parts[nr_parts-1].start + parts[nr_parts-1].size);
    sgpt_size = g_sdmmc_user_size - sgpt_start;

    if (pgpt_start != 0 || pgpt_size < 34 * 512 || pgpt_size % 512) {
        part_err("check param, err(pgpt_start = 0x%llx, pgpt_size = 0x%llx)\n", 
                        pgpt_start, pgpt_size);
        return 1;
    }

    if (sgpt_size < 33 * 512 || sgpt_size % 512) {
        part_err("check param, err(sgpt_size = 0x%llx)\n", sgpt_size);
        return 1;
    }
...
static gpt_entry *pack_entries_data(struct part_t *parts, int nr_parts)
{
    ...
    for (i = 0; i < nr_parts; i++) {
        ...
        entries[i].starting_lba = (u64)(parts[i].start / 512);
        entries[i].ending_lba = (u64)((parts[i].start + parts[i].size - 1) / 512);
