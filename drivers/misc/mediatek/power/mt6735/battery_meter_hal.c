...
// Auto adjust value
    if(R_FG_VALUE != 20)
    {
        bm_print(BM_LOG_FULL, "[fgauge_read_IM_current] Auto adjust value due to the Rfg is %d\n Ori current=%d, ", R_FG_VALUE, dvalue);
...
kal_int32 fgauge_set_columb_interrupt_internal(void *data, int reset)
{
	...
	bm_print(BM_LOG_FULL, "[fgauge_set_columb_interrupt] low:[0xcae]=0x%x  high:[0xcb0]=0x%x  %d %d \r\n", 
		pmic_get_register_value(PMIC_FG_BLTR),pmic_get_register_value(PMIC_FG_BFTR),
		pmic_get_register_value(PMIC_RG_INT_EN_FG_BAT_L),pmic_get_register_value(PMIC_RG_INT_EN_FG_BAT_H));		
		return STATUS_OK;
	}
	...
	{        
	    ...
	    if(m>1000)
	    {            
	        bm_print(BM_LOG_FULL, "[fgauge_set_columb_interrupt] fg_get_data_ready_status timeout 1 !");
	        break;
	    }
	}
   ...
   bm_print(BM_LOG_FULL, "[fgauge_set_columb_interrupt] FG_CAR = 0x%x   uvalue32_CAR_MSB:0x%x\r\n", uvalue32_CAR,uvalue32_CAR_MSB);
    ...
    bm_print(BM_LOG_FULL, "[fgauge_set_columb_interrupt] FG_CAR = 0x%x:%d  msb=0x%x  car=0x%x:%d FG_CAR2:0x%x:%d \r\n ",
...
*/
	bm_print(BM_LOG_FULL, "[fgauge_set_columb_interrupt] upper = 0x%x:%d  low=0x%x:%d  car=0x%x:%d\r\n", 
		upperbound,upperbound,lowbound,lowbound,pcar,pcar);

	upperbound=upperbound+pcar;
	lowbound=lowbound-pcar;

	bm_print(BM_LOG_FULL, "[fgauge_set_columb_interrupt]final upper = 0x%x:%d  low=0x%x:%d  car=0x%x:%d\r\n", 
		upperbound,upperbound,lowbound,lowbound,pcar,pcar);
