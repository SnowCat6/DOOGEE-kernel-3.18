...
kal_uint32 fan5405_reg_config_interface (kal_uint8 RegNum, kal_uint8 val)
{   
    int ret = 0;
...
static ssize_t store_fan5405_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        battery_log(BAT_LOG_FULL,"[store_fan5405_access] buf is %s and size is %d \n",buf,(int)size);
        reg_address = simple_strtoul(buf,&pvalue,16);
        
        if(size > 3)
        {        
            reg_value = simple_strtoul((pvalue+1),NULL,16);        
            battery_log(BAT_LOG_FULL,"[store_fan5405_access] write fan5405 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        {    
            ...
            battery_log(BAT_LOG_FULL,"[store_fan5405_access] read fan5405 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_fan5405);
            battery_log(BAT_LOG_FULL,"[store_fan5405_access] Please use \"cat fan5405_access\" to get value\r\n");
        }        
    }    
    return size;
}
...
{    
    ...
    {
        battery_log(BAT_LOG_CRTI,"[fan5405_init] failed to register fan5405 i2c driver.\n");
    }
    else
    {
        battery_log(BAT_LOG_CRTI,"[fan5405_init] Success to register fan5405 i2c driver.\n");
    }
