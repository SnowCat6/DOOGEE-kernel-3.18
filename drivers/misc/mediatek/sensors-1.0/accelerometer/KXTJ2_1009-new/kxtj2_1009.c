	...
	if (NULL == client) {
		...
		if (atomic_read(&priv->trace) & ADX_TRC_RAWDATA) {
			GSE_LOG("[%08X %08X %08X] => [%5d %5d %5d] after\n", data[KXTJ2_1009_AXIS_X],
				data[KXTJ2_1009_AXIS_Y], data[KXTJ2_1009_AXIS_Z], data[KXTJ2_1009_AXIS_X],
...
exit_KXTJ2_1009_CheckDeviceID:
	if (res < 0) {
		GSE_ERR("KXTJ2_1009_CheckDeviceID %d failt!\n ", KXTJ2_1009_ERR_I2C);
		return KXTJ2_1009_ERR_I2C;
	}
...
static int kxtj2_1009_init_client(struct i2c_client *client, int reset_cali)
{
	...
	if (res != KXTJ2_1009_SUCCESS)
		return res;
	/* printk("KXTJ2_1009_CheckDeviceID ok\n"); */
	...
	if (res != KXTJ2_1009_SUCCESS)
		return res;
	/* printk("KXTJ2_1009 disable interrupt function!\n"); */

	res = KXTJ2_1009_SetPowerMode(client, enable_status);	/* false);// */
	if (res != KXTJ2_1009_SUCCESS)
		return res;
	/* printk("KXTJ2_1009_SetPowerMode OK!\n"); */
...
static int kxtj2_1009_factory_set_cali(int32_t data[3])
{
	...
	/* obj */
	obj_i2c_data->cali_sw[KXTJ2_1009_AXIS_X] += data[0];
	obj_i2c_data->cali_sw[KXTJ2_1009_AXIS_Y] += data[1];
	obj_i2c_data->cali_sw[KXTJ2_1009_AXIS_Z] += data[2];
...
{
	data[0] = obj_i2c_data->cali_sw[KXTJ2_1009_AXIS_X];
	data[1] = obj_i2c_data->cali_sw[KXTJ2_1009_AXIS_Y];
	data[2] = obj_i2c_data->cali_sw[KXTJ2_1009_AXIS_Z];
	return 0;
}
