...
#define mtk_cooler_mdtxpwr_dprintk_always(fmt, args...) \
  do { pr_notice("thermal/cooler/mdtxpwr" fmt, ##args); } while(0)

#define mtk_cooler_mdtxpwr_dprintk(fmt, args...) \
  do { \
    if (1 == cl_mdtxpwr_klog_on) { \
      pr_notice("thermal/cooler/mdtxpwr" fmt, ##args); \
    } \
  } while(0)
...
static int cl_mdtxpwr_klog_on = 0;
...
static void 
mtk_cl_mdtxpwr_set_mdtxpwr_limit(void)
{
  // TODO: optimize
  int i = 0;
  int min_limit = 65535;
  ...
  if (min_limit != cl_mdtxpwr_cur_limit)
  {
    ...
    {
      int ret = 
        eemcs_notify_md_by_sys_msg(MD_SYS5, EXT_MD_TX_PWR_REDU_REQ, 30); // TODO: 30db as unlimit...
      mtk_cooler_mdtxpwr_dprintk_always("mtk_cl_mdtxpwr_set_mdtxpwr_limit() ret %d limit=30\n", ret);
    }
    else
    {
      int ret = 
        eemcs_notify_md_by_sys_msg(MD_SYS5, EXT_MD_TX_PWR_REDU_REQ, cl_mdtxpwr_cur_limit);
      mtk_cooler_mdtxpwr_dprintk_always("mtk_cl_mdtxpwr_set_mdtxpwr_limit() ret %d limit=%d\n", cl_mdtxpwr_cur_limit);
    }
#endif
  }
}
...
static int 
mtk_cl_mdtxpwr_get_max_state(struct thermal_cooling_device *cdev,
                          unsigned long *state)
{        
  *state = 1;
...
static int 
mtk_cl_mdtxpwr_get_cur_state(struct thermal_cooling_device *cdev,
                          unsigned long *state)
{
...
static int 
mtk_cl_mdtxpwr_set_cur_state(struct thermal_cooling_device *cdev,
                          unsigned long state)
{
...
static int mtk_cooler_mdtxpwr_register_ltf(void)
{
  ...
  for (i = MAX_NUM_INSTANCE_MTK_COOLER_MDTXPWR; i-- > 0; )
  {
    ...
    cl_mdtxpwr_dev[i] = mtk_thermal_cooling_device_register(temp, 
                                                         (void*) &cl_mdtxpwr_state[i], // put mdtxpwr state to cooler devdata
                                                         &mtk_cl_mdtxpwr_ops);
  }
...
static int _mtk_cl_mdtxpwr_proc_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    char *p = buf;
...
static ssize_t _mtk_cl_mdtxpwr_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    int len = 0;
    char desc[128];
                    ...
                    &klog_on, &limit0, &limit1, &limit2))
    {
        ...
        if (limit0 >= -1) MTK_CL_MDTXPWR_SET_LIMIT(limit0, cl_mdtxpwr_state[0]);
        if (limit1 >= -1) MTK_CL_MDTXPWR_SET_LIMIT(limit1, cl_mdtxpwr_state[1]);
        if (limit2 >= -1) MTK_CL_MDTXPWR_SET_LIMIT(limit2, cl_mdtxpwr_state[2]);
...
static int __init mtk_cooler_mdtxpwr_init(void)
{
  ...
  /* create a proc file */
  {
    ...
    entry = create_proc_entry("driver/mtk-cl-mdtxpwr", S_IRUGO | S_IWUSR | S_IWGRP, NULL);
