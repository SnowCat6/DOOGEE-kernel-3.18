...
#define DELTA_TEMP 2000
	...
	{
		printk("[Power/BTSMDPA_Thermal] drop this data\n");
		t_ret = pre_temp1;
	}
	else if((pre_temp1!=0) && (((pre_temp1-t_ret)>= DELTA_TEMP) || ((t_ret-pre_temp1)>= DELTA_TEMP)))
	{
		mtkts_btsmdpa_dprintk("[Power/BTSMDPA_Thermal] t_ret=%d,(t_ret-pre_temp1)=%d,(pre_temp1-t_ret)=%d\n",t_ret,(t_ret-pre_temp1),(pre_temp1-t_ret));
		t_ret = pre_temp1;
	}
	else
	{
...
{
	//cancel timer
	//printk("mtkts_btsmdpa_cancel_thermal_timer \n");
...
{
	//printk("mtkts_btsmdpa_start_thermal_timer \n");
    // resume thermal framework polling when leaving deep idle
    if (thz_dev != NULL && interval != 0)
	    mod_delayed_work(system_freezable_wq, &(thz_dev->poll_queue), round_jiffies(msecs_to_jiffies(3000))); // 60ms
}
