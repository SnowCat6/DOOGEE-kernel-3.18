...
static int LTE_TS_MCU3_T=0;

static int CPU_TS_MCU2_R=0;
static int GPU_TS_MCU1_R=0;
static int LTE_TS_MCU3_R=0;
...
static void read_each_bank_TS(thermal_bank_name bank_num)
{
	...
	switch(bank_num){
	    ...
	    case THERMAL_BANK2:
            //Bank2 : LTE (TSMCU3)
			LTE_TS_MCU3_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR3,&LTE_TS_MCU3_R);
	        break;
		default:
            ///Bank0 : CPU (TSMCU2)
			CPU_TS_MCU2_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR2,&CPU_TS_MCU2_R);
...
*/
    ...
    curr_temp  = MAX((MAX(CPU_TS_MCU2_T,GPU_TS_MCU1_T)),LTE_TS_MCU3_T);


    //tscpu_printk("\n tscpu_update_tempinfo, T=%d,%d,%d,max T=%d\n", CPU_TS_MCU2_T, GPU_TS_MCU1_T, LTE_TS_MCU3_T,curr_temp);
	...
	aee_rr_rec_thermal_temp3(LTE_TS_MCU3_T/1000);
...
static enum hrtimer_restart tscpu_update_tempinfo(struct hrtimer *timer)
{
	...
	tscpu_met_lock(&flags);
	a_tscpu_all_temp[0]=CPU_TS_MCU2_T; //Bank0:	CPU			(TSMCU2)
