...
Bank2:	LTE			(TSMCU2)
*/
static int CPU_TS_MCU3_T=0;
static int GPU_TS_MCU1_T=0;
static int LTE_TS_MCU2_T=0;

static int CPU_TS_MCU3_R=0;
...
static void set_adaptive_cpu_power_limit(unsigned int limit)
{
    ...
    if (prv_adp_cpu_pwr_lim != adaptive_cpu_power_limit)
    {
        tscpu_printk("set_adaptive_cpu_power_limit %d, T=%d,%d,%d\n", (final_limit != 0x7FFFFFFF) ? final_limit : 0,
                     CPU_TS_MCU3_T, GPU_TS_MCU1_T, LTE_TS_MCU2_T);
...
*/
	...
	switch(ts_bank){
        case THERMAL_BANK0://CPU (TSMCU3)
            x_roomt = g_x_roomt[2];
            break;
        case THERMAL_BANK1://GPU (TSMCU1)
            x_roomt = g_x_roomt[0];
            break;
        case THERMAL_BANK2://SOC (TSMCU2)
            x_roomt = g_x_roomt[1];
            break;
...
static void thermal_config_TS_in_banks(thermal_bank_name bank_num)
{
	...
	switch(bank_num){
        case THERMAL_BANK0://CPU(TSMCU3)
            ...
            break;
        case THERMAL_BANK1://SOC + GPU(TSMCU1)
            thermal_config_Bank1_TS();
            break;
        case THERMAL_BANK2://LTE(TSMCU2)
            thermal_config_Bank2_TS();
            break;
        default:
            thermal_config_Bank0_TS();//CPU(TSMCU3)
            break;
    }
}
...
static void read_each_bank_TS(thermal_bank_name bank_num)
{
	...
	switch(bank_num){
	    case THERMAL_BANK0:
			//Bank0 : CPU (TSMCU3)
			CPU_TS_MCU3_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR3,&CPU_TS_MCU3_R);
	        break;
	    case THERMAL_BANK1:
			//Bank1 : SOC + GPU (TSMCU1)
			//read extra ABB here
			GPU_TS_MCU1_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR1,&GPU_TS_MCU1_R);
	        break;
	    case THERMAL_BANK2:
            //Bank2 : LTE (TSMCU2)
...
*/
    ...
    curr_temp  = MAX((MAX(CPU_TS_MCU3_T,GPU_TS_MCU1_T)),LTE_TS_MCU2_T);
	...
	aee_rr_rec_thermal_temp2(LTE_TS_MCU2_T/1000);
	aee_rr_rec_thermal_temp3(CPU_TS_MCU3_T/1000);
...
*/
	...
	if(ts_bank == THERMAL_BANK0){
		bank_T = CPU_TS_MCU3_T;
	}
	else if(ts_bank == THERMAL_BANK1){
		bank_T = GPU_TS_MCU1_T;
	}
	else if(ts_bank == THERMAL_BANK2){
		bank_T = LTE_TS_MCU2_T;
	}


#if 0
	printk("\n\n");
	printk("Bank 0 : CPU  (TS_MCU3 = %d)\n",CPU_TS_MCU3_T);
	printk("Bank 1 : GPU  (TS_MCU1 = %d)\n",GPU_TS_MCU1_T);
	printk("Bank 2 : SOC  (TS_MCU2 = %d)\n",SOC_TS_MCU2_T);
#endif
...
static int ktp_thread(void *arg)
{
    ...
    for(;;)
	{
		...
		bank0_T    = MAX(MAX(CPU_TS_MCU3_T,GPU_TS_MCU1_T),LTE_TS_MCU2_T);
...
static enum hrtimer_restart tscpu_update_tempinfo(struct hrtimer *timer)
{
	...
	tscpu_met_lock(&flags);
	a_tscpu_all_temp[0]=CPU_TS_MCU3_T; //Bank0:	CPU			(TSMCU3)
	a_tscpu_all_temp[1]=GPU_TS_MCU1_T; //Bank1:	SOC + GPU	(TSMCU1)
	a_tscpu_all_temp[2]=LTE_TS_MCU2_T; //Bank2:	LTE			(TSMCU2)
	tscpu_met_unlock(&flags);
