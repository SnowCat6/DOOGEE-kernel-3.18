...
Bank2 : LTE     (TS_MCU2, TS_ABB)
*/
static int CPU_TS_MCU2_T=0;
static int GPU_TS_MCU2_T=0;
static int GPU_TS_MCU1_T=0;
static int LTE_TS_MCU2_T=0;
static int LTE_TS_ABB_T=0;

static int CPU_TS_MCU2_R=0;
static int GPU_TS_MCU2_R=0;
static int GPU_TS_MCU1_R=0;
static int LTE_TS_MCU2_R=0;
static int LTE_TS_ABB_R=0;
...
static void set_adaptive_cpu_power_limit(unsigned int limit)
{
    ...
    {
        tscpu_printk("set_adaptive_cpu_power_limit %d, T=%d,%d,%d,%d,%d\n", (final_limit != 0x7FFFFFFF) ? final_limit : 0,
                     CPU_TS_MCU2_T, GPU_TS_MCU2_T, GPU_TS_MCU1_T, LTE_TS_MCU2_T, LTE_TS_ABB_T);
...
static void set_static_cpu_power_limit(unsigned int limit)
{
    ...
    {
        tscpu_printk("set_static_cpu_power_limit %d, T=%d,%d,%d,%d,%d\n", (final_limit != 0x7FFFFFFF) ? final_limit : 0,
                     CPU_TS_MCU2_T, GPU_TS_MCU2_T, GPU_TS_MCU1_T, LTE_TS_MCU2_T, LTE_TS_ABB_T);
...
static void thermal_config_TS_in_banks(thermal_bank_name bank_num)
{
	...
	switch(bank_num){
        case THERMAL_BANK0://CPU(TSMCU2)
            ...
            break;
        case THERMAL_BANK1://SOC + GPU(TSMCU2, TSMCU1)
            thermal_config_Bank1_TS();
            break;
        case THERMAL_BANK2://LTE(TSMCU2, TSABB)
            thermal_config_Bank2_TS();
            break;
        default:
            thermal_config_Bank0_TS();//CPU(TSMCU2)
            break;
    }
}
...
/*
	...
	tscpu_printk("[cal] g_o_vtsmcu1     = 0x%x\n",g_o_vtsmcu1);
...
//	temp0 = DRV_Reg32(0xF02061A0);43
//	temp1 = DRV_Reg32(0xF020619C);42
//	temp2 = DRV_Reg32(0xF02061A4);44
//0x1020619C  ->42
//0x102061A0  ->43
//0x102061A4  ->44

	temp0 = get_devinfo_with_index(43);
	temp1 = get_devinfo_with_index(42);
	temp2 = get_devinfo_with_index(44);
...
int get_immediate_gpu_wrap(void)
{
	...
	curr_temp = MAX(GPU_TS_MCU2_T, GPU_TS_MCU1_T);
...
int get_immediate_lte_wrap(void)
{
	...
	curr_temp = MAX(LTE_TS_MCU2_T, LTE_TS_ABB_T);

    tscpu_dprintk("get_immediate_lte_wrap curr_temp=%d\n", curr_temp);
...
{
	int temp=0,raw=0;
	//u64 name = (u64) tempmsr_name;

    //tscpu_dprintk("read_tc_raw_temp,tempmsr_name=0x%llx,ts_num=%d\n",name,ts_name);
...
static void read_each_bank_TS(thermal_bank_name bank_num)
{
	...
	//TS_MCU1 -> THERMAL_SENSOR1,
	//TS_MCU2 -> THERMAL_SENSOR2,
	//TS_ABB -> THERMAL_SENSOR3

	switch(bank_num){
	    case THERMAL_BANK0:
			//Bank0 : CPU     (TS_MCU2)
			CPU_TS_MCU2_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR2,&CPU_TS_MCU2_R);
	        break;
	    case THERMAL_BANK1:
			//Bank1 : SOC+GPU (TS_MCU2, TS_MCU1)
			//read extra ABB here
			GPU_TS_MCU2_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR2,&GPU_TS_MCU2_R);
			GPU_TS_MCU1_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR1,THERMAL_SENSOR1,&GPU_TS_MCU1_R);
	        break;
	    case THERMAL_BANK2:
            //Bank2 : LTE     (TS_MCU2, TS_ABB)
			LTE_TS_MCU2_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR2,&LTE_TS_MCU2_R);
			LTE_TS_ABB_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR1,THERMAL_SENSOR3,&LTE_TS_ABB_R);
	        break;
		default:
            ///Bank0 : CPU     (TS_MCU2)
			CPU_TS_MCU2_T = read_tc_raw_and_temp((volatile u32 *)TEMPMSR0,THERMAL_SENSOR2,&CPU_TS_MCU2_R);
...
Bank2 : LTE     (TS_MCU2, TS_ABB)
*/
	curr_temp  = MAX(MAX(CPU_TS_MCU2_T, GPU_TS_MCU1_T), LTE_TS_ABB_T);


    //tscpu_printk("\n tscpu_get_temp, T=%d,%d,%d,max T=%d\n", CPU_TS_MCU3_T, GPU_TS_MCU1_T, LTE_TS_MCU2_T,curr_temp);
	...
	//return all D3 sensors
	aee_rr_rec_thermal_temp1(GPU_TS_MCU1_T/1000);
	aee_rr_rec_thermal_temp2(CPU_TS_MCU2_T/1000);
	aee_rr_rec_thermal_temp3(LTE_TS_ABB_T/1000);
...
*/
	...
	if(ts_bank == THERMAL_BANK0){
		bank_T = CPU_TS_MCU2_T;
	}
	else if(ts_bank == THERMAL_BANK1){
		bank_T = MAX(GPU_TS_MCU2_T, GPU_TS_MCU1_T);
	}
	else if(ts_bank == THERMAL_BANK2){
		bank_T = MAX(LTE_TS_MCU2_T, LTE_TS_ABB_T);
	}
...
static int ktp_thread(void *arg)
{
	...
	bank0_T  = MAX(MAX(CPU_TS_MCU2_T,GPU_TS_MCU1_T), LTE_TS_ABB_T);
...
Bank2 : LTE     (TS_MCU2, TS_ABB)
*/
	a_tscpu_all_temp[0]=GPU_TS_MCU1_T;
	a_tscpu_all_temp[1]=CPU_TS_MCU2_T;
	a_tscpu_all_temp[2]=LTE_TS_ABB_T;
	tscpu_met_unlock(&flags);
...
static int get_io_reg_base(void)
{
    ...
    if(node){
		...
		//printk("[THERM_CTRL] auxadc_ts_base=0x%p\n",auxadc_ts_base);
    }
    ...
    if(node){
		...
		//printk("[THERM_CTRL] infracfg_ao_base=0x%p\n",infracfg_ao_base);
    }
    ...
    if(node){
		...
		//printk("[THERM_CTRL] apmixed_base=0x%p\n",apmixed_base);
    }
...
static int __init tscpu_init(void)
{
	...
	*/
    ...
    BUG_ON(clock_is_on(MT_CG_PERI_AUXADC) == 0x0);
