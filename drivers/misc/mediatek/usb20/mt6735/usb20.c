...
static void mt_usb_enable(struct musb *musb)
{
	...
	virt_enable++;
	DBG(0, "<%d,%d>,<%d,%d,%d,%d>\n", mtk_usb_power, musb->power, virt_enable, virt_disable, real_enable, real_disable);
...
void mt_usb_connect(void)
{
	...
	if(cable_mode != CABLE_MODE_NORMAL){
		...
		if((!musb_skip_charge_detect && charger_type == STANDARD_HOST) || cable_mode == CABLE_MODE_CHRG_ONLY){
...
#if defined(CONFIG_USBIF_COMPLIANCE)
	if (!mtk_musb || !mtk_musb->is_ready || mtk_musb->is_host || !mtk_musb->power || polling_vbus) {
		pr_debug("is ready %d is_host %d power %d\n", mtk_musb->is_ready, mtk_musb->is_host, mtk_musb->power);
		if (polling_vbus)
...
bool usb_cable_connected(void)
{
	...
	charger_type = mt_get_charger_type();
	DBG(0, "type(%d)\n", charger_type);
	if(!pmic_intr_enable && mtk_musb->is_ready ){
		pmic_enable_charger_detection_int(1);
		pmic_intr_enable = 1;
	}
...
static void uart_usb_switch_dump_register(void)
{
	...
	printk("[MUSB]addr: 0x110020B0 (UART0), value: %x\n\n", DRV_Reg8(ap_uart0_base + 0xB0));
}
...
static int mt_usb_dts_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(musb_clk)) {
		printk(KERN_WARNING "cannot get musb clock\n");
		return PTR_ERR(musb_clk);
	}else{
		printk(KERN_WARNING "get musb clock ok, prepare it\n");
		retval = clk_prepare(musb_clk);
		if(retval == 0){
			printk(KERN_WARNING "prepare done\n");
		}else{
			printk(KERN_WARNING "prepare fail\n");
			return retval;
		}
	}
#endif
	...
	if(usb20_phy_init_debugfs()){
		printk("usb20_phy_init_debugfs fail!\n");
	}
    return retval;
