...
static int usb_driving_capability_show(struct seq_file *s, void *unused)
{
	...
	val = usb20_phy_debugfs_read_val(OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL, MSK_RG_USB20_TERM_VREF_SEL, BIT_WIDTH_3, str); 
	...
	val = usb20_phy_debugfs_read_val(OFFSET_RG_USB20_VRT_VREF_SEL, SHFT_RG_USB20_VRT_VREF_SEL, MSK_RG_USB20_VRT_VREF_SEL, BIT_WIDTH_3, str); 
	if (!strncmp(str, STRNG_0_WIDTH_3, BIT_WIDTH_3)){
		MYDBG("%s case\n", STRNG_0_WIDTH_3);
		tmp_val = VAL_0_WIDTH_3;
	}
...
static int rg_usb20_hstx_srctrl_show(struct seq_file *s, void *unused)
{
	...
	val = usb20_phy_debugfs_read_val(OFFSET_RG_USB20_HSTX_SRCTRL, SHFT_RG_USB20_HSTX_SRCTRL, MSK_RG_USB20_HSTX_SRCTRL, BIT_WIDTH_3, str); 
	seq_printf(s, "%s", str);
	return 0;
}
...
static int rg_usb20_intr_en_show(struct seq_file *s, void *unused)
{
	...
	val = usb20_phy_debugfs_read_val(OFFSET_RG_USB20_INTR_EN, SHFT_RG_USB20_INTR_EN, MSK_RG_USB20_INTR_EN, BIT_WIDTH_1, str); 
	seq_printf(s, "%s", str);
	return 0;
}
		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	char			buf[18];
	u8 val, tmp_val;
	char str_rg_usb20_term_vref_sel[18], str_rg_usb20_vrt_vref_sel[18], *after;
	memset(buf, 0x00, sizeof(buf));
	MYDBG("\n");
	if (copy_from_user(&buf, ubuf, min_t(size_t, sizeof(buf) - 1, count)))
		return -EFAULT;
	val = simple_strtol(buf, &after, 10);
	MYDBG("simple_strtol cnt:%d, val(%d)\n", (u8)(after-buf), val);
	...
	usb20_phy_debugfs_write_width3(OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL, str_rg_usb20_term_vref_sel);
	usb20_phy_debugfs_write_width3(OFFSET_RG_USB20_VRT_VREF_SEL, SHFT_RG_USB20_VRT_VREF_SEL, str_rg_usb20_vrt_vref_sel);
	return count;
}
		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(&buf, ubuf, min_t(size_t, sizeof(buf) - 1, count)))
		return -EFAULT;
	usb20_phy_debugfs_write_width3(OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL, buf);
	return count;
}
		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(&buf, ubuf, min_t(size_t, sizeof(buf) - 1, count)))
		return -EFAULT;
	usb20_phy_debugfs_write_width3(OFFSET_RG_USB20_VRT_VREF_SEL, SHFT_RG_USB20_VRT_VREF_SEL, buf);
	return count;
}
