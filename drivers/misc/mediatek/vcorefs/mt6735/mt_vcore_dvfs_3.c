			       ...
			       const char *buf, size_t count)
{
	...
	if (!strcmp(cmd, "HPM") && val < VCORE_INVALID) {
		...
		mutex_lock(&vcorefs_mutex);
		if (!feature_en && uv >= opp_ctrl_table[OPPI_LOW_PWR].vcore_uv) {
...
static int init_vcorefs_pwrctrl(void)
{
	...
	for (i = 0; i < NUM_OPP; i++) {
		switch (i) {
			case OPPI_PERF:
				opp_ctrl_table[i].vcore_uv = vcore_pmic_to_uv(get_vcore_ptp_volt(VCORE_1_P_25_UV)); // 1.25
				opp_ctrl_table[i].ddr_khz = get_ddr_khz_by_steps(0); // 1466 or 1600(6753T)
				break;
			case OPPI_LOW_PWR:
				opp_ctrl_table[i].vcore_uv = vcore_pmic_to_uv(get_vcore_ptp_volt(VCORE_1_P_15_UV)); // 1.15
