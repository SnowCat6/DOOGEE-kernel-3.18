...
INT32 i4VDEC_HAL_WMV_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    ...
    vVDecWriteMC(u4VDecID, RW_MC_PP_ENABLE, 0x00000001);
    {
        ...
        printk("WMV_DecStart u4PIC_W_H 0x%x\n", u4PIC_W_H);
        printk("WMV_DecStart u4YStartAddr 0x%x\n", u4YStartAddr);
        printk("WMV_DecStart prDecPrm->u4PIC_SIZE_BS    0x%x\n", prDecPrm->u4PIC_SIZE_BS);
        printk("WMV_DecStart prDecPrm->u4UFO_LEN_SIZE_Y 0x%x\n", prDecPrm->u4UFO_LEN_SIZE_Y);
        printk("WMV_DecStart prDecPrm->u4PIC_SIZE_Y_BS  0x%x\n", prDecPrm->u4PIC_SIZE_Y_BS);

        printk("WMV_DecStart PHYSICAL 512 byte PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS  0x%x\n", PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS);
        printk("WMV_DecStart PHYSICAL 64  byte PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y 0x%x\n", PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y);

        //printk("WMV_DecStart PHYSICAL 512 byte u4AbsDramANc ((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS  0x%x\n",u4AbsDramANc((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS);
        //printk("WMV_DecStart PHYSICAL 64  byte u4AbsDramANc ((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y 0x%x\n",u4AbsDramANc((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y);
    ...
    printk("WMV_DecStart PP ENABLE1\n");
    printk("WMV_DecStart PP ENSABLE1\n");
    printk("WMV_DecStart PP ENSABLE1\n");
#else
...
#if VMMU_SUPPORT
        printk("[WMV] i4VDEC_HAL_WMV_DecStart, VMMUTable:[0x%x, 0x%x]\n", ((UINT32)_pucVMMUTable[u4VDecID]), (UINT32)PHYSICAL((ULONG)_pucVMMUTable[u4VDecID]));
