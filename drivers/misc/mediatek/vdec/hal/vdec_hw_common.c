...
#endif
{
    ...
    if (u4VDecType == VDEC_H265)
    {
        ...
        //vVDecWriteHEVCMISC(u4VDecID, 0x4, 0x10);    //K2 turn on MISC_2-9 CRC access
        vVDecWriteHEVCMISC(u4VDecID, 0x4, 0x110);    //Everest turn on MISC_95-102 CRC access
    ...
    if (u4VDecType != VDEC_H265)
    {
        ...
        vVDecWriteDV(u4VDecID, 0x178, 0x1f);  // MISC(94)
    ...
    if (szFunction)
    {
        ...
        if (DUMP_FOR_SIM)
        {
...
#endif
            {
                ...
                {
                    case DV_REG_OFFSET0:
                        printk("        RISCWrite_MISC(%d, 32'h%x);\n", u4Addr >> 2, u4Val);
                        break;
                    ...
                    case HEVC_VLD_REG_OFFSET0:
                        printk("        RISCWrite_HEVC_VLD(%d, 32'h%x);\n", u4Addr >> 2, u4Val);
                        break;
                    default:
        ...
        {
            //printk("%s(u4VDecID, 0x%08X, 0x%08X); /* 0x%08X */\n", szFunction, u4Addr, u4Val, u4Val, u4BaseReg);
    ...
    if (szFunction)
    {
        if (DUMP_FOR_SIM)
        {
...
#endif
            {
                ...
                {
                    case DV_REG_OFFSET0:
                        printk("        RISCRead_MISC(%d); /* return 0x%x */\n", u4Addr >> 2, u4Val);
                        break;
                    case VLD_REG_OFFSET0:
                    ...
                    case RM_VDEC_PP_BASE:
                        ...
                        break;
                    case HEVC_VLD_REG_OFFSET0:
                        printk("        RISCRead_HEVC_VLD(%d); /* return 0x%x */\n", u4Addr >> 2, u4Val);
                        break;
                    default:
        ...
        {
            //printk("%s(u4VDecID, 0x%08X); /* return 0x%08X */ /* 0x%08X */\n", szFunction, u4Addr, u4Val, u4BaseReg);
...
void vVDecSimDump(UINT32 u4VDecID, UINT32 u4OffsetAddr, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    switch (u4OffsetAddr)
    {
        ...
        case DV_REG_OFFSET0:
        case DV_REG_OFFSET1:
            printk("[VDEC]RISCWrite('VDEC_FULL_ADDR_DV + 4*%d, 32'h%x );  \n", u4Addr >> 2, u4Val);
            break;
