...
#define UVVP_VDEC_TEST_VERSION_4                0x00440004  // 2013/03/28 Chia-Mao Hung For hevc early emulation
...
#define MFV_HW_WRITE(ptr,data)     mt_reg_sync_writel(data,ptr)
...
#if 0
// linux reserve physical memory 512MB
phys_addr_t mydriver_p;
void __init vdec_dvt_memory_reserve()
{
    printk("vdec_dvt_memory_reserve + \n");
    mydriver_p = arm_memblock_steal(SZ_256M, SZ_1M);
    if (mydriver_p)
    {
        printk("[Vdec] memblock done, p: %p\n", mydriver_p);
    }
    else
    {
        printk("[Vdec] memblock fail\n");
    }
    printk("vdec_dvt_memory_reserve - \n");
}
#endif
...
int readFileOffset(struct file *fp, char *buf, int readlen, UINT32 u4Offset)
{
    ...
    {
        fp->f_op->llseek(fp, u4Offset, SEEK_SET);
...
int readFileSize(struct file *fp, char *buf, int readlen)
{
    ...
    if (fp->f_op && fp->f_op->read)
    {
        i4FileSize = fp->f_op->llseek(fp, 0, SEEK_END);
...
int m4u_v2p_new(unsigned int va)
{
    ...
    printk("pmd_none=%d, pmd_bad=%d, pmd_val=0x%x\n", pmd_none(*pmd), pmd_bad(*pmd), (UINT)pmd_val(*pmd));
...
static const UCHAR *g_pu1AllocSA = 0xeda00000; //0xe6000000; //0x90000000; //0x88000000; //0x05000000;
                           ...
                           unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case UVVP_VDEC_TEST_VERSION:
            ...
            if (_tFileListInfo[param->u4InstanceId].i4FileId != 0xFFFFFFFFFFFFFFFF)
            {
                // fgHDDFsCloseFile(u4InstID); // temp avoid system crash
                _tFileListInfo[param->u4InstanceId].i4FileId = 0xFFFFFFFFFFFFFFFF;
            }
            //FS mount
            fgHDDFsUnMount(0);
#endif
        ...
        case UVVP_VDEC_TEST_VERSION_2:
            ...
            printk("Allocated %d + %dMB in %d sec %d usec\n", TESTDATA_BUFFER_SZ / 1024 / 1024, TESTDATA_BUFFER_SZ / 1024 / 1024, (int)tv2.tv_sec, (int)tv2.tv_usec);
            ...
            do_gettimeofday(&tv2);
            calc_time_diff(&tv1, &tv2);
            printk("memcpy %dMB in %d sec %d usec\n", TESTDATA_BUFFER_SZ / 1024 / 1024, (int)tv2.tv_sec, (int)tv2.tv_usec);
        ...
        case UVVP_VDEC_TEST_VERSION_3:
            ...
            printk("own_crc 0x%08lx, crc 0x%08lx, %s\n", own_crc, crc, (own_crc == crc) ? "Pass" : "Fail");
        ...
        case UVVP_VDEC_TEST_VERSION_4:
            ...
            sscanf(tmp_buf, "%i %i %i %i %i %s %i %c" , &frame_start, &frame_end, &width, &height, &debug_mode, bitstream_name, &error_rate, &UFO_MODE);
