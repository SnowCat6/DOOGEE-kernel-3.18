...
void vWMVWrData2PC(UINT32 u4InstID, UCHAR *ptAddr, UINT32 u4Size)
{
    ...
    if (prWMVPPS->ucPicType != SKIPFRAME)
    {
...
#endif
        {
            ...
            sprintf(_bFileStr1[u4InstID][3], "/mnt/sdcard/VC1_ART/ufo_%d_bits_Y.out", _u4FileCnt[u4InstID]);
            ...
            //if(!fgIsMonoPic(_u4VDecID))
            {
                ...
                sprintf(_bFileStr1[u4InstID][4], "/mnt/sdcard/VC1_ART/ufo_%d_bits_CbCr.out", _u4FileCnt[u4InstID]);
                ...
                sprintf(_bFileStr1[u4InstID][3], "/mnt/sdcard/VC1_ART/ufo_%d_len_Y.out", _u4FileCnt[u4InstID]);
                ...
                sprintf(_bFileStr1[u4InstID][4], "/mnt/sdcard/VC1_ART/ufo_%d_len_CbCr.out", _u4FileCnt[u4InstID]);
...
int vH265DumpMem(UINT32 u4InstID, unsigned char *buf, unsigned int size , int  frame_num , unsigned int type , bool isTimeout)
{
    ...
    if (_ptH265CurrFBufInfo[u4InstID]->bIsUFOEncoded)
    {
        sprintf(filename_error, "%s_decoded_%d_UFO_%c.dat", bitstream_name, frame_num, YCM);
    }
    else
    {
        sprintf(filename_error, "%s_decoded_%d_%c.dat", bitstream_name, frame_num, YCM);
    }
...
void vH264WrData2PC(UINT32 u4InstID, UCHAR *ptAddr, UINT32 u4Size)
{
    ...
    //vGenerateDownScalerGolden(u4InstID, (UINT32)_pucDecWorkBuf[u4InstID],(UINT32)(_pucDecCWorkBuf[u4InstID]),u4Size);
#else
    u4NonSwapYBase = (UCHAR *)_pucDecWorkBuf[u4InstID];
    u4NonSwapCBase = (UCHAR *)(_pucDecWorkBuf[u4InstID] + _ptCurrFBufInfo[u4InstID]->u4DramPicSize);
#if VDEC_UFO_SUPPORT
    u4NonSwapYLengthBase = (UINT32)(_pucDecWorkBuf[u4InstID] + _tVerMpvDecPrm[u4InstID].u4PIC_SIZE_BS);
    u4NonSwapCLengthBase = (UINT32)(u4NonSwapYLengthBase + _tVerMpvDecPrm[u4InstID].u4UFO_LEN_SIZE_Y);
    ...
    {
        u4NonSwapYBase = (UCHAR *)_pucDecWorkBuf[u4InstID];
        u4NonSwapCBase = (UCHAR *)_pucDecCWorkBuf[u4InstID];
        ...
        u4SwapSrcY = (UCHAR *)_pucDecWorkBuf[u4InstID];
        u4SwapSrcC = (UCHAR *)(_pucDecWorkBuf[u4InstID] + _ptCurrFBufInfo[u4InstID]->u4DramPicSize);

        u4NonSwapTargY = (UCHAR *)_pucAddressSwapBuf[u4InstID];
        u4NonSwapTargC = (UCHAR *)_pucAddressSwapBuf[u4InstID] + 0x1FE000;
        vMPEG_InvAddressSwap(u4InstID,
