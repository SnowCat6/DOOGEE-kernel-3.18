...
void vHEVCCreateLostPicture(UINT32 u4InstID, INT32 i4LostPoc)
{
    ...
    {
        if (abs(_ptH265FBufInfo[u4InstID][j].i4POC - i4LostPoc) < closestPoc &&
            abs(_ptH265FBufInfo[u4InstID][j].i4POC - i4LostPoc) != 0 &&
            _ptH265FBufInfo[u4InstID][j].i4POC != pCurrSliceHdr->i4POC &&
            (!_ptH265FBufInfo[u4InstID][j].bIsCopied))
        {
    ...
    for (j = 0; j < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; j++)
    {
        if (abs(_ptH265FBufInfo[u4InstID][j].i4POC - i4LostPoc) == closestPoc &&  _ptH265FBufInfo[u4InstID][j].i4POC != pCurrSliceHdr->i4POC)
        {
            ...
            _ptH265FBufInfo[u4InstID][u4LostBFindex].bIsCopied = 1;
            if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.i4RAPOC == MAX_INT)
            {
                _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.i4RAPOC = i4LostPoc;
            }
            break;
        }
    }
    ...
    for (i = pCurrRPS->u4NumberOfNegativePictures + pCurrRPS->u4NumberOfPositivePictures; i < pCurrRPS->u4NumberOfPictures; i++)
    {
        ...
        for (j = 0; j < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; j++)
        {
            ...
            */
            ...
            {
                if ((_ptH265FBufInfo[u4InstID][j].bUsedAsLongTerm) && (_ptH265FBufInfo[u4InstID][j].i4POC & (~(H265_MAX_UINT << _ptH265FBufInfo[u4InstID][j].u4POCBits))) == (pCurrRPS->i4POC[i] & (~(H265_MAX_UINT << _ptH265FBufInfo[u4InstID][j].u4POCBits))))
                {
                    isAvailable = 1;
                }
            }
        ...
        if (!isAvailable)
        {
            for (j = 0; j < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; j++)
            {
                ...
                {
                    if (((_ptH265FBufInfo[u4InstID][j].i4POC & (~(H265_MAX_UINT << _ptH265FBufInfo[u4InstID][j].u4POCBits))) == (pCurrRPS->i4POC[i] & (~(H265_MAX_UINT << _ptH265FBufInfo[u4InstID][j].u4POCBits)))) && _ptH265FBufInfo[u4InstID][j].bFirstSliceReferenced)
                    {
                        isAvailable = 1;
                        _ptH265FBufInfo[u4InstID][j].ucFBufRefType = LREF_PIC;
                        _ptH265FBufInfo[u4InstID][j].bUsedAsLongTerm = 1;
...
void vHEVCRefreshMarking(UINT32 u4InstID, UINT32 u4NalType, INT32 i4CurrPOC)
{
    ...
    {
        if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bRefreshPending == 1 &&
            pCurrSliceHdr->i4POC > _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.i4CRAPOC) // CRA reference marking pending
        {
            for(i=0; i<_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; i++)
            {
                vVerifyClrFBufInfo(u4InstID, i );
            }
            _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bRefreshPending = 0;
        }
        if ( u4NalType == NAL_UNIT_CODED_SLICE_CRA ) // CRA picture found
        {
            _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bRefreshPending = 1;
        }
    }
    */

    if (u4NalType == NAL_UNIT_CODED_SLICE_IDR || u4NalType == NAL_UNIT_CODED_SLICE_IDR_N_LP)   // IDR picture found
    {
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.i4CRAPOC = pCurrSliceHdr->i4POC;
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bPrevRAPisBLA = 0;
    }
    else if (u4NalType == NAL_UNIT_CODED_SLICE_CRA)   // CRA picture found
    {
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.i4CRAPOC = pCurrSliceHdr->i4POC;
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bPrevRAPisBLA = 0;
    }
    else if (u4NalType == NAL_UNIT_CODED_SLICE_BLA
             || u4NalType == NAL_UNIT_CODED_SLICE_BLANT
             || u4NalType == NAL_UNIT_CODED_SLICE_BLA_N_LP)  // BLA picture found
    {
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.i4CRAPOC = pCurrSliceHdr->i4POC;
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bPrevRAPisBLA = 1;
    }
...
UINT32 vHEVCPrepareRefPiclist(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
    ...
    if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.i4RAPOC == MAX_INT)  // start of random access point, m_pocRandomAccess has not been set yet.
    {
...
UINT32 vHEVCVerifyVDecSetPicInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
    ...
    if (ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->rSPS_PTL.generalPTL.u4ProfileIdc == 3 ||
        ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS->rSPS_PTL.generalPTL.bProfileCompatibilityFlag[3] == 1)
    {
        vVDEC_HAL_H265_SetStillImageInfo(u4InstID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);
    }
#endif
...
UINT32 vHEVCVDecGetLTRefPicbyPOC(UINT32 u4InstID, INT32 i4POC, BOOL bMSBpresentFlag)
{
    ...
    i4POCCycle = ~(H265_MAX_UINT << (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4Log2MaxPicOrderCntLsbMinus4 + 4));
    ...
    for (i = 0; i < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; i++)
    {
        ...
        if (_ptH265FBufInfo[u4InstID][i].ucFBufStatus != NO_PIC && _ptH265FBufInfo[u4InstID][i].i4POC != pCurrSliceHdr->i4POC)
        {
            ...
            if (!bMSBpresentFlag)
            {
                pic_poc = pic_poc & (~(H265_MAX_UINT << _ptH265FBufInfo[u4InstID][i].u4POCBits));
            }
...
void vHEVCVDecApplyRefPicList(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
    ...
    for (j = 0; j < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; j++)
    {
        ...
        for (; i < pCurrRPS->u4NumberOfPictures; i++)
        {
            ...
            {
                if ((_ptH265FBufInfo[u4InstID][j].ucFBufRefType == LREF_PIC) && ((_ptH265FBufInfo[u4InstID][j].i4POC & (~(H265_MAX_UINT << _ptH265FBufInfo[u4InstID][j].u4POCBits))) == (pCurrRPS->i4POC[i] & (~(H265_MAX_UINT << _ptH265FBufInfo[u4InstID][j].u4POCBits)))))
...
// *********************************************************************
// Function    : void vHEVCResetFBAddr(UINT32 u4InstID)
// Description : Reset frame buffer address
// Parameter   : None
// Return      : None
// *********************************************************************
void vHEVCResetFBAddr(UINT32 u4InstID, UINT32 u4FBindex)
{
    BOOL bIsUFO;
    ...
    UINT32 u4PicSizeYLCBS, u4PicSizeYLBS;
    UINT32 log2_max_cu_size, max_cu_size;
    H265_SPS_Data *prSPS;
    H265_PPS_Data *prPPS;
    ...
    u4DramMvSize = (u4DramPicSize) >> 4;

    if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4BitDepthLumaMinus8 > 0 ||
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4BitDepthChromaMinus8 > 0)
    {
        ...
        if (bIsUFO)
        {
            ...
            u4UFOLenCsize = (((u4UFOLenYsize >> 1) + 15) >> 4) << 4;

            u4PicSizeYLBS = ((u4DramPicSize + u4UFOLenYsize + 8191) >> 13) << 13;            //C 8192 align
            u4PicSizeYLCBS = ((u4PicSizeYLBS + (u4DramPicSize >> 1) + 63) >> 6) << 6;;         //CL 64 align
            u4DramPicYCSize  = (((u4PicSizeYLCBS + u4UFOLenCsize) + 511) >> 9) << 9;          //MV 512 align
            u4DramPicArea = ((u4DramPicYCSize +  u4DramMvSize + 8191) >> 13) << 13;        //Y 8192 align
        }
        else
        {
            u4DramPicYCSize = ((u4DramPicSize * 3 / 2 + 1023) >> 10) << 10; //1024 align
    ...
    */
    ...
    _ptH265FBufInfo[u4InstID][u4FBindex].u4Addr = ((UINT32)_pucDPB[u4InstID]) + (u4FBindex * u4DramPicArea);
    _ptH265FBufInfo[u4InstID][u4FBindex].u4YStartAddr = _ptH265FBufInfo[u4InstID][u4FBindex].u4Addr;
    _ptH265FBufInfo[u4InstID][u4FBindex].u4CAddrOffset = u4DramPicSize;

    if (bIsUFO)
    {
        _ptH265FBufInfo[u4InstID][u4FBindex].u4CAddrOffset = u4PicSizeYLBS;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4PicSizeCBS = u4DramPicSize >> 1;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4UFOLenYsize = u4UFOLenYsize;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4PicSizeYBS = u4DramPicSize;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4UFOLenCsize  = u4UFOLenCsize;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4YLenStartAddr = _ptH265FBufInfo[u4InstID][u4FBindex].u4YStartAddr + u4DramPicSize;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4CLenStartAddr = _ptH265FBufInfo[u4InstID][u4FBindex].u4YStartAddr + u4PicSizeYLCBS;
    }
    _ptH265FBufInfo[u4InstID][u4FBindex].u4MvStartAddr = _ptH265FBufInfo[u4InstID][u4FBindex].u4Addr + u4DramPicYCSize;
...
void vHEVCPartitionDPB(UINT32 u4InstID)
{
    ...
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum = (UCHAR)((DPB_SZ / u4DramPicArea > 255) ? 255 : DPB_SZ / u4DramPicArea);
    ...
    {
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum = (UCHAR)_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4NumRefFrames;
        printk("// ==============================================================================\n");
    ...
    printk("[INFO] [H265] Inst%d, u4DramPicSize:0x%x, u4DramMvSize:0x%x, u4DramPicArea:0x%x\n", u4InstID, u4DramPicSize, u4DramMvSize, u4DramPicArea);
    if (bIsUFO)
    {
        printk("[INFO] [H265]    u4UFOLenYsize:0x%x, u4UFOLenCsize:0x%x, u4PicSizeYLBS:0x%x, u4PicSizeYLCBS:0x%x\n", u4UFOLenYsize, u4UFOLenCsize, u4PicSizeYLBS, u4PicSizeYLCBS);
    }
    ...
    for (i = 0; i < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; i++)
    {
        ...
        if (bIsUFO)
        {
            _ptH265FBufInfo[u4InstID][i].u4CAddrOffset = u4PicSizeYLBS;
            _ptH265FBufInfo[u4InstID][i].u4PicSizeCBS = u4DramPicSize >> 1;
            _ptH265FBufInfo[u4InstID][i].u4UFOLenYsize = u4UFOLenYsize;
            _ptH265FBufInfo[u4InstID][i].u4PicSizeYBS = u4DramPicSize;
            _ptH265FBufInfo[u4InstID][i].u4UFOLenCsize  = u4UFOLenCsize;
            _ptH265FBufInfo[u4InstID][i].u4YLenStartAddr = _ptH265FBufInfo[u4InstID][i].u4YStartAddr + u4DramPicSize;
            _ptH265FBufInfo[u4InstID][i].u4CLenStartAddr = _ptH265FBufInfo[u4InstID][i].u4YStartAddr + u4PicSizeYLCBS;
            ...
            printk("[INFO] [H265] Inst%d, buf %d use UFO CBuffer 0x%x (0x%x)\n", u4InstID, i, _ptH265FBufInfo[u4InstID][i].u4CLenStartAddr, PHYSICAL(_ptH265FBufInfo[u4InstID][i].u4CLenStartAddr));
        }
...
UINT32 vHEVCAllocateFBuf(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm, BOOL fgFillCurrFBuf)
{
    ...
    if (fgFillCurrFBuf)
    {
        ...
        vHEVCResetFBAddr(u4InstID, _tVerMpvDecPrm[u4InstID].ucDecFBufIdx);
...
// *********************************************************************
// Function    : UINT32 vHEVCParseNALs(UINT32 u4InstID)
// Description : Parse NAL test
// Parameter   : None
// Return      : None
// *********************************************************************
...
UINT32 vHEVCSearchRealPic(UINT32 u4InstID)
{
    ...
    do
    {
        ...
        */
        ...
        _testCnt = u4VDEC_HAL_H265_ReadRdPtr(_u4BSID[u4InstID], u4InstID, (UINT32)_pucVFifo[u4InstID], &u4Bits);
#endif
        ...
        switch (_ucNalUnitType[u4InstID])
        {
            ...
            case NAL_UNIT_PPS:
                ...
                break;
#if HEVC_EOS_CLEAR_DPB
            case NAL_UNIT_EOS:
                printk("[INFO] NAL End of Sequence!! Clear DPB!! \n");
                // mark all pictures as not used for reference as random access
                for (u4Temp = 0; u4Temp < 25; u4Temp++)
                {
                    _ptH265FBufInfo[u4InstID][u4Temp].ucFBufStatus = NO_PIC;
                }
...
UINT32 vHEVCVerifyPPS_Rbsp(UINT32 u4InstID)
{
    ...
    for (sizeId = 0; sizeId < SCALING_LIST_SIZE_NUM; sizeId++)
    {
        for (listId = 0; listId < H265_scalingListNum[sizeId]; listId++)
        {
            ...
            if (pH265_PPS_DataInst->bSL_Init == NULL)
            {
    ...
    pH265_PPS_DataInst->bDeblockingFilterOverrideEnabledFlag = 0;
    // init end
...
UINT32 vHEVCParseSliceHeader(UINT32 u4InstID)
{
    ...
    if (0 == pH265_Slice_Hdr_DataInst->bDependentSliceSegmentFlag)
    {
        ...
        else
        {
...
#if DEBUG_LEVEL >= DBG_LEVEL_INFO
            DBG_H265_PRINTF(pfLogFile, "[INFO] iPrevPOC = %d; iPrevPOClsb = %d; iPrevPOCmsb = %d; iMaxPOClsb = %d\n", iPrevPOC, iPrevPOClsb, iPrevPOCmsb, iMaxPOClsb);
#endif
...
#if DEBUG_LEVEL >= DBG_LEVEL_INFO
            DBG_H265_PRINTF(pfLogFile, "[INFO] iPOCmsb %d, iPOClsb %d; \n", iPOCmsb, iPOClsb);
            ...
            {
#if DEBUG_LEVEL >= DBG_LEVEL_INFO
                DBG_H265_PRINTF(pfLogFile, "[INFO] bTemporalLayerNonReferenceFlag 0\n");
#endif
