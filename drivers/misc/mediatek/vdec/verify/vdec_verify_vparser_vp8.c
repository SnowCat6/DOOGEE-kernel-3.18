...
// Cheng-Jung 20131020 prevent kernel stack overflow
#define STACK_FRAME_LIMIT

#ifdef STACK_FRAME_LIMIT
static VDEC_INFO_VP8_FRM_HDR_T prVDecVp8FrmHdrPhysical;
static CHAR _bFileAddStrY[20] = {".y.dat\0"};
static CHAR _bFileAddStrC[20] = {".c.dat\0"};
#ifdef  SATA_HDD_READ_SUPPORT
static CHAR _bGoldFileName[256] = {"/post_\0"};
static CHAR _bPathAddStr[256] = {"\\Pattern\\gp\\"};
#else
static CHAR _bGoldFileName[256] = {"\\post_\0"};
static CHAR _bPathAddStr[256] = {"gp\\"};
#endif
...
void vVerInitVP8(UINT32 u4InstID)
{
    ...
    rVp8VDecInitPrm.u4VFifoSa = (ULONG)_pucVFifo[u4InstID];
    rVp8VDecInitPrm.u4VFifoEa = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ - 1;
    ...
    //    i4VDEC_HAL_VP8_InitVDecHW(u4InstID, &rVp8VDecInitPrm);
    prVDecVp8FrmHdr->u4FifoStart = (ULONG)_pucVFifo[u4InstID];
    prVDecVp8FrmHdr->u4FifoEnd = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ - 1;
    ...
    prVDecVp8FrmHdr->u4VldStartPos = (ULONG)_pucVFifo[u4InstID] + VP8_IVF_FILE_HEADER_SZ + VP8_IVF_FRAME_HEADER_SZ; // ivf file header + ivf frame header
#ifndef  RING_VFIFO_SUPPORT
    prVDecVp8FrmHdr->u4WritePos = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ - 1;
#else
    //    prVDecVp8FrmHdr->u4WritePos = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
    prVDecVp8FrmHdr->u4WritePos = (ULONG)_pucVFifo[u4InstID] + ((_u4LoadBitstreamCnt[u4InstID] % 2) ? (V_FIFO_SZ) : (V_FIFO_SZ >> 1));
#endif
...
void vVerVP8SetBufStatus(UINT32 u4InstID, VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr)
{
    ...
    */
    ...
    prVDecVp8FrmHdr->u4VLDWrapper = (ULONG)_pucVLDWrapper[u4InstID];
    prVDecVp8FrmHdr->u4SegIdWrapper = (UINT32)_pucSegIdWrapper[u4InstID];
    prVDecVp8FrmHdr->u4PPWrapperY = (ULONG)_pucPPWrapperY[u4InstID];
    prVDecVp8FrmHdr->u4PPWrapperC = (ULONG)_pucPPWrapperC[u4InstID];
#endif
    prVDecVp8FrmHdr->u4VLDWrapperWrok = (ULONG)_pucVLDWrapperWrok[u4InstID];
    prVDecVp8FrmHdr->u4PPWrapperWrok = (ULONG)_pucPPWrapperWork[u4InstID];

    switch (_u4VP8AltBufIdx[u4InstID])
    {
        case VP8_FRAME_CURRENT:
        {
            prVDecVp8FrmHdr->u4AlfYAddr = (ULONG)_pucWorkYBuf[u4InstID];
            switch (_u4VP8GoldfBufIdx[u4InstID])
            {
                case VP8_FRAME_CURRENT:
                {
                    prVDecVp8FrmHdr->u4GldYAddr = (ULONG)_pucWorkYBuf[u4InstID];
                    switch (_u4VP8LastBufIdx[u4InstID])
                    {
                        case VP8_FRAME_CURRENT:
                            prVDecVp8FrmHdr->u4LstYAddr = (ULONG)_pucWorkYBuf[u4InstID];
                            _u4VP8CurrBufIdx[u4InstID] = VP8_FRAME_LAST;
                            prVDecVp8FrmHdr->u4CurYAddr = (ULONG)_pucDumpLstYBuf[u4InstID];
                            //prVDecVp8FrmHdr->u4CurCAddr= (UINT32)_pucDumpLstCBuf[u4InstID];
                            break;
                        case VP8_FRAME_LAST:
                            prVDecVp8FrmHdr->u4LstYAddr = (ULONG)_pucDumpLstYBuf[u4InstID];
                            _u4VP8CurrBufIdx[u4InstID] = VP8_FRAME_GOLD;
                            prVDecVp8FrmHdr->u4CurYAddr = (ULONG)_pucDumpGldYBuf[u4InstID];
                            //prVDecVp8FrmHdr->u4CurCAddr= (UINT32)_pucDumpGldYBuf[u4InstID];
                            break;
                        case VP8_FRAME_GOLD:
                            prVDecVp8FrmHdr->u4LstYAddr = (ULONG)_pucDumpGldYBuf[u4InstID];
                            _u4VP8CurrBufIdx[u4InstID] = VP8_FRAME_LAST;
                            prVDecVp8FrmHdr->u4CurYAddr = (ULONG)_pucDumpLstYBuf[u4InstID];
                            //prVDecVp8FrmHdr->u4CurCAddr= (UINT32)_pucDumpLstCBuf[u4InstID];
                            break;
                        case VP8_FRAME_ALT:
                            prVDecVp8FrmHdr->u4LstYAddr = (ULONG)_pucDumpArfYBuf[u4InstID];
                            _u4VP8CurrBufIdx[u4InstID] = VP8_FRAME_LAST;
                            prVDecVp8FrmHdr->u4CurYAddr = (ULONG)_pucDumpLstYBuf[u4InstID];
                            //prVDecVp8FrmHdr->u4CurCAddr= (UINT32)_pucDumpLstCBuf[u4InstID];
                            break;
                ...
                case VP8_FRAME_LAST:
                {
                    prVDecVp8FrmHdr->u4GldYAddr = (ULONG)_pucDumpLstYBuf[u4InstID];
                    switch (_u4VP8LastBufIdx[u4InstID])
                    {
                        case VP8_FRAME_CURRENT:
                            prVDecVp8FrmHdr->u4LstYAddr = (ULONG)_pucWorkYBuf[u4InstID];
                        ...
                        case VP8_FRAME_GOLD:
                            ...
                            prVDecVp8FrmHdr->u4CurYAddr = (ULONG)_pucDumpArfYBuf[u4InstID];
                ...
                case VP8_FRAME_GOLD:
                {
                    prVDecVp8FrmHdr->u4GldYAddr = (ULONG)_pucDumpGldYBuf[u4InstID];
                    switch (_u4VP8LastBufIdx[u4InstID])
                    {
                        case VP8_FRAME_CURRENT:
                            prVDecVp8FrmHdr->u4LstYAddr = (ULONG)_pucWorkYBuf[u4InstID];
                ...
                case VP8_FRAME_ALT:
                {
                    prVDecVp8FrmHdr->u4GldYAddr = (ULONG)_pucDumpArfYBuf[u4InstID];
                    switch (_u4VP8LastBufIdx[u4InstID])
                    {
                        case VP8_FRAME_CURRENT:
                            prVDecVp8FrmHdr->u4LstYAddr = (ULONG)_pucWorkYBuf[u4InstID];
        ...
        case VP8_FRAME_LAST:
        {
            prVDecVp8FrmHdr->u4AlfYAddr = (ULONG)_pucDumpLstYBuf[u4InstID];
            switch (_u4VP8GoldfBufIdx[u4InstID])
            {
                case VP8_FRAME_CURRENT:
                {
                ...
                case VP8_FRAME_GOLD:
                {
                    ...
                    switch (_u4VP8LastBufIdx[u4InstID])
                    {
                        ...
                        case VP8_FRAME_ALT:
                            ...
                            prVDecVp8FrmHdr->u4CurYAddr = (ULONG)_pucWorkYBuf[u4InstID];
        ...
        case VP8_FRAME_GOLD:
        {
            prVDecVp8FrmHdr->u4AlfYAddr = (ULONG)_pucDumpGldYBuf[u4InstID];
            switch (_u4VP8GoldfBufIdx[u4InstID])
            {
                case VP8_FRAME_CURRENT:
                {
        ...
        case VP8_FRAME_ALT:
        {
            prVDecVp8FrmHdr->u4AlfYAddr = (ULONG)_pucDumpArfYBuf[u4InstID];
            switch (_u4VP8GoldfBufIdx[u4InstID])
            {
                case VP8_FRAME_CURRENT:
                {
                    ...
                    for (i = 0; i < 16; i++)
                    {
                        ...
                        else
                        {
                            if ((*(pbDecBuf)) != (*(pbGoldenBuf)))
                            {
                                ...
                                u4YPix = _u4MbH * 16 + j;
                                vVDecOutputDebugString("Pic count to [%d]\n", _u4FileCnt[u4InstID]);
                                vVDecOutputDebugString("Y Data Mismatch at [x= 0x%x, y=0x%x] = 0x%x, Golden = 0x%x !!! \n", u4XPix, u4YPix, (*pbDecBuf), (*pbGoldenBuf));
                                sprintf(strMessage, "Y Data Mismatch at [x= 0x%x, y=0x%x] = 0x%x, Golden = 0x%x !!! \n", u4XPix, u4YPix, (*pbDecBuf), (*pbGoldenBuf));
                                fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tFileListRecInfo[u4InstID]);
                                //fprintf(_tFileListRecInfo.fpFile, "Y Data Mismatch at [0x%.8x] = 0x%.2x, Golden = 0x%.2x !!! \n", i, pbDecBuf[i], _pucDumpBuf[i]);
                                fgDecErr = TRUE;
                                //vDumpReg();  // mark by ginny
...
void vVerVP8VDecEnd(UINT32 u4InstID)
{
    ...
    u4VldByte = u4VDEC_HAL_VP8_ReadRdPtr(0, u4InstID, (ULONG)_pucVFifo[u4InstID], &u4VldBit) - 4;
    ...
    if (_u4ReDecCnt[u4InstID] > 0)
    {
        rVp8VFifoInitPrm.u4VFifoSa = (ULONG)_pucVFifo[u4InstID];
        rVp8VFifoInitPrm.u4VFifoEa = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
        i4VDEC_HAL_VP8_InitVDecHW(u4InstID, &rVp8VFifoInitPrm);
        _rVDecVp8FrmHdr[u4InstID].u4FifoStart = (ULONG)_pucVFifo[u4InstID];
        _rVDecVp8FrmHdr[u4InstID].u4FifoEnd = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
        _rVDecVp8FrmHdr[u4InstID].u4VldStartPos = (ULONG)_pucVFifo[u4InstID] + _u4VP8ByteCount[u4InstID];
#ifndef  RING_VFIFO_SUPPORT
        _rVDecVp8FrmHdr[u4InstID].u4WritePos = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ;
#else
        //    _rVDecVp8FrmHdr[u4InstID].u4WritePos = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
        _rVDecVp8FrmHdr[u4InstID].u4WritePos = (ULONG)_pucVFifo[u4InstID] + ((_u4LoadBitstreamCnt[u4InstID] % 2) ? (V_FIFO_SZ) : (V_FIFO_SZ >> 1));
#endif
    ...
    if (fgNextFrameExist)
    {
        ...
        rVp8VFifoInitPrm.u4VFifoSa = (ULONG)_pucVFifo[u4InstID];
        rVp8VFifoInitPrm.u4VFifoEa = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ - 1;
        i4VDEC_HAL_VP8_InitVDecHW(u4InstID, &rVp8VFifoInitPrm);
        _rVDecVp8FrmHdr[u4InstID].u4FifoStart = (ULONG)_pucVFifo[u4InstID];
        _rVDecVp8FrmHdr[u4InstID].u4FifoEnd = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ - 1;
        _rVDecVp8FrmHdr[u4InstID].u4VldStartPos = (ULONG)_pucVFifo[u4InstID] + _u4VP8ByteCount[u4InstID];
#ifndef  RING_VFIFO_SUPPORT
        _rVDecVp8FrmHdr[u4InstID].u4WritePos = (ULONG)_pucVFifo[u4InstID] + V_FIFO_SZ - 1;
#else
        //    _rVDecVp8FrmHdr[u4InstID].u4WritePos = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
        _rVDecVp8FrmHdr[u4InstID].u4WritePos = (ULONG)_pucVFifo[u4InstID] + ((_u4LoadBitstreamCnt[u4InstID] % 2) ? (V_FIFO_SZ) : (V_FIFO_SZ >> 1));
#endif
...
#if (VDEC_VP8_ERR_TEST)
        u4Size0 = *((ULONG *)_pucSizeFileBuf[u4InstID] + (((_u4FileCnt[u4InstID] + 1) * 4)));
        u4Size1 = *((ULONG *)_pucSizeFileBuf[u4InstID] + (((_u4FileCnt[u4InstID] + 1) * 4) + 1));
        u4Size2 = *((ULONG *)_pucSizeFileBuf[u4InstID] + (((_u4FileCnt[u4InstID] + 1) * 4) + 2));
        u4Size3 = *((ULONG *)_pucSizeFileBuf[u4InstID] + (((_u4FileCnt[u4InstID] + 1) * 4) + 3));
        ...
        if ((_u4LoadBitstreamCnt[u4InstID] & 0x1)
            && (_rVDecVp8FrmHdr[u4InstID].u4ReadPointer  > ((ULONG)_pucVFifo[u4InstID] + (V_FIFO_SZ / 2))))
        {
        ...
        else if ((!(_u4LoadBitstreamCnt[u4InstID] & 0x1))
                 && (_rVDecVp8FrmHdr[u4InstID].u4ReadPointer  < ((ULONG)_pucVFifo[u4InstID] + (V_FIFO_SZ / 2))))
        {
            _tInFileInfo[u4InstID].fgGetFileInfo = TRUE;
...
void vVerVP8UpdateBufStatus(UINT32 u4InstID)
{
    ...
    else if (prVDecVp8FrmHdr->rVp8DecParam.uCopyBuf2Arf == 2) //gld to arf
    {
        if (_fgVP8DumpReg)
        {
            printk("Gold --> Alt\n");
    ...
    else if (prVDecVp8FrmHdr->rVp8DecParam.uCopyBuf2Gf == 2) // alt to gf
    {
        if (_fgVP8DumpReg)
        {
            printk("Alt --> Gold\n");
