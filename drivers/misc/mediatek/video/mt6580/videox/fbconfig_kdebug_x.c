...
typedef struct PM_TOOL_ST PM_TOOL_T;
struct PM_TOOL_ST {
	DSI_INDEX dsi_id;
	LCM_REG_READ reg_read;
...
*/
	...
	case FB_LAYER_DUMP:
	{
		...
		OVL_BASIC_STRUCT ovl_all[4];
			...
			ovl_get_info(0, ovl_all);
			layer_size =
			    ovl_all[global_layer_id].src_pitch *
			...
			if ((layer_size != 0)
				...
				ret =
				    m4u_query_mva_info(mva, layer_size,
						       &real_mva, &real_size);
				if (ret < 0) {
					pr_debug
				...
				ret =
				    m4u_mva_map_kernel(real_mva, real_size,
						       &kva, &mapped_size);
				if (ret < 0) {
					pr_debug
				...
				pr_debug("==> addr from user space is 0x%x\n",
				       (unsigned int)argp);
				pr_debug
				    ("==> kva is 0x%lx real_mva %x mva %x mmaped size is %dlayer_size is %d\n",
				     kva, real_mva, mva, mapped_size,
				     layer_size);
				ret =
				    copy_to_user(argp,
						 (void *)kva + (real_mva - mva),
						 layer_size - (real_mva -
							       mva)) ? -EFAULT :
				    0;
