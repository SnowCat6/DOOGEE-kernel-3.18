...
static int mtkfb_get_overlay_layer_info(struct fb_overlay_layer_info
					*layerInfo);
					   ...
					   disp_input_config *dst)
{
	...
	/* set Alpha blending */
	dst->alpha = src->alpha;
	if (MTK_FB_FORMAT_ARGB8888 == src->src_fmt
	    || MTK_FB_FORMAT_ABGR8888 == src->src_fmt) {
	...
	"_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=%p, paddr=%p, src fmt=%u, dst fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
		dst->layer_id, dst->layer_enable, dst->next_buff_idx,
		dst->src_base_addr, dst->src_phy_addr, src->src_fmt, dst->src_fmt,
		dst->src_pitch, dst->src_offset_x, dst->src_offset_y,
		dst->src_width, dst->src_height);
	DISPDBG(
	"_convert_fb_layer_to_disp_input():target xoff=%u, target yoff=%u, target w=%u, target h=%u, aen=%u\n",
		dst->tgt_offset_x, dst->tgt_offset_y, dst->tgt_width,
		dst->tgt_height, dst->alpha_enable);

	return 0;
}
#if 0
static int _overlay_info_convert(struct fb_overlay_layer *src,
...
/* #if defined (MTK_FB_ION_SUPPORT) */
	if (src->src_phy_addr != NULL) {
		...
		dst->addr =
		    mtkfb_query_buf_mva(src->layer_id,
					(unsigned int)src->next_buff_idx);
	}
#else
	...
	DISPMSG(
	"_convert_fb_layer_to_disp_input():id=%u, en=%u, next_idx=%u, vaddr=0x%lx, paddr=0x%lx, fmt=%u, pitch=%u, xoff=%u, yoff=%u, w=%u, h=%u\n",
		dst->layer, dst->layer_en, dst->buff_idx, dst->addr, dst->vaddr,
		dst->fmt, dst->src_pitch, dst->src_x, dst->src_y, dst->src_w,
		dst->src_h);
	DISPMSG(
	"_convert_fb_layer_to_disp_input():target xoff=%u, target yoff=%u, target w=%u, target h=%u, aen=%u\n",
				  ...
				  struct fb_info *info)
{
	...
	DISPDBG
	    ("xoffset=%u, yoffset=%u, xres=%u, yres=%u, xresv=%u, yresv=%u\n",
	...
	case 32:
		input->src_fmt =
		    (0 ==
		     var->
		     blue.offset) ? DISP_FORMAT_BGRA8888 : DISP_FORMAT_RGBX8888;
		break;
	default:
...
static void set_fb_fix(struct mtkfb_device *fbdev)
{
	...
	fix->line_length =
	    ALIGN_TO(var->xres_virtual,
		     MTK_FB_ALIGNMENT) * var->bits_per_pixel / 8;
...
static int mtkfb_set_par(struct fb_info *fbi)
{
	...
	fb_layer.src_base_addr =
	    (void *)((unsigned long)fbdev->fb_va_base +
		       ...
		       unsigned long arg)
{
	...
	MMProfileLogEx(MTKFB_MMP_Events.IOCtrl, MMProfileFlagPulse,
		       _IOC_NR(cmd), arg);
	...
	case MTKFB_GET_POWERSTATE:
		{
			...
			if (copy_to_user
			    (argp, &power_state, sizeof(power_state))) {
	...
	case MTKFB_CONFIG_IMMEDIATE_UPDATE:
		{
			MTKFB_LOG
			    ("[%s] MTKFB_CONFIG_IMMEDIATE_UPDATE, enable = %lu\n", __func__, arg);
			if (down_interruptible(&sem_early_suspend)) {
				MTKFB_LOG
				    ("[mtkfb_ioctl] can't get semaphore:%d\n", __LINE__);
				return -ERESTARTSYS;
			}
			sem_early_suspend_cnt--;
	...
	case MTKFB_CAPTURE_FRAMEBUFFER:
		{
			...
			if (copy_from_user
			    (&pbuf, (void __user *)arg, sizeof(pbuf))) {
				MTKFB_LOG
				    ("[FB]: copy_from_user failed! line:%d\n", __LINE__);
				r = -EFAULT;
			} else {
				dprec_logger_start(DPREC_LOGGER_WDMA_DUMP, 0,
						   0);
				primary_display_capture_framebuffer_ovl(pbuf,
									eBGRA8888);
	...
	case MTKFB_SET_VIDEO_LAYERS:
		{
			...
			MMProfileLog(MTKFB_MMP_Events.SetOverlayLayers,
				     MMProfileFlagStart);
			...
			if (copy_from_user
				...
				for (i = 0; i < VIDEO_LAYER_COUNT; ++i) {
					...
					input =
					    ...
					    config
					    [session_input.config_layer_num++];
					_convert_fb_layer_to_disp_input
					    (&layerInfo[i], input);
	...
	case MTKFB_TRIG_OVERLAY_OUT:
		{
			...
			MMProfileLog(MTKFB_MMP_Events.TrigOverlayOut,
				     MMProfileFlagPulse);
			primary_display_trigger(1, NULL, 0);
	...
	case MTKFB_CHANGE_UPDATESPEED:
		{
			...
			if (copy_from_user
			    (&speed, (void __user *)arg, sizeof(speed))) {
				MTKFB_LOG
				    ("[FB]: copy_from_user failed! line:%d\n", __LINE__);
				r = -EFAULT;
			} else {
...
struct compat_fb_overlay_layer {
	...
	/* set to 1, will go through tdshp first, then layer blending, then to color */
...
static long mtkfb_compat_ioctl(struct fb_info *info, unsigned int cmd,
			       unsigned long arg)
{
	struct fb_overlay_layer layerInfo;
	long ret = 0;
	...
	case COMPAT_MTKFB_SET_OVERLAY_LAYER:
		{
			...
			MTKFB_LOG
			    (" mtkfb_compat_ioctl():MTKFB_SET_OVERLAY_LAYER\n");

			arg = (unsigned long)compat_ptr(arg);
			if (copy_from_user
			    (&compat_layerInfo, (void __user *)arg,
	...
	case COMPAT_MTKFB_SET_VIDEO_LAYERS:
		{
			...
			MTKFB_LOG
			    (" mtkfb_compat_ioctl():MTKFB_SET_VIDEO_LAYERS\n");
...
#define RGB565_TO_ARGB8888(x)   \
	((((x) &  0x1F) << 3) | (((x) &  0x7E0) << 5) | (((x) & 0xF800) << 8) | (0xFF << 24))		/* opaque */
...
static void _mtkfb_draw_point(unsigned int addr, unsigned int x, unsigned int y,
			      unsigned int color)
{
...
static int _mtkfb_internal_test(unsigned long va, unsigned int w,
				unsigned int h)
{
	...
	for (i = 0;
	     i < w * h / _internal_test_block_size / _internal_test_block_size;
	     i++) {
		color = (i & 0x1) * 0xff;
...
int _parse_tag_videolfb(void)
{
	...
	pr_debug("[DT][videolfb]isvideofb_parse_done = %d\n",
	...
	if (of_scan_flat_dt(fb_early_init_dt_get_chosen, NULL) > 0) {
		videolfb_tag =
		    (struct tag_videolfb *)of_get_flat_dt_prop(video_node,
							       ...
							       NULL);
		if (videolfb_tag) {
			pr_debug("[DT][videolfb] find video info from lk\n");
			...
			pr_debug("[DT][videolfb] islcmfound = %d\n",
			       islcmconnected);
			pr_debug("[DT][videolfb] fps        = %d\n", lcd_fps);
			pr_debug("[DT][videolfb] fb_base    = 0x%x\n", fb_base);
			pr_debug("[DT][videolfb] vram       = %d\n", vramsize);
			pr_debug("[DT][videolfb] lcmname    = %s\n",
			       mtkfb_lcm_name);
			return 0;
		}

		pr_debug("[DT][videolfb] of_chosen not found\n");
		return 1;
	}
...
static int mtkfb_probe(struct device *dev)
{
	...
	DISPMSG
	    ("[FB Driver] fbdev->fb_pa_base = 0x%pa, fbdev->fb_va_base = 0x%p\n",
...
#if 0
	if (disp_helper_get_stage() != DISP_HELPER_STAGE_NORMAL) {
		_mtkfb_internal_test(fbdev->fb_va_base, MTK_FB_XRES,
				     MTK_FB_YRES);
	}
#endif
