			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				while (bLockedHW == VAL_FALSE) {
					...
					if (grVcodecHWLock.pvHandle ==
					    (VAL_VOID_T *) pmem_user_v2p_video((VAL_ULONG_T)
									       rHWLock.pvHandle)) {
						MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, one decoder instance try to lock twice, may cause lock HW timeout!! instance = 0x%lx, CurrentTID = %d\n",
					...
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						...
						if (FirstUseDecHW != 1) {
							spin_lock_irqsave(&HWLock, ulFlagsHWLock);
							if (grVcodecHWLock.pvHandle == 0)
								MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, maybe mediaserver restart before, please check!!\n");
							else
								MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, someone use HW, and check timeout value!!\n");

							spin_unlock_irqrestore(&HWLock,
									       ulFlagsHWLock);
						}
					} else if (VAL_RESULT_RESTARTSYS == eValRet) {
						MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, VAL_RESULT_RESTARTSYS return when HWLock!!\n");
						return -ERESTARTSYS;
					}
					...
					if (grVcodecHWLock.pvHandle == 0)	{ /* No one holds dec hw lock now */
						if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_DEC ||
							...
							if (VAL_RESULT_INVALID_ISR == eValRet
								...
								/* --gu4VDecIRQCount; */
								MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, reset power when HWLock (%d)!!\n",
								     gu4VDecIRQCount);
							}
							vdec_power_on();
							...
							if (VAL_RESULT_INVALID_ISR == eValRet
							    && FirstUseDecHW != 1) {
								MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, reset power when HWLock!!\n");
								vdec_power_off();
								/* disable_irq(VDEC_IRQ_ID); */
								/* --gu4VDecIRQCount; */
							}
				...
				while (bLockedHW == VAL_FALSE) {
					...
					if (grVcodecHWLock.pvHandle ==
					    (VAL_VOID_T *) pmem_user_v2p_video((VAL_ULONG_T)
									       rHWLock.pvHandle)) {
						MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, one encoder instance try to lock twice, may cause lock HW timeout!! instance = 0x%lx, CurrentTID = %d, type:%d\n",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
					...
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						...
						if (FirstUseEncHW != 1) {
							...
							if (grVcodecHWLock.pvHandle == 0) {
								...
								MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW, someone use HW, and check timeout value!! %d\n",
								     gLockTimeOutCount);
								++gLockTimeOutCount;
					...
					if (VAL_TRUE == bLockedHW) {
						MODULE_MFV_LOGD("VCODEC_LOCKHW, Lock ok grVcodecHWLock.pvHandle = 0x%lx, va:%lx, type:%d",
						     (VAL_ULONG_T) grVcodecHWLock.pvHandle,
						     (VAL_ULONG_T) rHWLock.pvHandle,
						     rHWLock.eDriverType);
						gLockTimeOutCount = 0;
				...
				MODULE_MFV_LOGD("[WARNING] VCODEC_LOCKHW Unknown instance\n");
				return -EFAULT;
			}
			MODULE_MFV_LOGD("VCODEC_LOCKHW - tid = %d\n", current->pid);
		}
	...
	case VCODEC_UNLOCKHW:
		{
			...
			if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				MODULE_MFV_LOGD("[WARNING] VCODEC_UNLOCKHW Unknown instance\n");
				return -EFAULT;
			}
			MODULE_MFV_LOGD("VCODEC_UNLOCKHW - tid = %d\n", current->pid);
		}
			...
			if (val_isr.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
				...
				if (val_isr.eDriverType == VAL_DRIVER_TYPE_H264_DEC
					...
					if (VAL_RESULT_INVALID_ISR == eValRet) {
						...
						MODULE_MFV_LOGD("[WARNING] VCODEC_WAITISR, VAL_RESULT_RESTARTSYS return when WAITISR!!\n");
						return -ERESTARTSYS;
					}
				}
				...
				MODULE_MFV_LOGD("[WARNING] VCODEC_WAITISR Unknown instance\n");
				return -EFAULT;
			}
			MODULE_MFV_LOGD("VCODEC_WAITISR - tid = %d\n", current->pid);
		}
