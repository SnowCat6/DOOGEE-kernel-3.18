...
//#define ENABLE_MMDVFS_VDEC    // [Jackal, 2015/03/03, Disable MM DVFS Feature]
        ...
        case VCODEC_LOCKHW:
        {
                     ...
                     rHWLock.eDriverType == VAL_DRIVER_TYPE_JPEG_ENC)
            {
                while (bLockedHW == VAL_FALSE)
                {
                    ...
                    {
                        MFV_LOGE("VCODEC_LOCKHW, Lock ok grVcodecEncHWLock.pvHandle = 0x%lx, va:%lx, type:%d", (VAL_ULONG_T)grVcodecEncHWLock.pvHandle, (VAL_ULONG_T)rHWLock.pvHandle, rHWLock.eDriverType);
                        gLockTimeOutCount = 0;
                    }
                    mutex_unlock(&VencHWLock);
                }
