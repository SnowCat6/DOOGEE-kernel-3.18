...
#define MTKFB_LOG(fmt, arg...) \
    do { \
        if (mtkfb_log_on) pr_warn("DISP/MTKFB " fmt, ##arg); \
    }while (0)
...
#define MTKFB_FUNC()	\
	do { \
		if(mtkfb_log_on) pr_info("DISP/MTKFB " "[Func]%s\n", __func__); \
	}while (0)

#define PRNERR(fmt, args...)  pr_info("DISP/MTKFB " fmt, ##args);
    ...
    switch(cmd)
    {
		...
		case COMPAT_MTKFB_SET_OVERLAY_LAYER:
		{
			...
			else 
			{
				...
				if(primary_display_is_sleepd())
				{
			    		printk("[FB Driver] error, set overlay in early suspend ,skip! \n");
			    		return MTKFB_ERROR_IS_EARLY_SUSPEND;
				}
