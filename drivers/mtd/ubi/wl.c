...
static void wl_tree_add(struct ubi_device *ubi, struct ubi_wl_entry *e, struct rb_root *root)
{
...
retry:
	if (!ubi->free.rb_node) {
		if (ubi->works_count == 0) {
			ubi_err("1 no free eraseblocks");
			ubi_err("free count:%d", ubi->free_count);
