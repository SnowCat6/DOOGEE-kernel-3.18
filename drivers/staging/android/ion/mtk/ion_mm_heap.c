                                      ...
                                      void *unused)
{
    ...
    for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
        ...
        if (client->task) {
        	...
        	ION_PRINT_LOG_OR_SEQ(s, "client(0x%p) %s (from_kernel) pid(%u) ================>\n", client, client->name,client->pid);
        }

        {
            ...
            for (m = rb_first(&client->handles); m; m = rb_next(m)) {
                ...
                ION_PRINT_LOG_OR_SEQ(s, "\thandle=0x%p, buffer=0x%p, heap=%d, backtrace is:\n",
...
struct ion_heap *ion_mm_heap_create(struct ion_platform_heap *unused)
{
	...
	heap->cached_pools = kzalloc(sizeof(struct ion_page_pool *) * num_orders,
			      GFP_KERNEL);
