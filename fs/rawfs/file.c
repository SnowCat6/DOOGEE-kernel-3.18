...
/* loff_t size; */
	...
	RAWFS_PRINT(RAWFS_DBG_FILE,
		"rawfs_block_file_aio_write %s, pos %lld, len %d\n",
		inode_info->i_name, pos, retval);
...
/* struct address_space *mapping=filp->f_mapping; */
	...
	RAWFS_PRINT(RAWFS_DBG_FILE,
		"rawfs_reg_file_aio_read %s, pos %lld, len %d, filesize: %lld\n",
...
/* pg_buf = kmap_atomic(page); */
	...
	RAWFS_PRINT(RAWFS_DBG_FILE,
		"rawfs_readpage %s @ folder %X, page %ld, pos %lld, len %d, filesize: %d, pg_buf %X\n",
		inode_info->i_name,
