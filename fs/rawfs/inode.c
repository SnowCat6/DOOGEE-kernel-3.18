...
static void rawfs_attach(struct inode *inode, const char *name)
{
	...
	RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_attach %s %X to hlist[%d]=%X\n", name,
		(unsigned int)(inode), (unsigned)rawfs_hash(name), (unsigned)head);
...
static void rawfs_detach(struct inode *inode)
{
	...
	RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_detach %s %X from hlist\n",
		RAWFS_I(inode)->i_name, (unsigned int)(inode));
...
struct inode *rawfs_alloc_inode(struct super_block *sb)
{
	...
	RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_alloc_inode, %X\n", (unsigned int)(ei));
...
static void rawfs_i_callback(struct rcu_head *head)
{
	...
	RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_i_callback, %X\n",
		(unsigned int)(RAWFS_I(inode)));
	kmem_cache_free(rawfs_inode_cachep, RAWFS_I(inode));
}

void rawfs_destroy_inode(struct inode *inode)
{
	RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_destroy_inode, %X\n",
		(unsigned int)(inode));
...
/* struct hlist_node		*ptr; */
	...
	RAWFS_PRINT(RAWFS_DBG_INODE,
		"rawfs_iget, by name %s @ folder %X in inode_hashtable[%ld]=%X\n",
		name, folder, rawfs_hash(name), (unsigned int)head);
	...
	hlist_for_each_entry(inode_info, head, i_rawfs_hash) {
		RAWFS_PRINT(RAWFS_DBG_INODE,
			"rawfs_iget, inode_hashtable[%ld][%d] %s @ folder %X, %X -> %X\n",
			rawfs_hash(name), index,
			inode_info->i_name,
			inode_info->i_parent_folder_id,
			(unsigned int)(&inode_info->vfs_inode),
			(unsigned int)(inode_info));
...
/* BUG_ON(i->vfs_inode.i_sb != sb); */
		if (strnicmp(inode_info->i_name, name, RAWFS_MAX_FILENAME_LEN+4) == 0) {
			RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_iget, igrab %X\n",
				(unsigned int)(&inode_info->vfs_inode));
	...
	if (IS_ERR_OR_NULL(inode))
		...
		RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_iget, %s was found at %X\n", name,
		(unsigned)(inode));
	...
	struct rawfs_file_info *file_info, int block_no, int page_no)
{
	...
	inode->i_uid = file_info->i_uid;
	inode->i_gid = file_info->i_gid;
	...
	struct rawfs_file_info *file_info, int block_no, int page_no)
{
		...
		if (inode) {
			...
			if (entry) {
				...
				file_info->i_atime = file_info->i_mtime = file_info->i_ctime =
					CURRENT_TIME_SEC;
				file_info->i_uid  = current_uid();
				file_info->i_gid  = current_gid();
				file_info->i_size = 0;
	...
	if (IS_ERR_OR_NULL(inode))	{
		RAWFS_PRINT(RAWFS_DBG_INODE,
			"rawfs_build_inode: new_inode failed %X\n", (unsigned int)(inode));
		goto out;
	}
	inode->i_version = 1;
...
rawfs_mknod(struct inode *dir, struct dentry *dentry, umode_t mode, dev_t dev)
{
	...
	RAWFS_PRINT(RAWFS_DBG_INODE, "rawfs_mknod %s %X = %X, i_nlink=%d\n",
		dentry->d_name.name, mode, (unsigned int)inode, inode->i_nlink);
	if (inode) {
			RAWFS_PRINT(RAWFS_DBG_INODE,
		...
		struct inode *new_dir, struct dentry *new_dentry)
{
	...
	RAWFS_PRINT(RAWFS_DBG_DIR, "rawfs_dir_rename, %s (%08X) -> %s (%08X)\n",
		old_dentry->d_name.name, (unsigned int)old_dentry,
		new_dentry->d_name.name, (unsigned int)new_dentry);
	...
	if (inode)	{
		RAWFS_PRINT(RAWFS_DBG_DIR,
			"rawfs_dir_rename: found old inode %X, expected %X\n",
			(unsigned)inode, (unsigned)old_dentry->d_inode);
		iput(inode);
	...
	int block_no, int page_no, umode_t mode, dev_t dev)
{
	...
	case S_IFREG:
		if ((block_no >= 0) && (page_no < 0)) { /* Speical block file */
			RAWFS_PRINT(RAWFS_DBG_INODE,
				"rawfs_fill_inode: block file %d, %X -> %X\n",
				block_no, (unsigned)inode,
				(unsigned)inode_info);
