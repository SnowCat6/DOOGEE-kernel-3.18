...
static mt6350_Codec_Data_Priv *mCodec_data = NULL;
...
static int mt6350_codec_trigger(struct snd_pcm_substream *substream , int command , struct snd_soc_dai *Daiport)
{
    switch (command)
    {
        ...
        if (Speaker_mode != AUDIO_SPEAKER_MODE_RECEIVER)    //2in1 speaker do not need this
        {
            ...
            { // Add speaker ramp function
                ...
                if(index > currentIdx)
                {
                    ...
                    {
                        Ana_Set_Reg(SPK_CON9, i << 8, 0x0F00);
                        //printk("AnalogClose RampUp SPK_CON9 %x index= %d Cur = %d", Ana_Get_Reg(SPK_CON9), index, currentIdx);
                        msleep(3);
                    }
                }
                else if((index < currentIdx) && (currentIdx != 0))
                {
                    ...
                    {
                        Ana_Set_Reg(SPK_CON9, i << 8, 0x0F00);
                        //printk("AnalogClose RampDown SPK_CON9 %x index= %d Cur = %d", Ana_Get_Reg(SPK_CON9), index, currentIdx);
                        msleep(3);
                    }
                }
                ...
                if(index > currentIdx)
                {
                    for(i = (currentIdx+1); i <= index; i++)
                    {
                        ...
                        //printk("AnalogClose RampUp AUDTOP_CON7 %x index= %d Cur = %d", Ana_Get_Reg(AUDTOP_CON7), index, currentIdx);
                        msleep(3);
                    }
                }
                else if((index < currentIdx) && (currentIdx != 0))
                {
                    ...
                    {
                        Ana_Set_Reg(AUDTOP_CON7, i << 4, 0x00F0);
                        //printk("AnalogClose RampDown AUDTOP_CON7 %x index= %d Cur = %d", Ana_Get_Reg(AUDTOP_CON7), index, currentIdx);
                        msleep(3);
                    }
                }
...
static bool TurnOnADcPowerDmic(int ADCType, bool enable)
{
    ...
    if (enable)
    {
        ...
        if (GetAdcStatus() == false)
        {
            ...
            TopCtlChangeTrigger();
            SetMicPGAGain();
            //Ana_Set_Reg(ABB_AFE_CON9, 0x0011, 0x0011);  // enable digital mic, 3.25M clock rate, bit8:0(one wire)/1(two wire), one wire
            Ana_Set_Reg(ABB_AFE_CON9, 0x0111, 0x0111);  // enable digital mic, 3.25M clock rate, bit8:0(one wire)/1(two wire), two wire
            Ana_Set_Reg(ABB_AFE_CON0, 0x0002, 0x0002);  // turn on UL
...
static int mt6350_codec_probe(struct snd_soc_codec *codec)
{
    ...
    snd_soc_dapm_new_controls(dapm, mt6350_dapm_widgets,
                              ARRAY_SIZE(mt6350_dapm_widgets));
    snd_soc_dapm_add_routes(dapm, mtk_audio_map,
                            ARRAY_SIZE(mtk_audio_map));

    //add codec controls
    snd_soc_add_codec_controls(codec, mt6350_snd_controls,
                               ARRAY_SIZE(mt6350_snd_controls));
    snd_soc_add_codec_controls(codec, mt6350_UL_Codec_controls,
                               ARRAY_SIZE(mt6350_UL_Codec_controls));
    snd_soc_add_codec_controls(codec, mt6350_Voice_Switch,
                               ARRAY_SIZE(mt6350_Voice_Switch));
...
static int mt6350_Readable_registers(struct snd_soc_codec *codec,
                                     unsigned int reg)
{
    return 1;
}

static int mt6350_volatile_registers(struct snd_soc_codec *codec,
                                     unsigned int reg)
{
    return 1;
}
