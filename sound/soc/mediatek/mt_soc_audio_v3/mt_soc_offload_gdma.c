...
//static unsigned int pin_extspkamp, pin_vowclk, pin_audclk, pin_audmiso, pin_audmosi, pin_i2s1clk, pin_i2s1dat, pin_i2s1mclk, pin_i2s1ws;
//static unsigned int pin_mode_audclk, pin_mode_audmosi, pin_mode_audmiso, pin_mode_vowclk, pin_mode_extspkamp, pin_mode_i2s1clk, pin_mode_i2s1dat, pin_mode_i2s1mclk, pin_mode_i2s1ws;
//static unsigned int if_config1, if_config2, if_config3, if_config4, if_config5, if_config6, if_config7, if_config8, if_config9;
#endif
...
static struct snd_dma_buffer *Dl2_Playback_dma_buf  = NULL;
...
int mtk_compr_offload_gdma_read(void *stream)
{
    ...
    if(afe_offload_block.u4WriteIdx >= afe_offload_block.u4ReadIdx)
        ...
        avail_size = afe_offload_block.data_buffer_size + afe_offload_block.u4WriteIdx - afe_offload_block.u4ReadIdx;
    ...
    if (copy_size != 0)
    {
        ...
        if( afe_offload_block.state == OFFLOAD_STATE_RUNNING || afe_offload_block.state == OFFLOAD_STATE_DRAIN)
        {
            if(afe_offload_block.u4WriteIdx >= afe_offload_block.u4ReadIdx)
            {
        ...
        //PRINTK_AUD_DL2("%s, debug, :%x %p %x %p\n", __func__, Afe_WriteIdx_tmp, data_w_ptr, copy_size, Afe_Block->pucVirtBufAddr);
        ...
        else  // copy twice
        {
            ...
            //PRINTK_AUD_DL2("mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %p data_w_ptr = %p size_1 = %x\n",
            ...
            //PRINTK_AUD_DL2("mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %p data_w_ptr+size_1 = %p size_2 = %x\n",
            ...
            //PRINTK_AUD_DL2("AudDrv_write finish2, copy size:%x, WriteIdx:%x,ReadIdx=%x DataRemained:%x \r\n",
...
static int mtk_compr_offload_gdma_int_prepare(struct snd_compr_stream *stream)
{
    ...
    if (mPrepareDone == false)
    {
        ...
        if(afe_offload_block.pcmformat == SNDRV_PCM_FORMAT_S32_LE || afe_offload_block.pcmformat == SNDRV_PCM_FORMAT_U32_LE)
        {
        ...
        SetCLkBclk(MclkDiv3,  afe_offload_block.samplerate, afe_offload_block.channels, Soc_Aud_I2S_WLEN_WLEN_32BITS);
...
#else
    SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I07, Soc_Aud_InterConnectionOutput_O00);
    SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I08, Soc_Aud_InterConnectionOutput_O01);
    SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I07, Soc_Aud_InterConnectionOutput_O03);
    SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I08, Soc_Aud_InterConnectionOutput_O04);
#endif
...
static void SetDL2Buffer(void)
{
    ...
    printk("SetDL2Buffer u4BufferSize = %d pucVirtBufAddr = %p pucPhysBufAddr = 0x%x\n",
...
static int mtk_compr_offload_gdma_open(struct snd_compr_stream *stream)
{
    ...
    printk("%s\n", __func__);
    afe_offload_block.pcm_stream = (struct snd_pcm_substream *) kmalloc(sizeof(struct snd_pcm_substream), GFP_KERNEL);
    if( afe_offload_block.pcm_stream == NULL )
    {
        printk("%s, allocate pcm sub stream fail\n", __func__);
    ...
    printk("%s, mtk_pcm_dl2_hardware.buffer_bytes_max = %zu mPlaybackSramState = %d\n", 
    ...
    {
        printk("%s, SNDRV_COMPRESS_STREAM_PLAYBACK mtkcompress_dl2playback_constraints\n", __func__);
    }
    else
    {
        printk("%s, SNDRV_COMPRESS_STREAM_CAPTURE mtkcompress_dl2playback_constraints\n", __func__);
    }
			...
			struct snd_compr_params *params)
{
    ...
    afe_offload_block.data_buffer_area = (kal_int8*)vmalloc(afe_offload_block.data_buffer_size);

    if(afe_offload_block.data_buffer_area == NULL)
    {
        printk("%s fail to allocate data buffer, size:%x\n", __func__, afe_offload_block.data_buffer_size);
    }
	...
	afe_offload_block.temp_buffer_area = (kal_int8*)kmalloc(afe_offload_block.temp_buffer_size, GFP_KERNEL);
	if(afe_offload_block.temp_buffer_area == NULL)
    {
        printk("%s fail to allocate temp buffer, size:%x\n", __func__, afe_offload_block.temp_buffer_size);
    }

	SetDL2Buffer();

    PRINTK_AUD_DL2("%s, rate:%x, period:%x, hw_buf_size:%x area:%p addr:%x data_buf_size:%x\n",__func__, 
        afe_offload_block.samplerate, afe_offload_block.period_size, afe_offload_block.hw_buffer_size, 
        afe_offload_block.hw_buffer_area, afe_offload_block.hw_buffer_addr, afe_offload_block.data_buffer_size);
...
{
    //PRINTK_AUDDRV("%s:ridx:%x, total:%lu\n", __func__, afe_offload_block.u4ReadIdx, (unsigned long)afe_offload_block.copied_total);
